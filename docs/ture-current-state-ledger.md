...
| MVP-05b | Last success, freshness and a missed/failed run are visible with a working recovery path | unverified | Draft PR #457 local source evidence distinguishes a clean no-trade scan from a run needing recovery review and rejects unknown or stale data as a recovery point; a supported scan/recovery journey remains required. |
