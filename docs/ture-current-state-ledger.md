...
| --- | --- | --- |
| MVP-01 | Existing session gate, owner-bound dashboard API, preview sign-in/reload/sign-out evidence and authenticated private-staging dashboard readback | Unverified: MVP-01a and MVP-01b are verified; loading and empty states are observed. Three bounded MVP-01c attempts stayed normal, including one bound to the exact private staging site ID. A values-free diagnosis route built correctly but the current browser client blocked its protected `/api` navigation before the application. All temporary changes were rolled back; the remaining gap is the Next/Netlify environment-to-route boundary plus a supported authenticated diagnostic reader. |
...
Current acceptance coverage: **11/18 verified for the new MVP candidate; 7
...
| MVP-01c | Loading, empty and failed dashboard states are understandable | unverified | Private `ture-staging` revision `d1cb32f4`, 2026-09-11: authenticated readback showed the loading label `Refreshing…` and the clear empty no-trade explanation. A first query read against temporary revision `00015d14` had been built before its Functions-only flag was added. A second, separately bounded read against `4fbf2f0e` had the non-secret flag in Build and Functions before ready deploy `6aa37cd8f73dc100080fccd8`, but stayed normal. A third separately bounded read against `752c3cd8` used the same canonical query, pre-deploy Build-and-Functions flag and Netlify's exact staging `SITE_ID`; ready deploy `6aa37ff26475310008d45802` still rendered normal no-trade. The values-free diagnostic route in ready deploy `6aa381b821798d000865e27e` could not be read because the current browser client blocked its protected `/api` URL before the application. All flags and temporary routes/code were removed; rollback deploy `6aa3825e04c20a00082a5a02` is ready. |
...
smallest_change_and_reused_components: Reused the rollbacked exact-origin probe only on the `staging` branch, then replaced its generic deployment-context check with Netlify's documented private staging `SITE_ID` Functions identity. All variants required a non-secret enabled flag, canonical `ture-staging.netlify.app` origin and explicit one-read query parameter after authentication. A final temporary route added only session-protected boolean readiness fields and could never return a configuration value. No code reached main.
behavior_check_and_environment: Local Next 16.3.4 production builds completed and focused dashboard suites passed 8/8, then 7/7 for the values-free diagnostic route. The first temporary private staging deploy `6aa379739547a400085d04f4` for `00015d14` had built before its Functions-only flag was added. The second deploy `6aa37cd8f73dc100080fccd8` for `4fbf2f0e` had the non-secret flag in both Build and Functions scopes before it built; its authenticated canonical-query read remained normal. Deploy metadata confirms `context: production` and staging branch. The third deploy `6aa37ff26475310008d45802` for `752c3cd8` used Netlify's documented exact `SITE_ID` Functions identity; its one authenticated canonical-query read also remained normal. The diagnostic route deploy `6aa381b821798d000865e27e` was ready, but the current browser client returned `ERR_BLOCKED_BY_CLIENT` before it could make the protected `/api` request, so it created no application read and yielded no route-level booleans. None of these are failure-state evidence. Each flag was removed immediately; all temporary code was removed in `a23da67e`, whose rollback deploy `6aa3825e04c20a00082a5a02` is ready.
external_effects_and_existing_authority: Only the private `ture-staging` site and its staging branch were touched. The non-secret flag existed only in that site's production context and was first Functions-only, then separately Build-and-Functions scoped three times; it was removed after each attempt. The temporary diagnostic route was deployed once but not reached, then removed. No production site, production database, provider, broker, secret or synthetic business record was touched.
blocker_or_fallback: The three consumed dashboard reads cannot be retried. Configuration order and staging-site identity are now ruled out as sufficient mechanisms; the values-free route diagnosis is blocked by the current browser client's pre-application `/api` navigation policy. Before any later read, use a supported authenticated client that can reach the existing protected staging route, then obtain separately bounded scope for the exact mechanism. Do not keep a probe deployed or widen scopes silently.
result_and_remaining_gap: The staging environment is clean and the probe is absent, but MVP-01c remains unverified because the controlled failed state was not actually observed. MVP-01 remains unaccepted at release scope.
```

### Next — ordered, next product slice

1. Use a supported authenticated client to perform the existing values-free,
   staging-only route-level presence check before selecting any new reversible
   trigger. A fourth failed-state read requires a separately bounded
   staging-only scope for that exact diagnosed mechanism; do not reuse the
   three consumed reads, weaken the private
...
| MVP-01c failed-state proof | Unverified, not a product-runtime defect: the correctly ordered `SITE_ID`-bound deploy `752c3cd8` still did not activate its exact-origin probe; the values-free diagnostic route then built but the current browser client blocked its protected `/api` request before the app. Flag and all temporary code were removed in `a23da67e` | Use a supported authenticated client for the existing values-free route diagnosis; only then select a newly authorized reversible staging trigger with one fresh read, preserving the private visitor gate and exact-origin session boundary. |
