...
| B-03 staging readiness | PR #314 verifies only an opaque-reference admission candidate. Action 666IY's one staging-only catalog audit then verified that the existing private writer package is present, RLS-contained and direct table access is denied to the three reviewed application roles, but executable routine access remains `service_role`-only | Keep B-03 `not_admitted`: a separate decision must establish a dedicated least-privileged application writer identity and private transport before any connection, writer invocation, runtime binding or production inference |
...
| Next decision | [AI-02.17's staging actual-evidence operation](./ai-02.17-staging-actual-evidence-decision.md), [AI-02.18](./ai-02.18-preview-route-attestation-closeout.md) and [AI-02.20](./ai-02.20-proxy-route-attestation-closeout.md) are all closed. AI-02.20 resolved the earlier proxy attribution but did not receive a success receipt. CAT-00.12 consumed its one disposable identity and stopped fail-closed. Action 666IY independently verified a current B-03 catalog/permission boundary without reading rows or changing staging. | Do not retry consumed AI/CAT proofs. The concrete B-03 gap is now known: a separate least-privileged application writer identity and private transport admission must be designed and independently reviewed before any connection or writer call. No current receipt authorizes runtime, deployment, broker or production work. |
