...
const actionPath =
  "docs/action-666il-position-version-lineage-v2-committed-result-receipt-omitted-position-id-own-data-rejection-selection.md";
const evidencePath =
...
test("666IL selects only the immutable receipt omitted position-id own-data rejection review", () => {
  const evidence = JSON.parse(source(evidencePath));

  expect(evidence.contract_version).toBe(
    "trade.action666il.position-version-lineage-v2-committed-result-receipt-omitted-position-id-own-data-rejection-selection.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "a13c958c616dce1ed15f1e82305ee0fb434ecf08",
    protected_main_tree: "1b2baa313a9367bfdf97671db99705d3dbeffeec",
    exact_main_ci_run: 33405890410,
    exact_main_ci_conclusion: "success",
    omitted_initial_history_identity_own_data_rejection_review_action:
      "ACTION_666IK",
  });
  expect(evidence.candidates).toEqual({
    v2_receipt_comparator_source_or_omitted_position_id_adaptation:
    ...
    v2_immutable_receipt_equivalence_omitted_position_id_own_data_rejection_review:
      "selected",
  });
  expect(evidence.selected_outcome).toMatchObject({
    ...
    review_fixture:
      "one_frozen_local_ordinary_receipt_with_exactly_four_enumerable_normal_own_data_fields_canonicalCommandDigest_disposition_initialHistoryIdentity_and_positionVersion_while_positionId_is_absent_without_a_replacement_key",
  });
  expect(evidence.selected_outcome.required_observations).toEqual([
    "object_keys_and_reflect_own_keys_expose_exactly_the_four_retained_canonical_data_keys_while_positionId_is_absent",
    "the_frozen_local_ordinary_omitted_position_id_own_data_receipt_is_rejected_in_either_argument_slot",
    "every_omitted_position_id_own_data_rejection_is_a_fresh_dedicated_comparator_error_with_the_stable_public_name_and_message",
  ...
  expect(evidence.decision).toMatchObject({
    type: "select_one_source_only_milestone_b_receipt_equivalence_omitted_position_id_own_data_rejection_review",
...
test("666IL retains the exact-cardinality position-id omission boundary without authorizing adaptation or runtime", () => {
  ...
  expect(source(comparatorPath)).toContain("positionId");
  ...
  expect(source(roadmapPath)).toMatch(/Action 666IL/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IL/);
});

test("666IL is registered once without changing the six-shard verification plan", () => {
