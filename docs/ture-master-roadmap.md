...
| Current bounded workstream | AI-02.3 through AI-02.15 are completed on exact main. AI-02.17's one separately authorized staging source invocation closed fail-closed on 2026-09-08 with only HTTP 404 before any marker, provider, source or outcome evidence. AI-02.18 then made one isolated no-provider preview-route request, which received HTTP 401 before the function could run; project metadata rules out password and team-SSO settings but leaves the upstream layer unidentified. AI-02.19 records `redesign_or_stop`: deploy metadata places the custom function before the Next catch-all and reports Edge functions, while available reads do not expose Edge rules/logs; the local scan alternative lacks its required server-owned owner principal. Each temporary preview, branch-scoped secret and branch was removed. The existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Preserve v1 provenance and default-deny. AI-02.17 and AI-02.18 are consumed: no retry, cohort, migration, active-evidence write, offline evaluation, promotion, runtime, deployment, broker or production work follows without a new decision. AI-02.19 admits no transport: a future path must first obtain read-only upstream-layer attribution and a fresh narrow authority record. |
...
| Next decision | [AI-02.17's staging actual-evidence operation](./ai-02.17-staging-actual-evidence-decision.md) is closed fail-closed after its only permitted function call returned 404 before any marker, provider, source or outcome operation. [AI-02.18](./ai-02.18-preview-route-attestation-closeout.md) separately proves only that an upstream preview layer returns 401 before its isolated function; the responsible layer is not yet identified. CAT-00.11 is now exact-main verified, but remains only a local redacted-receipt validator. | Any replacement AI staging operation needs a new product decision, new bounded authority record and fresh preconditions; it cannot retry either consumed proof. A fresh preview-transport decision must first identify the upstream layer and preserve branch-only scope. The remaining CAT external gate separately needs a dedicated least-privileged identity, fresh operator decision and independent evidence. |
...
| Agent Intelligence AI-02.18 | [Preview-route attestation closeout](./ai-02.18-preview-route-attestation-closeout.md): after Ready Full CI run `34253266199` passed the unchanged six-shard suite, protected aggregate and merge-candidate POC, exactly one branch-token-protected POST reached the matching ready deploy-preview from temporary PR #409 | HTTP `401` came from an upstream layer before the isolated function could run. A GET-only project-metadata read rules out password and team-SSO settings but does not identify that layer. The secret and all temporary PR/branch/worktree material were removed; no source, provider, database, outcome, evaluator, runtime, deployment, broker or production authority follows. Do not retry. |
