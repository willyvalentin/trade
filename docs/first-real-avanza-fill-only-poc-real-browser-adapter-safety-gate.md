...
- Final pre-live review confirms this safety gate remains active for review/final/submit/placement, credential/session, sell, Stop Loss, Glidande, cap, mismatch, and uncertainty.
...
- The local simulation confirms this safety gate still fails review/final/submit/placement, credential/session, sell, Stop Loss, Glidande, and unsafe scope requests.
...
- Added final harness decision coverage while preserving this safety gate.
- The harness blocks or fails safety on review/final/submit/placement, credential/session handling, sell, Stop Loss, Glidande, cap breach, wrong side/type, mismatch, or uncertainty.
...
# First Fill-Only POC Real Browser Adapter Safety Gate

## Purpose

Action 1040 adds the safety gate for a future first fill-only POC real browser
adapter. This is not the real browser adapter, not the real run, and not order
execution.

This action does not access Avanza, run a browser, query DOM, fill fields, click
buttons, click `Granska kop`, click `Bekrafta kop`, click `Bekrafta salj`,
submit forms, place orders, handle credentials, handle BankID/2FA, read session
tokens, call providers/routes/scans, call Supabase, invoke audit writer code, or
mutate trade/stats/PnL state.

## Gate Basis

- Manual approval has been captured as `approved_for_first_fill_only_poc`.
- Operator setup evidence is
...
- Final confirmation remains permanently forbidden for this POC path.
- First POC locked scope remains buy-only, Avancerad/Limit, amount-based,
  capped at 1,000 SEK or lower, user-present, manually logged in, manually
  account-verified, manually instrument-verified, stopped before review, no
  review modal, no final confirmation, no order placement, no unattended run,
  and no credential/2FA handling by Ture or the agent.

## Real Browser Adapter Prerequisites

| Prerequisite | Status | Notes |
| --- | --- | --- |
| Approval captured | Pass | `approved_for_first_fill_only_poc` is recorded. |
| Operator setup ready | Pass | `operator_setup_ready_for_manual_run_setup` is recorded. |
| Manual setup simulation passed | Pass | Prior local-only setup simulation passed. |
| Execution dry-run simulation passed | Pass | Action 1039 reached `ready_for_execution_dry_run_setup`. |
| Selector contract exists | Pass | Real Avanza selector mapping contract exists. |
| Guard/cap/final-submit protections exist | Pass | Fill-only guard blocks cap violations and final actions. |
| Real browser adapter not added yet | Warn/expected | This action is the gate only. |
| No real browser run performed | Warn/expected | Browser automation remains unapproved. |
| No real run evidence package yet | Warn/expected | Evidence package belongs to a later approved run path. |

## Future Real Browser Adapter Allowed Scope

A future adapter may only be considered if it remains:

- disabled by default.
- limited to fill-only browser dry-run planning.
- buy-only.
- Avancerad/Limit only.
- amount-based.
- capped at 1,000 SEK or lower.
- stopped before `Granska kop`.
- blocked from opening or proceeding through a review modal.
- blocked from final confirmation.
- blocked from order placement.
- blocked from unattended operation.
- blocked from credential, session-token, cookie, localStorage, or BankID/2FA
  handling.

## Browser Adapter Hard Restrictions

A future adapter must not:

- store credentials.
- handle BankID or 2FA.
- read session tokens, cookies, or localStorage.
- click `Granska kop`.
- click `Bekrafta kop`.
- click `Bekrafta salj`.
- submit any form.
...
- switch account.
- use steppers.
- click `Valj alla pa kontot`.
- support sell, Stop Loss, or Glidande in the first POC.

A future adapter must abort on:
...
- final button targeted.
- account uncertainty.
- instrument uncertainty.
- side uncertainty.
- amount/price/total uncertainty.

## Required Constraints For Future Adapter Skeleton

Action 1041 must:

- be disabled by default.
- expose all capabilities as false unless later separately approved.
- model browser capability separately from execution capability.
- define only interfaces, types, constants, planned steps, and gate metadata.
...
- never click.
- never submit.
- use no env/browser credentials.
- use no Supabase/audit/provider/route/scan code.
- preserve final selector hard-stop behavior.

## Gate Decision

`real_browser_adapter_safety_gate_ready`

This means ready to add a disabled-by-default real browser adapter skeleton. It
does not mean ready to run browser automation.
...
Action 1041 - Add Disabled Real Browser Adapter Skeleton.

Reason: after the safety gate, the next step may add a disabled-by-default real
browser adapter skeleton, still with no real browser launch or Avanza access.
