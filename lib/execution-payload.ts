...
import type { BrokerCostEstimate, BrokerCostModel } from "@/lib/broker-costs";
...
export function createHandoffSessionId(now = new Date(), random = Math.random()) {
  const timestamp = sanitizePayloadIdPart(now.toISOString());
  const suffix = Math.floor(Math.abs(random) * 0xffffff)
    .toString(36)
    .padStart(4, "0")
    .slice(0, 6);

  return `handoff_${timestamp}_${suffix}`;
}
...
export function buildTradeExecutionPayload(
  ...
  if (input.brokerCostEstimate?.warnings.length) {
    safetyWarnings.push(...input.brokerCostEstimate.warnings);
  }
  ...
  const handoffSessionId =
    input.handoffSessionId?.trim() || createHandoffSessionId(new Date(createdAt));
  ...
  const payloadWithoutFingerprint: TradeExecutionPayload = {
    ...
    broker_cost_model_snapshot: input.brokerCostModelSnapshot ?? null,
    broker_cost_estimate: input.brokerCostEstimate ?? null,
