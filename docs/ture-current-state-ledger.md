...
| Agent Intelligence AI-02.18 | [Preview-route attestation](./ai-02.18-preview-route-attestation-decision.md) is a temporary non-merge function-route/token diagnostic selected after AI-02.17's opaque 404. Its one-purpose handler is restricted to the expected `trade-vl` deploy-preview and returns a fixed redacted receipt; it imports no application route and has no database, market-data, provider, scanner, source, outcome, evaluator, runtime or broker capability | The proof begins as Draft and may become Ready only to run the unchanged six-shard Full CI gate; auto-merge stays disabled. At most one branch-preview attestation may then be attempted with one fresh branch-scoped token. Any result stops the step and requires cleanup. It cannot retry AI-02.17 or authorize source/provider/staging work. |
...
- AI-02.18 is the independent, route-only prerequisite. It may attest exactly
  one token-protected temporary non-merge preview function and returns no deploy identity,
  credential value, application data or provider result. It never invokes
  AI-02.17 or a canonical route.
