...
import {
  ...
  persistTwentyFourShardAggregateRebuildV1,
...
const plans = [
  ...
  },
  {
    scenario_id: "rpc_catalog_body_drift",
    run_id: "run-b",
    shard_id: "rpc-catalog-body-drift-b",
  },
  {
  ...
  },
  {
    scenario_id: "incompatible_append_only_function",
    run_id: "run-b",
    shard_id: "incompatible-append-only-function-b",
  },
];
...
function establishScenarioPrecondition(container, scenarioId) {
  if (scenarioId === "rpc_catalog_body_drift") {
    psql(container, [
      "-c",
      `do $fixture$
       begin
         ...
         if not found then
           raise exception 'action_661j5r8 selected RPC not found';
         end if;
       end
       $fixture$;`,
    ]);
  ...
  if (scenarioId === "incompatible_append_only_function") {
    psql(container, [
      "-c",
      `alter function public.action_650_reject_execution_audit_mutation()
         set search_path = public;`,
    ]);
    return;
  }
  throw new Error("action_661j5r8.scenario_not_supported");
}
...
async function runOne(plan, manifest, inspectedImage) {
  ...
  const container = `ture-661j5-r8-${plan.scenario_id}-${plan.run_id}-${suffix}`;
  ...
  try {
    ...
    if (runtimeIdentity.identity_digest !== expectedRuntimeIdentityDigest) {
      throw new Error("action_661j5r8.runtime_identity_mismatch");
    }
    ...
    const fileIdentity = `${plan.run_id}.${plan.shard_id}.${plan.scenario_id}.rpc-append-only-rebuild-v1.json`;
    const result = await runRpcAppendOnlyScenarioRebuildV1({
    ...
    const idempotent = persistRpcAppendOnlyResultFileRebuildV1({
      file: result.file,
      output_path: join(runDirectory, fileIdentity),
    });
...
function preflight() {
  ...
  if (POSTGRES_READINESS_POLICY_DIGEST !== expectedReadinessPolicyDigest) {
    throw new Error("action_661j5r8.readiness_policy_mismatch");
  }
  const stale = docker([
    "ps",
    "-a",
  ...
  if (
    ...
    throw new Error("action_661j5r8.historical_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r8.relation_state_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r8.history_runtime_report_mismatch");
  }
  if (
    fileSha256(
      resolve(
    ...
    throw new Error("action_661j5r8.predecessor_aggregate_mismatch");
  }
  terminalResultEntries.forEach((entry) => {
    if (
      fileSha256(resolve(root, entry.root, entry.path)) !== entry.sha256
    ) {
      throw new Error("action_661j5r8.terminal_evidence_mismatch");
    }
  });
  return {
...
async function main() {
  ...
  const rpcAppendOnlyFiles = runs.map((entry) => entry.file);
  const aggregate = buildTwentyFourShardAggregateRebuildV1({
    column_acl_policy_files: preflightResult.column_acl_policy_files,
    rpc_append_only_files: rpcAppendOnlyFiles,
  ...
  const aggregateRetry = persistTwentyFourShardAggregateRebuildV1({
    aggregate,
    column_acl_policy_files: preflightResult.column_acl_policy_files,
    rpc_append_only_files: rpcAppendOnlyFiles,
    historical_files: preflightResult.historical_files,
  ...
  const report = {
    report_version:
      "action_661j5r8_rpc_append_only_runtime_certification_report_v1",
