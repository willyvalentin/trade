...
const functionPath = "netlify/functions/ai02-proxy-route-attestation.ts";
...
function request(token = "proxy-attestation-token", method = "POST") {
  return new Request(
    "https://deploy-preview-999--trade-vl.netlify.app/api/automation/ai02-proxy-route-attestation",
    { method, headers: { "x-ai02-proxy-route-attestation-token": token } },
  );
}
...
test("AI-02.20 returns one fixed receipt only for its protected expected preview", async () => {
  const response = await loadFunction("proxy-attestation-token")(
    request(),
    previewContext,
  );
...
test("AI-02.20 hides its route on wrong method, token, context, site, or absent secret", async () => {
  const handler = loadFunction("proxy-attestation-token");
...
test("AI-02.20 stays within the existing proxy public automation prefix", () => {
  ...
  expect(implementation).toContain(
    'Netlify.env.get("AI02_PROXY_ROUTE_ATTESTATION_TOKEN")',
  );
  expect(proxy).toContain('pathname.startsWith("/api/automation/")');
