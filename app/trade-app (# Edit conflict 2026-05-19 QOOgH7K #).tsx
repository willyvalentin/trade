...
function recommendationStatusLabel(value: RecommendationStatus) {
  ...
  if (value === "ignored" || value === "watched") {
    return "discarded";
  }
...
export function TradeApp() {
  ...
  async function submitClosePosition(event: FormEvent<HTMLFormElement>) {
    ...
    const entryPriceValue = selectedPosition.entryPriceValue;
    const positionSizeValue = selectedPosition.positionSizeValue;
    const currentStopValue = selectedPosition.stopLossValue;
    ...
    const priceMove =
      selectedPosition.direction === "Short"
        ? entryPriceValue - actualExitPrice
        : actualExitPrice - entryPriceValue;
    const pnl = priceMove * positionSizeValue;
    const pnlPercent =
      entryPriceValue !== 0 ? (priceMove / entryPriceValue) * 100 : 0;
    const riskPerShare =
      currentStopValue === null
        ? null
        : selectedPosition.direction === "Short"
          ? currentStopValue - entryPriceValue
          : entryPriceValue - currentStopValue;
    const rMultiple =
      riskPerShare !== null && riskPerShare > 0
        ? priceMove / riskPerShare
        : null;
            ...
            <HistorySection title="Recommendation History">
              {isLoading ? (
                ...
                <EmptyState
                  ...
                  message="Completed recommendation decisions will appear here."
                />
              ) : (
