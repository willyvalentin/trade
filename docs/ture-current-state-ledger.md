...
| Agent Intelligence AI-02.15 | [Staging nonsecret preflight receipt admission](./ai-02.15-staging-nonsecret-preflight-receipt-admission.md) is completed on exact main as PR #383; PR #386 closed its local implementation queue and recorded the failed CLI transport proof. On 2026-09-07, a later one-time Git deploy-preview proof reached its temporary function artifact, but its single token-protected HTTPS request returned no admissible fixed-redaction receipt; its two preview-only variables, Draft PR and remote branch were removed immediately | Neither attempt admits the preflight or a future one-shot adapter. No credential, owner, deploy or source identifier, source row, staging connection, provider/evaluator invocation, persistence, runtime, deployment, broker or production scope was accessed or admitted. |
...
- AI-02.10 closed the additive local v2 compatibility work on exact main
  ...
  Git deploy-preview proof built a temporary function artifact, but its single
  token-protected HTTPS request returned no admissible
  receipt. The two deploy-preview-only variables, Draft PR and remote branch
  were then removed immediately. This is not credential, owner, deploy,
  source-row or staging-access evidence. Before any further attempt, select a
  separately scoped, read-only diagnostic that can explain the missing receipt
  without reading a credential or source row or opening a staging connection.
  It cannot open a
