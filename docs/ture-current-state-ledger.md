...
| MVP-06 | Private staging can now complete a fresh authenticated recommendation-card readback and exact cleanup; the complete manual journey remains unverified | Unverified: use a supported market-session/provider path or the already selected cache-only test boundary; never invent broker-fill attestation. Applicable release verification and supervised market-session evidence remain after it |
...
| MVP-05a | A supported scan uses licensed data within its declared usage budget | verified | On 2026-09-11, the natural 17:15 UTC private-staging cadence returned HTTP 200, ranked/selected/built/published one accepted recommendation and persisted a `completed`/`healthy` run with zero incomplete candidates. The existing Twelve Data cadence budget was unchanged and Polygon supplied calendar metadata only; no manual trigger, broker or public production action occurred. |
| MVP-05b | Last success, freshness and a missed/failed run are visible with a working recovery path | verified | Private authenticated staging's Engine Insights → Advanced scan diagnostics retained the earlier completed/healthy run as **Last Successful Scan**, marked a later stale run **Latest scan needs review**, and showed **Runs Needing Review**. One ordinary dashboard refresh retained that state without starting a scan. |
| MVP-05c | Recommendation snapshots and outcomes retain attributable identity and truthful completion state | verified | Private staging deploy `6aa44a6d462970000850602f` for `513abe1b` retained one attributable snapshot and exactly one truthful `neither_hit` outcome at each of 15m, 30m and 60m after the natural 18:45 UTC schedule. No manual trigger, provider-budget expansion, broker or public production action occurred. |
