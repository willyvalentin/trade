...
behavior_check_and_environment: The single browser navigation to `https://staging--ture-staging.netlify.app/` stopped at Netlify's private team-protection password page before the application login or dashboard could load. It is therefore not an authenticated dashboard read and supplies no failure-state result. The existing restored staging deployment `6aa2dbb53b7f3c00085b116e` remained `ready` at commit `0b763313`.
...
blocker_or_fallback: The necessary protected staging session was unavailable at the Netlify access gate. Do not retry this flag-only arrangement or broaden the host/runtime boundary. A later proof needs a distinct safe explanation for how an authorized staging session reaches the app without weakening private access; until then, continue with independent MVP criteria.
