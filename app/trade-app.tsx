...
function urgencyPillClassName(urgency: PositionUpdateUrgency["urgency"]) {
  if (urgency === "critical") {
    return "border-rose-300/45 bg-rose-300/15 text-rose-100";
  }
...
function getBaseManagementChecklistItems(action: string): ManagementChecklistItem[] {
  if (action === "HOLD") {
    return [
      { id: "setup-valid", label: "Setup still valid?" },
      { id: "stop-respected", label: "Stop loss still respected?" },
      { id: "no-action", label: "No action needed right now?" },
    ];
  }

  if (action === "MOVE_STOP_TO_BREAKEVEN") {
    return [
      { id: "move-stop", label: "Move stop to breakeven in broker", requiresFreshData: true },
      { id: "confirm-stop", label: "Confirm updated stop level", requiresFreshData: true },
      { id: "continue-monitoring", label: "Continue monitoring" },
    ];
  }

  if (action === "TAKE_PARTIAL_PROFIT") {
    return [
      { id: "take-partial", label: "Take partial profit in broker", requiresFreshData: true },
      { id: "confirm-shares", label: "Confirm remaining shares", requiresFreshData: true },
      { id: "confirm-stop-plan", label: "Confirm updated stop plan" },
    ];
  }

  if (action === "TAKE_PROFIT") {
    return [
      { id: "full-profit", label: "Close position or take full profit in broker", requiresFreshData: true },
      { id: "confirm-exit", label: "Confirm exit price", requiresFreshData: true },
      { id: "close-in-app", label: "Close trade in app" },
    ];
  }

  if (action === "CLOSE_POSITION") {
    return [
      { id: "close-broker", label: "Close position in broker", requiresFreshData: true },
      { id: "confirm-exit", label: "Confirm exit price", requiresFreshData: true },
      { id: "close-in-app", label: "Close trade in app" },
      { id: "exit-notes", label: "Add exit notes" },
    ];
  }

  return [
    { id: "review-plan", label: "Review trade plan" },
    { id: "confirm-price", label: "Confirm latest price" },
    { id: "refresh-if-needed", label: "Refresh live position if needed" },
  ];
}

function getManagementChecklistItems(
  action: string,
  staleStatus: StalePositionStatus,
  eodSafetyStatus: EndOfDaySafetyStatus,
) {
  const baseItems = getBaseManagementChecklistItems(action);
  const eodItems: ManagementChecklistItem[] =
    eodSafetyStatus.status === "review_required" ||
    eodSafetyStatus.status === "overnight_risk"
      ? [
          { id: "eod-confirm-broker-status", label: "Confirm broker position status" },
          {
            id: "eod-manage-broker-position",
            label: "Close or intentionally manage position in broker",
            requiresFreshData: true,
          },
          {
            id: "eod-close-trade-in-app",
            label: "Close trade in app after broker execution",
          },
          { id: "eod-exit-notes", label: "Add exit notes" },
        ]
      : eodSafetyStatus.status === "approaching_close"
        ? [
            {
              id: "eod-prepare-exit-plan",
              label: "Prepare exit plan before close",
            },
            { id: "eod-confirm-stop-target", label: "Confirm stop/target plan" },
            {
              id: "eod-avoid-overnight",
              label: "Avoid holding overnight unintentionally",
            },
          ]
        : [];
  const items = [
    ...eodItems,
    ...baseItems.filter((item) => !eodItems.some((eodItem) => eodItem.id === item.id)),
  ];
  ...
  return [
    ...eodItems,
    {
      id: "refresh-live-position",
      label: "Refresh live position before acting",
    },
    ...baseItems.filter((item) => !eodItems.some((eodItem) => eodItem.id === item.id)),
  ];
}
...
function getChecklistStorageKey(positionId: string, action: string, itemKey: string) {
  return `trade-management-checklist:${positionId}:${action}:${itemKey}`;
}

function readChecklistState(storageKey: string) {
  if (typeof window === "undefined") {
    return {};
  }
...
function writeChecklistState(storageKey: string, state: Record<string, boolean>) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  try {
    ...
    // TODO: Persist trade management checklist events.
  }
}

function logManagementChecklistEvent({
  positionId,
  action,
  itemLabel,
}: {
  ...
  try {
    const event = {
      type: "management_checklist_item_completed",
      position_id: positionId,
      action,
      item_label: itemLabel,
      completed_at: new Date().toISOString(),
    };
    const key = "trade-management-checklist-events";
    const events = JSON.parse(window.localStorage.getItem(key) ?? "[]") as unknown[];
    window.localStorage.setItem(key, JSON.stringify([...events.slice(-99), event]));
  } catch {
    // TODO: Persist trade management checklist events.
  }
}
            ...
            {isLoading ? (
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                {elevatedPreTradeContexts.length > 0 && (
                  <div className="rounded-lg border border-amber-300/25 bg-amber-300/10 p-4 text-sm leading-6 text-amber-100 xl:col-span-2">
        ...
        {activeTab === "Live Day Trades" && (
          <section className="space-y-5">
            ...
            {hasEndOfDayReviewRequired && (
              <div className="rounded-lg border border-rose-300/35 bg-rose-300/10 p-4 text-sm leading-6 text-rose-100">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 font-mono text-xs font-semibold uppercase tracking-[0.12em] text-zinc-200">
              {getSetupTypeLabel(recommendation.setupType)}
            </span>
            <span
              className={`rounded-full border px-2.5 py-1 font-mono text-xs font-bold uppercase tracking-[0.12em] ${
                freshness === "fresh"
            ...
            <span
              className={`rounded-full border px-2.5 py-1 font-mono text-xs font-bold uppercase tracking-[0.12em] ${confidenceClassName}`}
            >
              {recommendation.confidenceScore === null
                ? "Confidence n/a"
            ...
            {decisionStack && (
              <span
                className={`rounded-full border px-2.5 py-1 font-mono text-xs font-bold uppercase tracking-[0.12em] ${recommendationDecisionStackClassName(
                  decisionStack.overall_status,
                )}`}
              >
                {decisionStack.overall_status}
...
function TradeModal({
  ...
  const [payloadReviewed, setPayloadReviewed] = useState(false);
  const [avanzaOrderPrepared, setAvanzaOrderPrepared] = useState(false);
  const [agentPreparedOrderForm, setAgentPreparedOrderForm] = useState(false);
  const [buyingPowerChecked, setBuyingPowerChecked] = useState(false);
  ...
  const brokerHandoffTouched = handoffStatus !== "draft";
      ...
      >
        ...
        <section className="mt-5 rounded-lg border border-white/10 bg-white/[0.025] p-4">
          ...
          <div className="mt-4 space-y-2">
            <ChecklistRow
              label="Review execution payload"
              checked={payloadReviewed}
              onChange={setPayloadReviewed}
            />
            <ChecklistRow
              label="Copy payload or mark ready for agent"
              checked={brokerHandoffTouched}
              readOnly
            />
            <ChecklistRow
              label="Agent prepared order form"
              checked={agentPreparedOrderForm}
              onChange={updateAgentPreparedOrderForm}
              disabled={payloadExpired || agentReadiness.status === "blocked"}
            />
            <ChecklistRow
              label="Fill/prepare Avanza order"
              checked={avanzaOrderPrepared}
              onChange={setAvanzaOrderPrepared}
            />
            <ChecklistRow
              label="Buying power checked"
              checked={buyingPowerChecked}
              onChange={setBuyingPowerChecked}
            />
            <ChecklistRow
...
function ChecklistRow({
  ...
  return (
    <label
      ...
      } ${readOnly || disabled ? "cursor-default opacity-70" : "cursor-pointer"}`}
    >
...
function ActivePositionCard({
  ...
  const checklistItems = getManagementChecklistItems(
    action,
    staleStatus,
    eodSafetyStatus,
  );
  const checklistItemKey = checklistItems.map((item) => item.id).join("|");
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="space-y-2 text-left sm:text-right">
          <div
            className={`inline-flex rounded-full border px-3 py-1 font-mono text-xs font-bold uppercase tracking-[0.12em] ${urgencyPillClassName(
              positionUrgency.urgency,
            )}`}
          >
            {positionUrgency.urgency}
      ...
      <div
        className={`mt-5 rounded-md border p-4 text-sm leading-6 ${eodSafetyPanelClassName(
          eodSafetyStatus,
        )}`}
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2">{eodSafetyStatus.message}</p>
            {showEodManualReview && (
              <div className="mt-3 space-y-1">
                <p className="font-mono text-[11px] font-bold uppercase tracking-[0.14em]">
                  Manual review required
                </p>
                <p>Close in broker first, then close trade in app.</p>
              </div>
            )}
            {eodRiskAcknowledged && showEodManualReview && (
          ...
          <div className="flex flex-col items-start gap-2 sm:items-end">
            <span
              className={`w-fit rounded-full border px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.12em] ${eodSafetyPillClassName(
                eodSafetyStatus,
              )}`}
            >
              {endOfDaySafetyLabel(eodSafetyStatus.status)}
            </span>
            {showEodManualReview && !eodRiskAcknowledged && (
              <button
                type="button"
        ...
        <div className="mt-5 rounded-md border border-white/10 bg-black/25 p-4">
          ...
          <div className="mt-4 space-y-2">
            ...
            {position.invalidation && (
              <TextBlock label="Intraday Invalidation" value={position.invalidation} />
            )}
          </div>
      ...
      <TradeManagementChecklist
        key={`${position.id}:${action}:${eodSafetyStatus.status}:${checklistItemKey}`}
        positionId={position.id}
        action={action}
        items={checklistItems}
        staleStatus={staleStatus}
        urgency={positionUrgency.urgency}
        closeRequiredAction={closeRequiredAction}
        isSaving={isSaving}
        onCloseTrade={() => onClosePosition(position)}
      />
    </article>
  );
}

function TradeManagementChecklist({
  positionId,
  action,
  items,
  staleStatus,
  ...
  positionId: string;
  action: string;
  items: ManagementChecklistItem[];
  staleStatus: StalePositionStatus;
  urgency: PositionUpdateUrgency["urgency"];
  closeRequiredAction: boolean;
  isSaving: boolean;
  onCloseTrade: () => void;
}) {
  const isVeryStale = staleStatus === "very_stale";
  const itemKey = items.map((item) => item.id).join("|");
  const storageKey = getChecklistStorageKey(positionId, action, itemKey);
  const [completedItems, setCompletedItems] = useState<Record<string, boolean>>(
    () => readChecklistState(storageKey),
  );
  const isCompleted =
    items.length > 0 && items.every((item) => completedItems[item.id]);

  function toggleItem(item: ManagementChecklistItem) {
    const nextCompleted = !completedItems[item.id];
    const nextState = {
      ...completedItems,
      [item.id]: nextCompleted,
    };

    setCompletedItems(nextState);
    writeChecklistState(storageKey, nextState);

    if (nextCompleted) {
      logManagementChecklistEvent({
        positionId,
        action,
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-zinc-400">
            Manual execution required. Trade does not place orders for you.
          </p>
        </div>
        <span
          className={`w-fit rounded-full border px-2 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.12em] ${positionActionClassName(
            action,
          )}`}
        >
      ...
      {isVeryStale && (
        <p className="mt-3 rounded-md border border-amber-300/25 bg-amber-300/10 p-3 text-sm leading-6 text-amber-100">
          Data is very stale. Refresh before acting on this recommendation.
        </p>
      )}

      <div className="mt-4 space-y-2">
        {items.map((item) => {
          const isRefreshItem = item.id === "refresh-live-position";
          const disabled = isVeryStale && item.requiresFreshData === true && !isRefreshItem;

          return (
            <label
              key={item.id}
              className={`flex items-start gap-3 rounded-md border p-3 text-sm leading-5 transition ${
                disabled
                  ? "cursor-not-allowed border-white/5 bg-white/[0.02] text-zinc-600"
                  : "cursor-pointer border-white/10 bg-white/[0.025] text-zinc-200 hover:border-white/20"
              }`}
            >
              <input
                type="checkbox"
                checked={completedItems[item.id] === true}
                disabled={disabled}
                onChange={() => toggleItem(item)}
                className="mt-0.5 h-4 w-4 accent-[#00db94]"
              />
              <span
                className={
                  completedItems[item.id] ? "line-through decoration-zinc-500" : ""
                }
              >
                {item.label}
              </span>
      ...
      {isCompleted && (
        <p className="mt-3 font-mono text-xs font-semibold uppercase tracking-[0.12em] text-emerald-200">
      ...
      {closeRequiredAction && (
        <p className="mt-3 text-sm leading-6 text-zinc-500">
          Complete broker action first, then close trade in app with exit price
          and notes.
        </p>
      )}

      {closeRequiredAction && !isCompleted && (
        <p className="mt-4 rounded-md border border-amber-300/25 bg-amber-300/10 p-3 text-sm leading-6 text-amber-100">
          Broker action should be completed before closing this trade in app.
        </p>
      )}

      {closeRequiredAction && isCompleted && (
        <p className="mt-4 rounded-md border border-[#00db94]/25 bg-[#00db94]/10 p-3 text-sm leading-6 text-emerald-100">
          Next: use CLOSE TRADE after broker execution is complete.
        </p>
      )}
      ...
      <button
        ...
        disabled={isSaving}
        className={`mt-5 min-h-11 w-full rounded-md border px-4 py-3 font-mono text-xs font-bold uppercase tracking-[0.14em] transition disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-zinc-900 disabled:text-zinc-600 ${
          closeRequiredAction && isCompleted
            ? "border-rose-200/70 bg-rose-300/20 text-rose-50 hover:border-rose-100"
            : "border-rose-300/35 bg-rose-300/10 text-rose-100 hover:border-rose-200/70"
        }`}
      >
        Close Trade
      </button>
...
function ClosedPositionCard({ position }: { position: ClosedPosition }) {
  ...
  return (
    <article className="bg-surface-subtle rounded-lg border border-white/10 p-5">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <Detail label="Setup Type" value={getSetupTypeLabel(position.setupType)} />
        <Detail label="Entry Price" value={position.entryPrice} />
      ...
      <BrokerExecutionMetadataPanel metadata={position.executionMetadata} compact />
      ...
      <HandoffQualityPanel position={position} closedAt={position.closedAtRaw} />
      <ExecutionImprovementSuggestionsPanel
        position={position}
        closedAt={position.closedAtRaw}
      />
      <TradeOutcomeExplainerPanel position={position} />
      <HandoffSessionReplayPanel position={position} closedAt={position.closedAtRaw} />
      <ExecutionTimelinePanel position={position} closedAt={position.closedAtRaw} />
