...
import {
  ...
  columnAclPolicyPolicyForScenario,
...
import {
  ...
  verifyColumnAclPolicyRunnerIdentityReceiptRebuildV1,
} from "./action-661j5r7-column-acl-policy-runner-authority-rebuild-v1.mjs";

export const COLUMN_ACL_POLICY_ATOMIC_EVIDENCE_VERSION =
  "action_661j5r7_column_acl_policy_atomic_evidence_rebuild_v1";
export const COLUMN_ACL_POLICY_RECORD_VERSION =
  "action_661j5r7_column_acl_policy_record_rebuild_v1";
export const COLUMN_ACL_POLICY_SHARD_VERSION =
  "action_661j5r7_column_acl_policy_shard_rebuild_v1";
export const COLUMN_ACL_POLICY_FILE_VERSION =
  "action_661j5r7_column_acl_policy_persisted_file_rebuild_v1";
...
function validateCapture(capture, scenarioId) {
  ...
  for (const snapshot of [capture.prestate, capture.poststate]) {
    if (
      ...
      canonicalJson(expectedColumnAclPolicyHistoryInventory(scenarioId))
    ) {
    ...
    const targetPolicies = domainValue(snapshot, "rls_policies").filter(
      (entry) =>
        entry.schema === "public" &&
        entry.table === "historical_candles",
    );
    const expectedTargetColumnAcl =
      scenarioId === "column_acl_state" ? [COLUMN_ACL_PRECONDITION] : [];
    const expectedTargetPolicies =
      scenarioId === "policy_state" ? [POLICY_PRECONDITION] : [];
    if (
      unknownTargetAcl.length !== 0 ||
      canonicalJson(targetColumnAcl) !==
        canonicalJson(expectedTargetColumnAcl) ||
      canonicalJson(targetPolicies) !== canonicalJson(expectedTargetPolicies)
    ) {
      fail(REASON_CODES.precondition_reference, "acl_policy_inventory");
    }
  }
...
export function buildColumnAclPolicyAtomicEvidenceRebuildV1({
  capture,
  scenario_id,
}) {
  columnAclPolicySelectionForScenario(scenario_id);
  validateCapture(capture, scenario_id);
  const projection = {
    atomic_evidence_version: COLUMN_ACL_POLICY_ATOMIC_EVIDENCE_VERSION,
    atomicity_decision: "no_transition_verified",
    diagnostic: capture.diagnostic,
    guarded_reads: normalizedReads(capture.guarded_reads),
    migration_applied: false,
    policy_registry: COLUMN_ACL_POLICY_POLICY_REGISTRY,
    policy_registry_digest: COLUMN_ACL_POLICY_POLICY_REGISTRY_DIGEST,
    poststate: capture.poststate,
    precondition_reference:
      buildColumnAclPolicyPreconditionReference(scenario_id),
    prestate: capture.prestate,
    protocol_version: COLUMN_ACL_POLICY_RESULT_PROTOCOL_VERSION,
    runner_identity: buildColumnAclPolicyRunnerIdentityReceiptRebuildV1(),
    runtime_capture_digest: capture.runtime_capture_digest,
    runtime_identity: capture.runtime_identity,
    runtime_registry: COLUMN_ACL_POLICY_RUNTIME_REGISTRY,
    runtime_registry_digest: COLUMN_ACL_POLICY_RUNTIME_REGISTRY_DIGEST,
    scenario_id,
...
export function verifyColumnAclPolicyAtomicEvidenceRebuildV1(evidence) {
  assertClosed(evidence, EVIDENCE_FIELDS);
  if (
    evidence.atomic_evidence_version !== COLUMN_ACL_POLICY_ATOMIC_EVIDENCE_VERSION ||
    evidence.protocol_version !== COLUMN_ACL_POLICY_RESULT_PROTOCOL_VERSION ||
  ...
  if (
    canonicalJson(evidence.policy_registry) !==
      canonicalJson(COLUMN_ACL_POLICY_POLICY_REGISTRY) ||
    evidence.policy_registry_digest !== COLUMN_ACL_POLICY_POLICY_REGISTRY_DIGEST
  ) fail(REASON_CODES.policy);
  if (
    canonicalJson(evidence.runtime_registry) !==
      canonicalJson(COLUMN_ACL_POLICY_RUNTIME_REGISTRY) ||
    evidence.runtime_registry_digest !== COLUMN_ACL_POLICY_RUNTIME_REGISTRY_DIGEST
  ) fail(REASON_CODES.runtime_registry);
  verifyColumnAclPolicyPreconditionReference(
    evidence.precondition_reference,
    evidence.scenario_id,
  );
  verifyColumnAclPolicyRunnerIdentityReceiptRebuildV1(evidence.runner_identity);
  validateCapture({
...
export function buildColumnAclPolicyRecordRebuildV1({
  evidence,
  run_id,
  shard_id,
}) {
  verifyColumnAclPolicyAtomicEvidenceRebuildV1(evidence);
  ...
  const projection = {
    ...
    protocol_version: COLUMN_ACL_POLICY_RESULT_PROTOCOL_VERSION,
    record_version: COLUMN_ACL_POLICY_RECORD_VERSION,
    run_id,
...
export function verifyColumnAclPolicyRecordRebuildV1(record) {
  assertClosed(record, RECORD_FIELDS);
  verifyColumnAclPolicyAtomicEvidenceRebuildV1(record.evidence);
  if (
    record.protocol_version !== COLUMN_ACL_POLICY_RESULT_PROTOCOL_VERSION ||
    record.record_version !== COLUMN_ACL_POLICY_RECORD_VERSION ||
...
export function buildColumnAclPolicyShardRebuildV1(record) {
  verifyColumnAclPolicyRecordRebuildV1(record);
  const projection = {
    ...
    shard_version: COLUMN_ACL_POLICY_SHARD_VERSION,
    terminal_count: 1,
  };
  return deepFreeze({ ...projection, shard_digest: sha256(projection) });
}

export function verifyColumnAclPolicyShardRebuildV1(shard, record) {
  assertClosed(shard, SHARD_FIELDS);
  const expected = buildColumnAclPolicyShardRebuildV1(record);
...
export function buildColumnAclPolicyFileRebuildV1({ record, shard }) {
  verifyColumnAclPolicyShardRebuildV1(shard, record);
  const projection = {
    file_identity: `${record.run_id}.${record.shard_id}.${record.scenario_id}.column-acl-policy-rebuild-v1.json`,
    file_version: COLUMN_ACL_POLICY_FILE_VERSION,
    protocol_version: COLUMN_ACL_POLICY_RESULT_PROTOCOL_VERSION,
    record,
    record_digest: record.record_digest,
    shard,
...
export function verifyColumnAclPolicyFileRebuildV1(file) {
  assertClosed(file, FILE_FIELDS);
  const expected = buildColumnAclPolicyFileRebuildV1({
    record: file.record,
    shard: file.shard,
  });
...
export function serializeColumnAclPolicyFileRebuildV1(file) {
  verifyColumnAclPolicyFileRebuildV1(file);
  return `${canonicalJson(file)}\n`;
}

export function parseColumnAclPolicyFileRebuildV1(bytes) {
  let parsed;
  try {
  ...
  verifyColumnAclPolicyFileRebuildV1(parsed);
  if (serializeColumnAclPolicyFileRebuildV1(parsed) !== bytes) {
    fail(REASON_CODES.persistence_readback, "canonical_bytes");
  }
  return deepFreeze(parsed);
}

export function persistColumnAclPolicyResultFileRebuildV1({
  file,
  output_path,
}) {
  verifyColumnAclPolicyFileRebuildV1(file);
  ...
  const bytes = serializeColumnAclPolicyFileRebuildV1(file);
  ...
  try {
    ...
    parseColumnAclPolicyFileRebuildV1(readFileSync(output_path, "utf8"));
  } catch (error) {
...
export function buildColumnAclPolicyResultChainRebuildV1({
  capture,
  run_id,
  scenario_id,
  shard_id,
}) {
  const evidence = buildColumnAclPolicyAtomicEvidenceRebuildV1({
    capture,
    scenario_id,
  });
  const record = buildColumnAclPolicyRecordRebuildV1({
    evidence,
    run_id,
    shard_id,
  });
  const shard = buildColumnAclPolicyShardRebuildV1(record);
  const file = buildColumnAclPolicyFileRebuildV1({ record, shard });
