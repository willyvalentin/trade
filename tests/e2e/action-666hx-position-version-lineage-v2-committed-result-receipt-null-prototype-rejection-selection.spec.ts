...
const actionPath =
  "docs/action-666hx-position-version-lineage-v2-committed-result-receipt-null-prototype-rejection-selection.md";
const evidencePath =
...
test("666HX selects only the immutable receipt null-prototype rejection review", () => {
  const evidence = JSON.parse(source(evidencePath));

  expect(evidence.contract_version).toBe(
    "trade.action666hx.position-version-lineage-v2-committed-result-receipt-null-prototype-rejection-selection.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "4025b418f2ab83653ea315ee702ee370f762d0d1",
    protected_main_tree: "cfb17f568f6120e31c68306c2fe20bb366fc0adc",
    exact_main_ci_run: 33328661884,
    exact_main_ci_conclusion: "success",
    cross_realm_policy_review_action: "ACTION_666HW",
  });
  expect(evidence.candidates).toEqual({
    v2_receipt_comparator_source_or_null_prototype_adaptation:
    ...
    v2_immutable_receipt_equivalence_null_prototype_rejection_review:
      "selected",
  });
  expect(evidence.selected_outcome).toMatchObject({
    identifier:
      "v2_immutable_receipt_equivalence_null_prototype_rejection_review",
    review_action: "ACTION_666HY",
  ...
  expect(evidence.selected_outcome.required_observations).toEqual([
    "a_frozen_local_null_prototype_receipt_with_exact_canonical_own_data_fields_is_rejected_in_either_argument_slot",
    "every_null_prototype_rejection_is_a_fresh_dedicated_comparator_error_with_the_stable_public_name_and_message",
    "no_rejection_error_aliases_an_input_or_a_cross_invocation_error",
    "an_ordinary_local_realm_valid_receipt_control_remains_admissible",
  ]);
  expect(evidence.decision).toMatchObject({
    type: "select_one_source_only_milestone_b_receipt_equivalence_null_prototype_rejection_review",
...
test("666HX retains the comparator boundary without authorizing adaptation or runtime", () => {
  ...
  expect(source(roadmapPath)).toMatch(/Action 666HX/);
  expect(source(ledgerPath)).toMatch(/ACTION 666HX/);
});

test("666HX is registered once without changing the six-shard verification plan", () => {
