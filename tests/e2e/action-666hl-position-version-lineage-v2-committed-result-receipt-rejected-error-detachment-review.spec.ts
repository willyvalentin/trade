...
const actionPath =
  "docs/action-666hl-position-version-lineage-v2-committed-result-receipt-rejected-error-detachment-review.md";
const evidencePath =
  "docs/evidence/action-666hl-position-version-lineage-v2-committed-result-receipt-rejected-error-detachment-review.json";
...
function captureRejectedError(
  ...
  throw new Error("expected invalid receipt comparison to reject");
}

test("666HL returns detached dedicated errors for repeated rejected receipt comparisons", () => {
  ...
  for (const invalid of invalidReceipts) {
    for (const [left, right] of [
      ...
      for (let invocation = 0; invocation < 3; invocation += 1) {
        const error = captureRejectedError(comparator, left, right);
        expect(error).not.toBe(left);
  ...
  expect(rejectedErrors).toHaveLength(24);
...
test("666HL remains source-only and is registered once without reopening runtime", () => {
  const evidence = JSON.parse(source(evidencePath));
  const documentation = [source(actionPath), source(evidencePath)].join("\n");
  const registration = JSON.parse(source(registrationPath)) as string[];

  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "070563296187519880b1df6971a7e16ff04e0038",
    protected_main_tree: "8ddd40b3a29031d95898b269692e3146818ecd38",
    exact_main_ci_run: 33288400798,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666HK",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_rejected_error_detachment_review",
    comparator_source_changed: false,
    rejected_input_variants: 4,
    calls_per_invalid_argument_order: 3,
    invalid_input_in_both_argument_slots: true,
    every_rejection_is_a_fresh_dedicated_comparator_error: true,
    dedicated_error_public_name: errorName,
    dedicated_error_message: errorMessage,
    error_input_aliasing: false,
  ...
  expect(source(roadmapPath)).toMatch(/Action 666HL/);
  expect(source(ledgerPath)).toMatch(/ACTION 666HL/);
