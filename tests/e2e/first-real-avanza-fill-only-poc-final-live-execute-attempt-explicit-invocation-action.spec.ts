...
import {
  buildFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction,
...
test("build helper keeps the action disabled by default", () => {
  expect(
    buildFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction(),
  ).toMatchObject({
...
test("no-runner path returns ready_for_final_live_execute_attempt_explicit_invocation without execution", () => {
  const result =
    runFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction(
      safeInput(),
    );
...
test("fake no-op runner path returns final_live_execute_attempt_explicit_invocation_plan_created with exact sequence", () => {
  const calls: string[] = [];
  const result =
    runFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction(
...
test("action stops before Granska kop and exposes no final action capability", () => {
  const result =
    runFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction(
      safeInput(),
    );
...
test.describe("blocks before runner calls for missing gates and unsafe inputs", () => {
  const cases: {
    name: string;
    overrides: Partial<FirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationActionInput>;
    reason: string;
    ...
    {
      name: "mismatched confirmation scope",
      overrides: { final_checklist_confirmation_scope_matches: false },
      reason: "final_checklist_confirmation:scope_mismatch",
    },
    {
...
test("runner total parse failure aborts without completing stop-before-review", () => {
  const calls: string[] = [];
...
test("runner total above cap aborts before evidence capture", () => {
  const calls: string[] = [];
  const result =
    runFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction(
...
test("runner visible-state failure aborts immediately", () => {
  const calls: string[] = [];
  const result =
    runFirstRealAvanzaFillOnlyPocFinalLiveExecuteAttemptExplicitInvocationAction(
      safeInput({
...
test("runner with unsupported method key aborts before calls", () => {
  const calls: string[] = [];
  const runner = {
    ...fakeRunner(calls),
    clickReview: () => ({ ok: false }),
...
test("action source imports only the wrapper and has no browser or external wiring", () => {
  ...
  expect(packageJson).not.toContain(
    "final-live-execute-attempt-explicit-invocation-action",
  );
});
