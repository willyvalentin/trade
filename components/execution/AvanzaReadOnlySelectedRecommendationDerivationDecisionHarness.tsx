import {
  ...
  type AvanzaReadOnlySelectedRecommendationDerivationDecisionFixture,
} from "@/lib/avanza-read-only-selected-recommendation-derivation-decision-fixtures";

type AvanzaReadOnlySelectedRecommendationDerivationDecisionHarnessProps = {
  fixtures?: readonly AvanzaReadOnlySelectedRecommendationDerivationDecisionFixture[];
};
...
export function AvanzaReadOnlySelectedRecommendationDerivationDecisionHarness({
  fixtures = avanzaReadOnlySelectedRecommendationDerivationDecisionFixtures,
}: AvanzaReadOnlySelectedRecommendationDerivationDecisionHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Read-only selectedRecommendation derivation decision fixtures
        </h3>
        <p className="mt-1 text-xs leading-5 text-zinc-400">
          Static derivation decisions only. This harness does not fetch, call
          the bridge, read app state, read real selectedRecommendation state,
          call the adapter, derive real preview state, or enable execution.
        </p>
      </div>
            ...
            >
              ...
              <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["canReadInput", formatBoolean(decision.canReadInput)],
                  [
