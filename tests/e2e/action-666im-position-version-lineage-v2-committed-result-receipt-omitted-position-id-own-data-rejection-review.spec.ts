...
const actionPath =
  "docs/action-666im-position-version-lineage-v2-committed-result-receipt-omitted-position-id-own-data-rejection-review.md";
const evidencePath =
  "docs/evidence/action-666im-position-version-lineage-v2-committed-result-receipt-omitted-position-id-own-data-rejection-review.json";
const selectionEvidencePath =
  "docs/evidence/action-666il-position-version-lineage-v2-committed-result-receipt-omitted-position-id-own-data-rejection-selection.json";
...
const omittedCanonicalField = "positionId";
const errorName =
...
function omittedPositionIdOwnDataReceipt() {
...
function captureOmittedPositionIdOwnDataRejection(
  ...
  throw new Error("expected omitted position-id own-data receipt comparison to reject");
}
...
function expectOmittedPositionIdOwnDataReceiptShape(
  receipt: object,
...
test("666IM independently reviews omitted position-id own-data receipt rejection", () => {
  const comparator = loadComparator();
  const values = receiptValues();
  const control = localReceipt();
  const matchingControl = localReceipt();
  const omittedReceipt = omittedPositionIdOwnDataReceipt();
  const errors: Error[] = [];
  const validVerdict =
    comparator.comparePositionVersionLineageV2WriterImmutableCommittedResultReceipts(
      control,
  ...
  expectOmittedPositionIdOwnDataReceiptShape(omittedReceipt, values);
  ...
  for (const [left, right] of [
    ...
    for (let call = 0; call < 3; call += 1) {
      errors.push(captureOmittedPositionIdOwnDataRejection(comparator, left, right));
    }
  }
  ...
  for (const error of errors) {
    expect([control, matchingControl, omittedReceipt, validVerdict]).not.toContain(
      error,
    );
  }
});

test("666IM retains the selected omitted position-id own-data boundary without widening authority", () => {
  ...
  expect(evidence.contract_version).toBe(
    "trade.action666im.position-version-lineage-v2-committed-result-receipt-omitted-position-id-own-data-rejection-review.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "524cbb7bb794938f9d9971b7287b6bd193074438",
    protected_main_tree: "73c495b556666038198773c7fbedacab28aa9aea",
    exact_main_ci_run: 33414213953,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666IL",
    selection_pr_number: 277,
  });
  expect(selectionEvidence.action_id).toBe("ACTION_666IL");
  expect(selectionEvidence.selected_outcome).toMatchObject({
    review_action: "ACTION_666IM",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_omitted_position_id_own_data_rejection_review",
    ...
    omitted_position_id_own_data_receipt_rejected_in_either_argument_slot: true,
  ...
  expect(documentation).toMatch(/omitted position-id/i);
  expect(documentation).toMatch(/positionId/);
  expect(documentation).toMatch(/Reflect\.ownKeys/i);
  expect(documentation).toMatch(/no CI deduplication is\s+authorized/i);
  expect(source(roadmapPath)).toMatch(/Action 666IM/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IM/);
