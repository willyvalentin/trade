...
# First Real Avanza Fill-Only POC Live Invocation Execute Wrapper

## Purpose

This document records Action 1067: adding the live invocation execute wrapper for the first real Avanza fill-only POC.

The wrapper is disabled by default, explicit-trigger-only, and dependency-injected. It can create a fill-only execute plan and can call only an injected fake/no-op runner in tests after all gates pass and an explicit execution-plan flag is set.

This action did not perform a live run. It did not access Avanza, launch or control a browser, query DOM, fill real fields, click `Granska köp`, open a review modal, click `Bekräfta köp/sälj`, submit, or place an order.

## Implementation

- Added `lib/first-real-avanza-fill-only-poc-live-invocation-execute-wrapper.ts`.
- Added `tests/e2e/first-real-avanza-fill-only-poc-live-invocation-execute-wrapper.spec.ts`.
- The production/lib wrapper imports no Playwright/Puppeteer package and instantiates no browser runner.
- The runner boundary is dependency-injected only.
- Tests use fake in-memory runner methods only.
...
- Live invocation execution gate: `live_invocation_execution_gate_ready`.
- Live invocation run attempt gate: `live_invocation_run_attempt_gate_ready`.
- Final operator GO: `final_operator_go`.
- Immediate pre-invocation confirmation: `immediate_pre_invocation_confirmation_ready`.
- Live invocation attempt wrapper: `attempt_plan_created`.
- Final live invocation wrapper: `ready_for_live_fill_only_invocation`.
- Final harness: `ready_for_final_fill_only_run`.
- Final pre-run evidence: `final_pre_run_evidence_ready`.
- Operator present.
- Manual Avanza login confirmed.
- Account verified manually.
- Instrument verified manually.
- Cap at or below 1,000 SEK.
- Amount-based buy-side Avancerad/Limit scope.
- Stop before review.

## Execution Phases

1. `verify_execution_gate`
2. `verify_run_attempt_gate`
3. `verify_final_operator_go`
4. `verify_immediate_pre_invocation_confirmation`
5. `verify_final_invocation_wrapper_ready`
6. `verify_final_harness_ready`
7. `verify_final_pre_run_evidence`
8. `verify_operator_presence`
9. `verify_manual_login_confirmed`
10. `verify_account_confirmed`
11. `verify_instrument_confirmed`
12. `verify_visible_order_form_state`
13. `verify_buy_side`
14. `verify_advanced_limit_order_type`
15. `prepare_amount_field_fill`
16. `prepare_price_field_fill`
17. `read_total_amount`
18. `verify_cap_after_total_parse`
19. `capture_filled_fields_or_plan_evidence`
20. `capture_stop_before_review_evidence`
21. `stop_before_review`

## Runner Boundary

Allowed runner methods:
...
- No runner call unless all gates pass.
- No runner call unless `execution_plan_explicitly_requested` is true.
- If runner is omitted and all gates pass, return `ready_for_live_invocation_execute` with a complete execute plan.
- If a fake runner is injected in tests and all gates pass, return `execute_plan_created` after only allowed runner methods run and the wrapper stops before review.
...
- Focused docs/path/status checks confirmed `first_real_avanza_fill_only_poc_live_invocation_execute_wrapper_added`, `ready_for_live_invocation_execute`, `execute_plan_created`, and the Action 1068 recommendation.
- `./node_modules/.bin/tsc --noEmit` passed.
- Focused Playwright spec `tests/e2e/first-real-avanza-fill-only-poc-live-invocation-execute-wrapper.spec.ts` passed with 7 tests. The first sandboxed Playwright attempt hit the known local port 3010 `EPERM` startup issue, so the focused spec was rerun with local port-binding escalation.
- `npm run lint` passed with the existing Babel deopt note for large `app/trade-app.tsx`.
- Source-only safety scan on `lib/first-real-avanza-fill-only-poc-live-invocation-execute-wrapper.ts` found no Playwright/Puppeteer import, browser launch/control, DOM query, fetch, Supabase, env/service-role, provider/route/scan, or audit-writer invocation code.
- Runner-boundary forbidden-method scan found no `clickReview`, `clickConfirm`, `submit`, `placeOrder`, `readCookies`, `readSessionStorage`, `handleCredentials`, `switchAccount`, or `switchSide` methods in the production/lib wrapper.
- Hard-stop/value scans confirmed `Granska köp`, `Bekräfta köp/sälj`, GameStop, Valentin Labs KF, 427,26 SEK, 21,98 USD, 438,05 SEK, the 1,000 SEK cap, mismatch/uncertainty, Stop Loss, Glidande, `final_operator_go`, `immediate_pre_invocation_confirmation_ready`, `final_pre_run_evidence_ready`, and `live_invocation_execution_gate_ready` coverage.
...
- No credential/BankID/2FA/session/cookie/localStorage/sessionStorage handling.
...
Reason: the execute wrapper exists and has focused fake-runner coverage; the next action should add a fuller local dry-run simulation proving the wrapper remains bounded before any real invocation attempt.
