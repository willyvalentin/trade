...
export type ScanLogEntry = {
  ...
  top_candidate_indicators?: { isAboveVwap: boolean | null; momentumDirection: string; volumeTrend: string } | null;
...
export function getScanLogResultFromMessage(message: string, recommendationsCreated = 0): ScanLogResult {
  if (recommendationsCreated > 0) return "recommendation_created";
  if (/power hour/i.test(message)) return "power_hour_blocked";
  if (/current recommendation limit/i.test(message)) return "recommendation_limit_reached";
  if (/active position already exists/i.test(message)) return "active_position_exists";
  if (/existing current setup|duplicate/i.test(message)) return "duplicate_ticker_skipped";
  if (/pre-market watchlist updated/i.test(message)) return "pre_market_watchlist_updated";
  if (/pre-market.*no candidates/i.test(message)) return "pre_market_no_candidates";
  if (/pre-market.*holiday|pre-market.*weekend/i.test(message)) return "pre_market_skipped_holiday";
  if (/pre-market|not open for active day trading yet/i.test(message)) return "pre_market";
  ...
  if (/no high-quality|below threshold|no .*setup/i.test(message)) return "no_high_quality_setup";
  if (/skipped/i.test(message)) return "skipped";
  return "unknown";
}

export function buildScanLogMessage(message: string, scanLog: ScanLogEntry) {
  return `${message} scan_window=${scanLog.scan_window ?? "unknown"} ${scanLogPrefix}${JSON.stringify(scanLog)}]`;
}
...
export function parseScanLogFromMessage(row: ScanLogRunRow): ScanLogEntry {
  ...
  if (markerIndex !== -1 && message.endsWith("]")) {
    try {
      const parsed = JSON.parse(message.slice(markerIndex + scanLogPrefix.length, -1)) as ScanLogEntry;
      return { ...parsed, id: row.id, created_at: row.created_at ?? parsed.created_at };
    } catch {
...
export type ScanLogPersistenceInput = {
  scanDate: string;
