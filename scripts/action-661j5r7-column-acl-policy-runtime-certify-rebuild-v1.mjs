...
import {
  ...
  persistTwentyShardAggregateRebuildV1,
...
} from "../lib/action-661j5r7-postgres-runtime-collector-rebuild-v2.mjs";
import {
...
function establishScenarioPrecondition(container, scenarioId) {
  if (scenarioId === "column_acl_state") {
    psql(container, [
      "-c",
      `create role action_661j5_column_acl nologin;
       grant select (ticker) on table public.historical_candles
         to action_661j5_column_acl;`,
    ]);
    return;
  }
  if (scenarioId === "policy_state") {
    psql(container, [
      "-c",
      `create role action_661j5_policy_role nologin;
       create policy action_661j5r7_policy_fixture
         on public.historical_candles
         as permissive
         for select
         to action_661j5_policy_role
         using (true);`,
    ]);
    return;
  }
  throw new Error("action_661j5r7.scenario_not_supported");
}
...
async function runOne(plan, manifest, inspectedImage) {
  ...
  const container = `ture-661j5-r7-${plan.scenario_id}-${plan.run_id}-${suffix}`;
  ...
  try {
    ...
    if (runtimeIdentity.identity_digest !== expectedRuntimeIdentityDigest) {
      throw new Error("action_661j5r7.runtime_identity_mismatch");
    }
    ...
    const fileIdentity = `${plan.run_id}.${plan.shard_id}.${plan.scenario_id}.column-acl-policy-rebuild-v1.json`;
    const result = await runColumnAclPolicyScenarioRebuildV1({
    ...
    const idempotent = persistColumnAclPolicyResultFileRebuildV1({
      file: result.file,
      output_path: join(runDirectory, fileIdentity),
    });
...
function preflight() {
  ...
  if (POSTGRES_READINESS_POLICY_DIGEST !== expectedReadinessPolicyDigest) {
    throw new Error("action_661j5r7.readiness_policy_mismatch");
  }
  const stale = docker([
    "ps",
    "-a",
  ...
  if (
    ...
    throw new Error("action_661j5r7.historical_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r7.relation_state_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r7.history_runtime_report_mismatch");
  }
  if (
    fileSha256(
      resolve(
    ...
    throw new Error("action_661j5r7.predecessor_aggregate_mismatch");
  }
  terminalResultEntries.forEach((entry) => {
    if (
      fileSha256(resolve(root, entry.root, entry.path)) !== entry.sha256
    ) {
      throw new Error("action_661j5r7.terminal_evidence_mismatch");
    }
  });
  return {
...
async function main() {
  ...
  const columnAclPolicyFiles = runs.map((entry) => entry.file);
  const aggregate = buildTwentyShardAggregateRebuildV1({
  ...
  const aggregateRetry = persistTwentyShardAggregateRebuildV1({
    aggregate,
    column_acl_policy_files: columnAclPolicyFiles,
    historical_files: preflightResult.historical_files,
    history_files: preflightResult.history_files,
  ...
  const report = {
    report_version:
      "action_661j5r7_column_acl_policy_runtime_certification_report_v1",
