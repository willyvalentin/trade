...
async function parseBody(
  request: Request,
): Promise<CorrectedPayloadRefetchRouteBody> {
  try {
    const text = await request.text();
    if (!text.trim()) return {};
    const parsed = JSON.parse(text);
    return parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as CorrectedPayloadRefetchRouteBody)
      : {};
  } catch {
    return {};
  }
...
function hasRejectedOverride(body: CorrectedPayloadRefetchRouteBody) {
  return [
    body.ticker,
    body.provider,
    body.endpoint,
...
export async function POST(request: Request) {
  ...
  if (body.execute_corrected_payload_refetch !== true) {
    return jsonNoStore(
      {
        error: "execute_corrected_payload_refetch_true_required",
        route_build_marker:
          firstTinyCorrectedCandlePayloadRefetchExecuteMarker,
        ...noEffectResponse,
      },
