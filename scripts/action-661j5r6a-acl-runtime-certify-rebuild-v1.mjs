...
import {
  ...
  persistSixteenShardAggregateRebuildV1,
...
function establishScenarioPrecondition(container, scenarioId) {
  if (scenarioId === "duplicate_containment_history") {
    psql(container, [
      ...
      `insert into supabase_migrations.schema_migrations(
         ...
         '20260726000000',
         array['action_661j5r6_duplicate_fixture'],
         'action_661j5r6_duplicate_fixture'
       );`,
    ]);
    return;
  }
  if (scenarioId === "unknown_acl_state") {
    psql(container, [
      "-c",
      `create role action_661j5_unknown_acl nologin;
       grant select on table public.historical_candles
         to action_661j5_unknown_acl;`,
    ]);
    return;
  }
  throw new Error("action_661j5r6a.scenario_not_supported");
}
...
async function runOne(plan, manifest, inspectedImage) {
  ...
  const container = `ture-661j5-r6a-${plan.scenario_id}-${plan.run_id}-${suffix}`;
  ...
  try {
    ...
    if (runtimeIdentity.identity_digest !== expectedRuntimeIdentityDigest) {
      throw new Error("action_661j5r6a.runtime_identity_mismatch");
    }
    ...
    const fileIdentity = `${plan.run_id}.${plan.shard_id}.${plan.scenario_id}.terminal-boundary-rebuild-v1.json`;
    const result = await runTerminalScenarioRebuildV1({
    ...
    verifyTerminalFileRebuildV1(result.file);
    const idempotent = persistTerminalResultFileRebuildV1({
      file: result.file,
      output_path: join(runDirectory, fileIdentity),
    });
...
function preflight() {
  ...
  if (POSTGRES_READINESS_POLICY_DIGEST !== expectedReadinessPolicyDigest) {
    throw new Error("action_661j5r6a.readiness_policy_mismatch");
  }
  const stale = docker([
    "ps",
    "-a",
  ...
  if (
    ...
    throw new Error("action_661j5r6a.historical_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r6a.relation_state_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r6a.history_runtime_report_mismatch");
  }
  ...
  duplicateResultPaths.forEach((path, index) => {
    if (fileSha256(join(partialR6Root, path)) !== duplicateFileHashes[index]) {
      throw new Error("action_661j5r6a.duplicate_evidence_mismatch");
    }
  });
  return {
    ...
    duplicate_files: duplicateResultPaths.map((path) =>
      parseTerminalFileRebuildV1(
        readFileSync(join(partialR6Root, path), "utf8"),
      ),
    ),
    image: inspectImage(),
...
async function main() {
  ...
  const terminalFiles = [
    ...preflightResult.duplicate_files,
    ...runs.map((entry) => entry.file),
  ];
  const aggregate = buildSixteenShardAggregateRebuildV1({
  ...
  const aggregateRetry = persistSixteenShardAggregateRebuildV1({
    aggregate,
    historical_files: preflightResult.historical_files,
    history_files: preflightResult.history_files,
    terminal_files: terminalFiles,
  ...
  const report = {
    report_version: "action_661j5r6a_acl_runtime_certification_report_v1",
