  ...
  return {
    ...
    recommendation_id:
      "recommendation_id" in overrides
        ...
        : `rec-${ticker}`,
    scan_run_id: overrides.scan_run_id ?? "rec_scan_run_confidence",
    ...
    confidence: "confidence" in overrides ? (overrides.confidence ?? null) : 72,
  ...
  return {
    ...
    scanner_output_qa: {
      ...
      metadata_coverage: {
        ...
        qa_checked_source_path: "confidence_calibration_test",
    ...
    batch_memory: {
      ...
      latest_batch: {
        batch_fingerprint: "rec_batch_confidence",
        scan_run_fingerprint: "rec_scan_run_confidence",
      },
...
function calibrationRows(
  lowCount: number,
  highCount: number,
  highOutperforms: boolean,
) {
  return [
    ...Array.from({ length: lowCount }, (_, index) => ({
      snapshot_identity: `low-${index}`,
      confidence: 62,
      visibility: "visible",
      entry_triggered: true,
      target_hit: false,
      stop_hit: !highOutperforms,
      best_r: highOutperforms ? 0.1 : 0.8,
      worst_r: highOutperforms ? -0.4 : -0.1,
      terminal_r: 0,
      setup_family: "momentum_breakout",
      sector: "technology",
      ticker: "AAPL",
      regime: "risk_on",
      quality_label: highOutperforms ? "fair" : "good",
    })),
    ...Array.from({ length: highCount }, (_, index) => ({
      snapshot_identity: `high-${index}`,
      confidence: 82,
      visibility: index % 2 === 0 ? "visible" : "research_only",
      entry_triggered: true,
      target_hit: highOutperforms,
      stop_hit: false,
      best_r: highOutperforms ? 0.9 : 0.05,
      worst_r: highOutperforms ? -0.05 : -0.7,
      terminal_r: highOutperforms ? 0.5 : -0.2,
      setup_family: "trend_day_pullback",
      sector: "technology",
      ticker: "MSFT",
      regime: "risk_on",
      quality_label: highOutperforms ? "strong" : "weak",
    })),
  ];
}

test("confidence calibration assigns confidence buckets", () => {
  expect(bucketForConfidence(0)).toBe("0_49");
  expect(bucketForConfidence(59)).toBe("50_59");
  expect(bucketForConfidence(60)).toBe("60_69");
  expect(bucketForConfidence(72)).toBe("70_79");
  expect(bucketForConfidence(88)).toBe("80_89");
  expect(bucketForConfidence(99)).toBe("90_100");
  expect(bucketForConfidence(null)).toBe("unknown");
});

test("confidence calibration falls back to unknown confidence safely", () => {
  const summary = buildConfidenceCalibrationSummary([
    {
      snapshot_identity: "missing-confidence",
  ...
  const unknown = summary.buckets.find((item) => item.bucket === "unknown");

  expect(summary.advisory_only).toBe(true);
  expect(summary.unknown_confidence_count).toBe(1);
  expect(summary.metadata_gaps).toContain("missing_confidence");
  expect(unknown?.unknown_visibility_outcome_count).toBe(1);
  expect(unknown?.calibration_label).toBe("too_early");
});

test("confidence calibration records visibility and intelligence mix per bucket", () => {
  const summary = buildConfidenceCalibrationSummary([
    {
      snapshot_identity: "visible",
      confidence: 75,
      visibility: "visible",
      entry_triggered: true,
      target_hit: true,
  ...
  const bucket = summary.buckets.find((item) => item.bucket === "70_79");

  expect(bucket?.visible_outcome_count).toBe(1);
  expect(bucket?.research_only_outcome_count).toBe(1);
  expect(bucket?.setup_family_mix.momentum_breakout).toBe(1);
  expect(bucket?.sector_mix.technology).toBe(2);
  expect(bucket?.ticker_mix.AAPL).toBe(1);
  expect(bucket?.regime_mix.risk_on).toBe(2);
  expect(bucket?.quality_mix.good).toBe(1);
});

test("confidence calibration uses conservative sample confidence labels", () => {
  expect(
    buildConfidenceCalibrationSummary(calibrationRows(1, 0, true)).sample_confidence,
  ).toBe("low");
  expect(
    buildConfidenceCalibrationSummary(calibrationRows(30, 0, true)).sample_confidence,
  ).toBe("medium");
  expect(
    buildConfidenceCalibrationSummary(calibrationRows(100, 0, true)).sample_confidence,
  ).toBe("high");
});

test("confidence calibration is too early and inconclusive on low sample", () => {
  const summary = buildConfidenceCalibrationSummary(calibrationRows(2, 2, true));
  const bucket = summary.buckets.find((item) => item.bucket === "80_89");

  expect(bucket?.calibration_label).toBe("too_early");
  expect(
    summary.monotonicity_check.higher_confidence_outperforms_lower,
  ).toBeNull();
  expect(summary.monotonicity_check.caution_flags).toContain(
    "insufficient_sample_size",
  );
});

test("confidence calibration detects monotonicity and underperformance", () => {
  const aligned = buildConfidenceCalibrationSummary(calibrationRows(30, 30, true));
  const misaligned = buildConfidenceCalibrationSummary(
    calibrationRows(30, 30, false),
  );
  const weakHighBucket = misaligned.buckets.find(
    (item) => item.bucket === "80_89",
  );

  expect(
    aligned.monotonicity_check.higher_confidence_outperforms_lower,
  ).toBe(true);
  expect(
    misaligned.monotonicity_check.higher_confidence_outperforms_lower,
  ).toBe(false);
  expect(weakHighBucket?.calibration_label).toBe("underperforming");
});

test("daily learning review includes confidence calibration and tier fallback", () => {
  ...
  const summary = buildDailyLearningReviewSummary({
    ...
    outcomes: [
      ...
      outcome("NOPE", {
        snapshot_fingerprint: missing.snapshot_fingerprint,
        recommendation_id: missing.recommendation_id,
      }),
    ],
    now: evaluatedAt,
  });
...
test("market diagnostics renders confidence calibration section", () => {
  const aapl = snapshot("AAPL", { confidence: 82 });
  const summary = buildDailyLearningReviewSummary({
    trading_day: tradingDay,
    latest_batch_fingerprint: "rec_batch_confidence",
  ...
  const section = diagnostics.sections.find(
    (item) => item.section_id === "confidence_calibration",
  );
  ...
  expect(dailySection?.lines.join("\n")).toContain("Confidence monotonicity");
});
