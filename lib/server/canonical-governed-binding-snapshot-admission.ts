...
export function validateCanonicalBoundedSnapshotPayload(
  ...
  while (stack.length > 0) {
    ...
    const stringKeys = (ownKeys as string[]).sort((first, second) =>
...
export function createCanonicalExternalImprovementBindingEntry(input: {
  ...
  if (
    ...
    !shaPattern.test(input.observed_binding_digest) ||
    !shaPattern.test(input.expected_binding_digest) ||
    input.observed_binding_digest !== input.expected_binding_digest ||
    !shaPattern.test(input.source_section_digest) ||
    !instant ||
    (input.entry_type === "capture_binding" &&
...
function validateAuthority(input: {
  ...
  if (
    !shaPattern.test(authority.frozen_manifest_digest) ||
    !shaPattern.test(authority.expected_authority_root_digest) ||
    !shaPattern.test(authority.expected_snapshot_digest) ||
    (authority.expected_predecessor_digest !== null &&
      !shaPattern.test(authority.expected_predecessor_digest))
  ) {
...
function validateEntry(
  ...
  if (
    !shaPattern.test(entry.observed_binding_digest) ||
...
function execute(input: {
  ...
  try {
    ...
    const admission = failureResult({
      ...
      observedSnapshotIdentity: null,
      observedSnapshotDigest: digest(
        { snapshot_or_authority_read_failed: true },
        input.counters,
      ),
      authority: null,
      reasonCodes: ["binding_admission_dependency_read_failed"],
      counters: input.counters,
    });
    const lineage = buildLineage({
      requestDigest,
  ...
  if (!authorityValidation.authority) {
    const validation =
      validateCanonicalBoundedSnapshotPayload(rawSnapshot);
    ...
    if (validation.status === "valid") {
      try {
        input.counters.clones += 1;
        observedSnapshot = structuredClone(rawSnapshot);
        observedDigest = digest(observedSnapshot, input.counters);
      } catch {
        observationReasons.push("snapshot_payload_clone_failed");
        observedDigest = forensicObservedDigest(
          null,
          observationReasons,
          input.counters,
        );
    ...
    const admission = failureResult({
      ...
      observedSnapshotIdentity:
        observedSnapshotIdentity(rawSnapshot),
      observedSnapshotDigest: observedDigest,
      authority: null,
      reasonCodes: [
        ...authorityValidation.reasons,
        ...observationReasons,
      ],
      counters: input.counters,
      validationFailure:
...
export function createCanonicalBindingBackedImprovementReplayHarness(
  ...
  const enabled =
    input.enabled ?? DEFAULT_OFF_BINDING_SNAPSHOT_ADMISSION_ENABLED;
  const killSwitch =
  ...
  if (
    ...
    dependencies.snapshot_dependency.source_contract_version !==
  ...
  return {
    ...
    replay: (request: CanonicalBindingBackedReplayRequest) =>
      execute({ request, dependencies, counters }),
    counters,
    ...safety,
  };
}
...
export function verifyCanonicalBindingBackedImprovementReplayResult(input: {
  ...
  if (!harness.replay) {
    return deepFreeze({
      ...
      reason_codes: ["binding_backed_replay_rebuild_unavailable"],
    });
  }
