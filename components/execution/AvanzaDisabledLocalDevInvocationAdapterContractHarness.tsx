...
type AvanzaDisabledLocalDevInvocationAdapterContractHarnessProps = {
  fixtures?: readonly AvanzaDisabledLocalDevInvocationAdapterContractFixture[];
};
...
export function AvanzaDisabledLocalDevInvocationAdapterContractHarness({
  fixtures = avanzaDisabledLocalDevInvocationAdapterContractFixtures,
}: AvanzaDisabledLocalDevInvocationAdapterContractHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Disabled local-dev invocation adapter contract fixtures
        </h3>
        <p className="mt-2 text-xs leading-5 text-zinc-400">
          Static contract metadata only. The adapter contract defines a future
          request shape between the model-only bridge dry-run and local smoke
          runner layers, but canInvokeTargetNow is always false and runtime
          invocation remains locked.
        </p>
      </div>
            ...
            >
              ...
              <dl className="grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["approvalRunbookId", contract.approvalRunbookId],
                  ["checkpointId", contract.checkpointId],
                  ["dryRunId", contract.dryRunId],
                  ["disabledStopReason", contract.disabledStopReason],
                  [
                    "allowedFuturePayloadSummary",
                    contract.allowedFuturePayloadSummary,
                  ],
                  ["forbiddenPayload", contract.forbiddenPayload],
              ...
              <div className="grid gap-2 rounded-md border border-white/10 bg-white/[0.03] p-3">
                ...
                <dl className="grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                  {Object.entries(contract.requestShape ?? {}).map(
                    ([label, value]) => (
                      <div
                        className="rounded-md border border-white/10 bg-black/20 p-2"
                        key={`${fixture.fixtureId}-request-${label}`}
                      >
                        <dt className="font-mono text-[10px] font-bold uppercase text-zinc-500">
                          {label}
                        </dt>
