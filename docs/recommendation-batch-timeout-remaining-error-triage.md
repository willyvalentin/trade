...
operator-provided screenshot no longer shows the
`recommendation_batches?select=*&scan_run_fingerprint=in.(...)` timeout or the
`recommendation_snapshots` HTTP 500. `scheduled_scan_attempts` HTTP 404 remains
visible and is separate from the recommendation batch timeout path.
...
`recommendation_batches` scan-run timeout still active. Action 963 skips
oversized scan-run backfill lists before querying Supabase.
