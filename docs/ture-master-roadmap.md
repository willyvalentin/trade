...
| Current bounded workstream | AI-02.3 through AI-02.15 are completed on exact main. AI-02.17's one separately authorized staging source invocation closed fail-closed on 2026-09-08 with only HTTP 404 before any marker, provider, source or outcome evidence; its temporary preview, branch-scoped secret and branch were removed. AI-02.18 is the separate temporary non-merge preview-route attestation that can distinguish a future function-route/token binding from that old redacted 404, without accessing staging or market data. It begins as Draft and may become Ready solely for unchanged six-shard Full CI; auto-merge remains disabled. The existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Preserve v1 provenance and default-deny. AI-02.17 is consumed: no retry, cohort, migration, active-evidence write, offline evaluation, promotion, runtime, deployment, broker or production work follows without a new decision. AI-02.18 itself may make at most one fixed-receipt preview attestation and cannot make a provider or source request. |
...
| Next decision | [AI-02.17's staging actual-evidence operation](./ai-02.17-staging-actual-evidence-decision.md) is closed fail-closed after its only permitted function call returned 404 before any marker, provider, source or outcome operation. [AI-02.18](./ai-02.18-preview-route-attestation-decision.md) may independently attest only a fresh Draft preview route and its one-time branch token. CAT-00.11 is now exact-main verified, but remains only a local redacted-receipt validator. | Any replacement AI staging operation needs a new product decision, new bounded authority record and fresh preconditions; it cannot retry AI-02.17. AI-02.18 must clean up after one route-only result and cannot progress to a source operation. The remaining CAT external gate separately needs a dedicated least-privileged identity, fresh operator decision and independent evidence. |
