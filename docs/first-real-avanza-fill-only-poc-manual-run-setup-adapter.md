...
- Final pre-live review includes the manual setup adapter chain as ready.
...
- Manual login/account/instrument verification remain required for any future live wrapper.
...
- Added final harness local simulation that reuses the manual setup adapter decision chain as local fixture input.
- Result status: `first_real_avanza_fill_only_poc_final_real_browser_fill_only_run_harness_simulation_added`.
- No credentials/session handling or live browser action was performed.
