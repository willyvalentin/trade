...
function verifiedChain() {
  ...
  return { requestPreview, operatorApproval, executionPlan };
}
...
test("empty input does not throw and remains advisory signal readiness only", () => {
  const readiness = buildFirstTinyHistoricalFetchApprovalSignalReadiness();

  expect(readiness.advisory_only).toBe(true);
  expect(readiness.approval_signal_readiness_only).toBe(true);
  expect(readiness.approval_signal_status).toBe("not_configured");
  expect(readiness.detected_signal.source_type).toBe("none");
  expect(readiness.detected_signal.source_present).toBe(false);
  expect(readiness.detected_signal.signal_active).toBe(false);
});

test("ready prerequisites create reviewable contract and future signal acceptance", () => {
  const { operatorApproval, requestPreview, executionPlan } = verifiedChain();
  const readiness = buildFirstTinyHistoricalFetchApprovalSignalReadiness({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  ...
  expect(readiness.approval_signal_status).toBe("not_configured");
  expect(readiness.readiness.ready_to_review_signal_contract).toBe(true);
  expect(readiness.readiness.ready_to_accept_future_signal).toBe(true);
  expect(readiness.expected_signal_contract.expected_ticker).toBe("COIN");
  expect(readiness.expected_signal_contract.expected_interval).toBe("5min");
});

test("future signal acceptance requires prerequisites", () => {
  const { requestPreview, executionPlan } = verifiedChain();
  const readiness = buildFirstTinyHistoricalFetchApprovalSignalReadiness({
    request_preview: requestPreview,
    execution_plan: executionPlan,
  });
  ...
  expect(readiness.readiness.ready_to_review_signal_contract).toBe(true);
  expect(readiness.readiness.ready_to_accept_future_signal).toBe(false);
  expect(readiness.blockers).toContain("operator_approval_record_not_ready");
});

test("invalid scope flags invalid", () => {
  const { operatorApproval, requestPreview, executionPlan } = verifiedChain();
  const readiness = buildFirstTinyHistoricalFetchApprovalSignalReadiness({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  ...
  expect(readiness.approval_signal_status).toBe("invalid");
  expect(readiness.validation.ticker_scope_valid).toBe(false);
  expect(readiness.validation.signal_shape_valid).toBe(false);
  expect(readiness.detected_signal.scope_matches_preview).toBe(false);
});

test("valid future signal remains inactive and does not enable fetch", () => {
  const { operatorApproval, requestPreview, executionPlan } = verifiedChain();
  const readiness = buildFirstTinyHistoricalFetchApprovalSignalReadiness({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  ...
  expect(readiness.approval_signal_status).toBe("valid_for_future_action");
  expect(readiness.validation.signal_shape_valid).toBe(true);
  expect(readiness.detected_signal.signal_active).toBe(false);
  expect(readiness.safety.signal_active).toBe(false);
  expect(readiness.readiness.ready_to_enable_future_fetch).toBe(false);
  expect(readiness.readiness.ready_to_execute_now).toBe(false);
  expect(readiness.readiness.ready_to_call_provider_now).toBe(false);
  expect(readiness.readiness.ready_to_persist_candles_now).toBe(false);
});
...
test("runtime effects remain disabled", () => {
  ...
  expect(readiness.readiness.ready_to_create_fetch_run_now).toBe(false);
  expect(readiness.readiness.ready_to_create_synthetic_outcomes).toBe(false);
  expect(readiness.readiness.ready_to_run_replay).toBe(false);
  expect(readiness.readiness.ready_to_affect_scanner).toBe(false);
  expect(readiness.safety.provider_fetch_added).toBe(false);
  expect(readiness.safety.historical_fetch_added).toBe(false);
  expect(readiness.safety.provider_call_executed).toBe(false);
...
test("diagnostics section prints expected safety lines", () => {
  ...
  const section = diagnostics.sections.find(
    (item) =>
      item.section_id ===
      "first_tiny_historical_fetch_approval_signal_readiness",
  );
  ...
  expect(section?.lines).toContain("Approval signal readiness only: yes");
  expect(section?.lines).toContain("Approval signal status: not_configured");
  expect(section?.lines).toContain(
    "Supported signal sources: server_env, manual_operator_note",
  );
  expect(section?.lines).toContain("Source type: none");
  expect(section?.lines).toContain("Source present: no");
  expect(section?.lines).toContain("Signal active: no");
  expect(section?.lines).toContain("Expected provider: Twelve Data");
  expect(section?.lines).toContain("Expected ticker: COIN");
  ...
  expect(section?.lines).toContain("Expected persist allowed: no");
  expect(section?.lines).toContain("Expected replay allowed: no");
  expect(section?.lines).toContain("Expected scanner effect allowed: no");
  expect(section?.lines).toContain("Operator approval record ready: yes");
  ...
  expect(section?.lines).toContain("Signal shape valid: no");
  expect(section?.lines).toContain("Scope matches preview: unknown");
  expect(section?.lines).toContain("Ready to review signal contract: yes");
  expect(section?.lines).toContain("Ready to accept future signal: yes");
  expect(section?.lines).toContain("Ready to enable future fetch: no");
  ...
  expect(
    intelligence?.lines.some((line) =>
      line.startsWith(
        "First tiny fetch approval signal: not_configured / accept future signal yes / execute no / provider call no",
      ),
    ),
  ).toBe(true);
});
