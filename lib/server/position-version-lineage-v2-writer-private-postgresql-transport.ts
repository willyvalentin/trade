...
/**
 * Runs the frozen private routine inside a transaction that is always rolled
 * back. This deliberately has no route or runtime binding: it exists only for
 * a separately admitted staging containment proof and can never commit a
 * recommendation transition, position, history row or idempotency receipt.
 */
