...
import {
  ...
  persistTwelveShardAggregateRebuildV1,
...
const plans = [
  ...
  },
  {
    scenario_id: "missing_action_650_history",
    run_id: "run-b",
    shard_id: "missing-action-650-history-b",
  },
  {
  ...
  },
  {
    scenario_id: "incident_history_present",
    run_id: "run-b",
    shard_id: "incident-history-present-b",
  },
];
  ...
  if (scenarioId === "incident_history_present") {
    psql(container, [
      ...
      `insert into supabase_migrations.schema_migrations(
         ...
         '20260724003000',
         array['action_661j5r5_incident_fixture'],
         'action_661j5r5_incident_fixture'
       );`,
    ]);
    return;
  }
  throw new Error("action_661j5r5.scenario_not_supported");
}
...
async function runOne(plan, manifest, inspectedImage) {
  ...
  const container = `ture-661j5-r5-${plan.scenario_id}-${plan.run_id}-${suffix}`;
  ...
  try {
    ...
    if (runtimeIdentity.identity_digest !== expectedRuntimeIdentityDigest) {
      throw new Error("action_661j5r5.runtime_identity_mismatch");
    }
    ...
    const fileIdentity = `${plan.run_id}.${plan.shard_id}.${plan.scenario_id}.history-boundary-rebuild-v1.json`;
    const result = await runHistoryScenarioRebuildV1({
    ...
    verifyHistoryFileRebuildV1(result.file);
    const idempotent = persistHistoryResultFileRebuildV1({
      file: result.file,
      output_path: join(runDirectory, fileIdentity),
    });
...
function preflight() {
  ...
  if (POSTGRES_READINESS_POLICY_DIGEST !== expectedReadinessPolicyDigest) {
    throw new Error("action_661j5r5.readiness_policy_mismatch");
  }
  const stale = docker([
    "ps",
    "-a",
  ...
  if (
    ...
    throw new Error("action_661j5r5.historical_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r5.relation_state_runtime_report_mismatch");
  }
  return {
...
async function main() {
  ...
  const historyFiles = runs.map((entry) => entry.file);
  const aggregate = buildTwelveShardAggregateRebuildV1({
  ...
  const aggregateRetry = persistTwelveShardAggregateRebuildV1({
    aggregate,
    historical_files: preflightResult.historical_files,
    history_files: historyFiles,
    output_path: aggregatePath,
  ...
  const report = {
    report_version: "action_661j5r5_history_boundary_runtime_certification_report_v1",
