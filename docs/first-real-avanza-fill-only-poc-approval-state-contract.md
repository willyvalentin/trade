...
- Final pre-live review confirms approval-state readiness.
...
- Wrong or missing approval remains blocking.
...
- Added final harness local simulation using the approval state contract.
- Result status: `first_real_avanza_fill_only_poc_final_real_browser_fill_only_run_harness_simulation_added`.
- Wrong or missing approval blocks readiness.
...
- The adapter reuses this approval contract and blocks missing or unsafe
  approval snapshots.
...
- Approval contract remains unchanged and continues to gate future setup
  actions.
...
- Approval state remains unchanged; operator setup evidence is now complete
  from screenshot observations.
