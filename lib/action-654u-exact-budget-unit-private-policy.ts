...
const contractVersion = "action_654u_exact_budget_unit_private_policy_v1" as const;
const gateVersion = "action_654o_canonical_gate_v1" as const;
const policyVersion = "action_654u_private_string_budget_policy_v1" as const;
const frameVersion = "action_654u_utf16_observation_frame_v1" as const;
const inputDomain = "action_654u_canonical_readiness_gate_input" as const;
...
// Module-owned production policy. All leaves are primitives, so freezing this
// record is a complete deep freeze. It is never exported or caller-selected.
...
export type Action654uExactBudgetResult = Readonly<{
...
function evaluateProductionBudget(codeUnitCount: number): BudgetEvaluation {
...
function observeLosslessly(
  ...
  return freezeOwned({
    ...
    digest: `action_654u_observed_gate_utf16_${hashAction650sCanonicalValue(frame)}`,
  });
}
...
function completedWork(
  ...
  return freezeOwned({
    ...
    gate_snapshot_constructions: snapshotConstructions,
    ...
    confirmation_consumptions: predecessor?.effects.confirmation_consumptions ?? 0,
    caller_property_reads: 0,
    getter_executions: 0,
    proxy_hooks_executed: 0,
    callback_executions: 0,
...
function observedFailure(
  ...
  reason: Extract<
    FailureReason,
    "gate_json_parse_failed" | "gate_schema_rejected" | "gate_noncanonical_code_units"
  >,
  parserInvocations: 0 | 1,
  canonicalSerializations: 0 | 1,
): Action654uExactBudgetResult {
  const failureIdentity = `action_654u_gate_failure_${hashAction650sCanonicalValue({
    contract_version: contractVersion,
    policy_version: policyVersion,
    reason,
    observed_string_digest: observation.digest,
  })}`;
  const terminalDigest = `action_654u_gate_terminal_${hashAction650sCanonicalValue({
    contract_version: contractVersion,
  ...
  return freezeOwned({
    ...
    observed_string_total_upper_bound_bytes: observation.total_upper_bound_bytes,
    observed_string_digest: observation.digest,
    failure_identity: failureIdentity,
...
/**
 * Sole production runtime export. It owns its frozen policy and accepts no
 * policy, matrix, evaluator, counter, factory, or privileged review hook.
 */
export function runAction654uExactBudgetUnitPrivatePolicy(
  ...
  if (typeof captured !== "string") {
    return inertResult("gate_input_not_primitive_string", 0);
  }
  ...
  const observation = observeLosslessly(captured, codeUnitCount, budget);
  let parsed: unknown;
  try {
  ...
  if (canonical !== captured) {
    return observedFailure(observation, "gate_noncanonical_code_units", 1, 1);
  }
