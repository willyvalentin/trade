...
function isAllowedChangedFile(relativePath) {
  if (relativePath === ".gitignore") return isSafeNetlifyGitignoreDiff();
  if (isBoundSuccessfulAction529Result(relativePath)) return true;
  if (isBoundHistoricalAbortedAction534Result(relativePath)) return true;
  if (relativePath.startsWith("docs/")) return true;
  if (relativePath === "app/api/runtime-health/ping/route.ts") return true;
  if (isBoundAction518EvaluateOutcomesRoute(relativePath)) return true;
  ...
  if (relativePath === "lib/learning-dataset-static-fixtures.ts") return true;
  if (relativePath === "lib/intelligence-context-static-fixtures.ts") return true;
  if (relativePath === "lib/snapshot-to-learning-dataset-mapper.ts") return true;
  if (relativePath === "lib/pure-pattern-discovery.ts") return true;
  if (isBoundAction492PureConfidenceCalibrationFile(relativePath)) return true;
  if (relativePath === "lib/confidence-calibration-advisory-adapter.ts") return true;
  if (relativePath === "lib/confidence-calibration-recommendation-advisory-projection.ts") return true;
  if (relativePath === "lib/confidence-calibration-recommendation-advisory-projection-preview.ts") return true;
  if (relativePath === "lib/confidence-calibration-recommendation-advisory-projection-preview-flag.ts") return true;
  if (relativePath === "components/recommendations/ConfidenceCalibrationProjectionPreview.tsx") return true;
  if (relativePath === "components/recommendations/RecommendationDetailsModal.tsx") return true;
  if (relativePath === "components/recommendations/RecommendationCardContainer.tsx") return true;
  if (
    [
      "scripts/action-309-post-recovery-safety-guard.mjs",
      "scripts/replay-with-signal-package-static-preview.mjs",
