...
export const EIGHT_SHARD_AGGREGATE_VERSION =
  "action_661j5r4_eight_shard_aggregate_rebuild_v1";
...
function normalizedFiles(historicalFiles, relationStateFiles) {
  if (
...
export function buildEightShardAggregateRebuildV1({
  historical_files,
  relation_state_files,
}) {
  const files = normalizedFiles(historical_files, relation_state_files);
  ...
  const projection = {
    aggregate_version: EIGHT_SHARD_AGGREGATE_VERSION,
    decision: "certified",
...
export function verifyEightShardAggregateRebuildV1(
  aggregate,
  inputs,
) {
  assertClosed(aggregate, FIELDS);
  const expected = buildEightShardAggregateRebuildV1(inputs);
...
export function persistEightShardAggregateRebuildV1({
  aggregate,
  historical_files,
  output_path,
  relation_state_files,
  ...
  verifyEightShardAggregateRebuildV1(aggregate, {
    historical_files,
    relation_state_files,
  });
  ...
  try {
    ...
    verifyEightShardAggregateRebuildV1(readback, {
      historical_files,
      relation_state_files,
    });
  } catch (error) {
