...
type FirstTinyFetchRunAuditWriteRouteBody = {
...
async function parseBody(
  request: Request,
): Promise<FirstTinyFetchRunAuditWriteRouteBody> {
  try {
    const text = await request.text();
    if (!text.trim()) return {};
    const parsed = JSON.parse(text);
    return parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as FirstTinyFetchRunAuditWriteRouteBody)
      : {};
  } catch {
    return {};
  }
...
function hasRejectedOverride(body: FirstTinyFetchRunAuditWriteRouteBody) {
  return [
    body.ticker,
    body.provider,
    body.endpoint,
...
export async function POST(request: Request) {
  ...
  if (body.execute_fetch_run_audit_write !== true) {
    return jsonNoStore(
      {
        error: "execute_fetch_run_audit_write_true_required",
        ...noEffectResponse,
      },
      400,
    );
  ...
  const result = await executeFirstTinyFetchRunAuditWrite({
    ...
    supabase_client: getServerSupabaseClient().client,
  });
