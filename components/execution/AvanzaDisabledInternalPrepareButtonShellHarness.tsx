import {
  ...
  type AvanzaDisabledInternalPrepareButtonShellFixture,
} from "@/lib/avanza-disabled-internal-prepare-button-shell-fixtures";

type AvanzaDisabledInternalPrepareButtonShellHarnessProps = {
  fixtures?: readonly AvanzaDisabledInternalPrepareButtonShellFixture[];
};
...
export function AvanzaDisabledInternalPrepareButtonShellHarness({
  fixtures = avanzaDisabledInternalPrepareButtonShellFixtures,
}: AvanzaDisabledInternalPrepareButtonShellHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Disabled internal prepare button shell fixtures
        </h3>
        <p className="mt-2 text-xs leading-5 text-zinc-400">
          Static fixture results only. This harness renders explicit shell model
          output and does not read Trade UI state, call the API route, fetch
          localhost, call a bridge, control a browser, fill forms, click review,
          click confirm, submit orders, handle credentials, or write execution
          records.
        </p>
      </div>
            ...
            >
              ...
              <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["shellEnabled", formatBoolean(result.shellEnabled)],
                  ["canRenderShell", formatBoolean(result.canRenderShell)],
