...
```bash
curl -s "https://trade.valentinlabs.com/api/environment-boundary-audit" | jq '.'
```
...
- `audit.route_versions.first_tiny_fetch_route_expected_marker` is `action_275_environment_boundary_audit`
...
- `ok` is true
- `route_ping` is true
- `route_version` is `action_275_environment_boundary_audit`
- `route_build_marker` is `action_275_environment_boundary_audit`
