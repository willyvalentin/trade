...
active_hour_budget: Within the existing 4–16 active-hour MVP slice; exact active hours not tracked.
behavior_check_and_environment: `tests/e2e/mvp-05-scan-recovery-visibility.spec.ts` passes 7/7 locally, covering a later failed scan, clean no-trade, unobserved empty, provider-warning empty, unknown/stale data and the mixed clean-no-trade/failed aggregate. Scoped ESLint, full TypeScript no-emit and diff checks pass.
...
active_hour_budget: Within the existing 4–16 active-hour MVP slice; exact active hours not tracked.
behavior_check_and_environment: Draft PR #453 adds regressions to `tests/e2e/action-550-outcome-completion-path-root-cause.spec.ts`; targeted Playwright coverage passes 8/8, including false-complete paths with a partial current-price observation and with no observation input at all, plus impossible long/short-plan rejection before a candle can create a terminal result. The adjacent same-day official-batch revisit suite also passes 5/5. Scoped ESLint, TypeScript no-emit and `git diff --check` pass. The check is local and does not establish a supported-environment acceptance result.
