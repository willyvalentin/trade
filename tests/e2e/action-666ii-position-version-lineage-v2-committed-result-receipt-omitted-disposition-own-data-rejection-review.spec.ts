...
const actionPath =
  "docs/action-666ii-position-version-lineage-v2-committed-result-receipt-omitted-disposition-own-data-rejection-review.md";
const evidencePath =
  "docs/evidence/action-666ii-position-version-lineage-v2-committed-result-receipt-omitted-disposition-own-data-rejection-review.json";
...
const omittedCanonicalField = "disposition";
const errorName =
...
function omittedDispositionOwnDataReceipt() {
...
function captureOmittedDispositionOwnDataRejection(
  ...
  throw new Error("expected omitted disposition own-data receipt comparison to reject");
}
...
function expectOmittedDispositionOwnDataReceiptShape(
  receipt: object,
...
test("666II independently reviews omitted disposition own-data receipt rejection", () => {
  const comparator = loadComparator();
  const values = receiptValues();
  const control = localReceipt();
  const matchingControl = localReceipt();
  const omittedReceipt = omittedDispositionOwnDataReceipt();
  const errors: Error[] = [];
  const validVerdict =
    comparator.comparePositionVersionLineageV2WriterImmutableCommittedResultReceipts(
      control,
  ...
  expectOmittedDispositionOwnDataReceiptShape(omittedReceipt, values);
  ...
  for (const [left, right] of [
    ...
    for (let call = 0; call < 3; call += 1) {
      errors.push(
        captureOmittedDispositionOwnDataRejection(comparator, left, right),
      );
    }
  }
...
test("666II retains the selected omitted disposition own-data boundary without widening authority", () => {
  ...
  expect(evidence.contract_version).toBe(
    "trade.action666ii.position-version-lineage-v2-committed-result-receipt-omitted-disposition-own-data-rejection-review.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "68970984e64558c419a5e960c94779fdf659ce6d",
    protected_main_tree: "f90576f494cc8ee7562bc125a405edb33350d6a8",
    exact_main_ci_run: 33383959076,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666IH",
    selection_pr_number: 273,
  });
  expect(selectionEvidence.action_id).toBe("ACTION_666IH");
  expect(selectionEvidence.selected_outcome).toMatchObject({
    review_action: "ACTION_666II",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_omitted_disposition_own_data_rejection_review",
    ...
    omitted_disposition_own_data_receipt_rejected_in_either_argument_slot: true,
  ...
  expect(source(roadmapPath)).toMatch(/Action 666II/);
  expect(source(ledgerPath)).toMatch(/ACTION 666II/);
