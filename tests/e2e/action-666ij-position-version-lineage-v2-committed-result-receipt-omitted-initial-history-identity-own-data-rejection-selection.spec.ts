...
const actionPath =
  "docs/action-666ij-position-version-lineage-v2-committed-result-receipt-omitted-initial-history-identity-own-data-rejection-selection.md";
const evidencePath =
...
test("666IJ selects only the immutable receipt omitted initial-history-identity own-data rejection review", () => {
  const evidence = JSON.parse(source(evidencePath));

  expect(evidence.contract_version).toBe(
    "trade.action666ij.position-version-lineage-v2-committed-result-receipt-omitted-initial-history-identity-own-data-rejection-selection.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "76b61d8682587ea01b34e733b750549aacfbfca7",
    protected_main_tree: "e24a9c26ede922036693daf00f03cd5d8eb611c6",
    exact_main_ci_run: 33390268703,
    exact_main_ci_conclusion: "success",
    omitted_disposition_own_data_rejection_review_action: "ACTION_666II",
  });
  expect(evidence.candidates).toEqual({
    v2_receipt_comparator_source_or_omitted_initial_history_identity_adaptation:
    ...
    v2_immutable_receipt_equivalence_omitted_initial_history_identity_own_data_rejection_review:
      "selected",
  });
  expect(evidence.selected_outcome).toMatchObject({
    ...
    review_fixture:
      "one_frozen_local_ordinary_receipt_with_exactly_four_enumerable_normal_own_data_fields_canonicalCommandDigest_disposition_positionId_and_positionVersion_while_initialHistoryIdentity_is_absent_without_a_replacement_key",
  });
  expect(evidence.selected_outcome.required_observations).toEqual([
    "object_keys_and_reflect_own_keys_expose_exactly_the_four_retained_canonical_data_keys_while_initialHistoryIdentity_is_absent",
    "the_frozen_local_ordinary_omitted_initial_history_identity_own_data_receipt_is_rejected_in_either_argument_slot",
    "every_omitted_initial_history_identity_own_data_rejection_is_a_fresh_dedicated_comparator_error_with_the_stable_public_name_and_message",
  ...
  expect(evidence.decision).toMatchObject({
    type: "select_one_source_only_milestone_b_receipt_equivalence_omitted_initial_history_identity_own_data_rejection_review",
...
test("666IJ retains the exact-cardinality initial-history-identity omission boundary without authorizing adaptation or runtime", () => {
  ...
  expect(source(comparatorPath)).toContain("initialHistoryIdentity");
  ...
  expect(source(roadmapPath)).toMatch(/Action 666IJ/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IJ/);
});

test("666IJ is registered once without changing the six-shard verification plan", () => {
