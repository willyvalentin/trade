...
begin
  select array_agg(oid order by oid) into permitted_role_oids from pg_roles where rolname in ('anon', 'authenticated', 'service_role');
  if cardinality(permitted_role_oids) <> 3 then raise exception 'Action 659B rejects missing runtime roles'; end if;
  if not exists (select 1 from supabase_migrations.schema_migrations where version = '20260724002000' and name = 'contain_production_trading_data_access' and cardinality(statements) = 6) then
    raise exception 'Action 659B requires exact Action 650 migration history';
  end if;
  if exists (select 1 from supabase_migrations.schema_migrations where version = '20260724003000') then
  ...
  foreach target_table in array target_tables loop
    ...
    if exists (select 1 from pg_class classes cross join lateral aclexplode(coalesce(classes.relacl, acldefault('r', classes.relowner))) acl where classes.oid = format('public.%I', target_table)::regclass and acl.grantee <> 0 and acl.grantee <> classes.relowner and not (acl.grantee = any(permitted_role_oids)))
      or exists (select 1 from pg_attribute attributes cross join lateral aclexplode(attributes.attacl) acl where attributes.attrelid = format('public.%I', target_table)::regclass and attributes.attnum > 0 and not attributes.attisdropped and acl.grantee <> 0 and acl.grantee <> (select relowner from pg_class where oid = attributes.attrelid) and not (acl.grantee = any(permitted_role_oids))) then
      raise exception 'Action 659B rejects unknown ACL grantee on public.%', target_table;
    end if;
    if exists (select 1 from pg_attribute attributes join pg_class classes on classes.oid = attributes.attrelid cross join lateral aclexplode(attributes.attacl) acl where attributes.attrelid = format('public.%I', target_table)::regclass and attributes.attnum > 0 and not attributes.attisdropped and acl.grantee <> classes.relowner) then
    ...
    if not (select relrowsecurity from pg_class where oid = format('public.%I', target_table)::regclass) then repair_required := true; end if;
    foreach privilege_name in array required_privileges loop
      if has_table_privilege('anon', format('public.%I', target_table), privilege_name) or has_table_privilege('authenticated', format('public.%I', target_table), privilege_name) or not has_table_privilege('service_role', format('public.%I', target_table), privilege_name) then repair_required := true; end if;
    end loop;
    foreach privilege_name in array denied_privileges loop
      if has_table_privilege('anon', format('public.%I', target_table), privilege_name) or has_table_privilege('authenticated', format('public.%I', target_table), privilege_name) or has_table_privilege('service_role', format('public.%I', target_table), privilege_name) then repair_required := true; end if;
    end loop;
  ...
  if exists (select 1 from unnest(audit_tables) audit_tables(table_name) left join pg_class classes on classes.oid = format('public.%I', audit_tables.table_name)::regclass left join pg_trigger triggers on triggers.tgrelid = classes.oid and not triggers.tgisinternal group by audit_tables.table_name having count(triggers.oid) <> 1 or bool_or(triggers.tgname <> 'action_650_append_only' or triggers.tgfoid <> to_regprocedure('public.action_650_reject_execution_audit_mutation()') or triggers.tgenabled <> 'O' or triggers.tgtype <> 27)) then
    raise exception 'Action 659B rejects unknown Action 650 trigger state';
  end if;
  if (select count(*) from pg_proc where oid in (to_regprocedure('public.app_open_position_transaction(uuid,text,text,numeric,numeric,numeric,numeric,numeric,jsonb,text)'), to_regprocedure('public.app_login_abuse_reserve(text)'), to_regprocedure('public.app_login_abuse_finalize_success(text)')) and prosecdef and pg_get_userbyid(proowner) = 'postgres' and coalesce(array_to_string(proconfig, ','), '') like '%search_path=pg_catalog, public%') <> 3 then
    raise exception 'Action 659B rejects Action 652 RPC drift';
  end if;
  if not repair_required then raise exception 'Action 659B recovery is not required for the contained contract'; end if;
  ...
  foreach target_table in array target_tables loop
    if not (select relrowsecurity from pg_class where oid = format('public.%I', target_table)::regclass) or exists (select 1 from pg_policies where schemaname = 'public' and tablename = target_table) then raise exception 'Action 659B postcondition failed for public.%', target_table; end if;
    if exists (select 1 from pg_class classes cross join lateral aclexplode(coalesce(classes.relacl, acldefault('r', classes.relowner))) acl where classes.oid = format('public.%I', target_table)::regclass and acl.grantee = 0) then raise exception 'Action 659B PUBLIC privilege postcondition failed for public.%', target_table; end if;
    foreach privilege_name in array required_privileges loop
      if has_table_privilege('anon', format('public.%I', target_table), privilege_name) or has_table_privilege('authenticated', format('public.%I', target_table), privilege_name) or not has_table_privilege('service_role', format('public.%I', target_table), privilege_name) then raise exception 'Action 659B required privilege postcondition failed for public.%', target_table; end if;
    end loop;
    foreach privilege_name in array denied_privileges loop
      if has_table_privilege('anon', format('public.%I', target_table), privilege_name) or has_table_privilege('authenticated', format('public.%I', target_table), privilege_name) or has_table_privilege('service_role', format('public.%I', target_table), privilege_name) then raise exception 'Action 659B denied privilege postcondition failed for public.%', target_table; end if;
    end loop;
  end loop;

  insert into supabase_migrations.schema_migrations(version, statements, name)
  values ('20260724003000', array['repair_contained_trading_data_access_acl_rls'], 'repair_contained_trading_data_access_acl_rls');
end
$action_659b_bundle$;
