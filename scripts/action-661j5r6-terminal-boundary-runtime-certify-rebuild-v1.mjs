...
const plans = [
  ...
  },
  {
    scenario_id: "duplicate_containment_history",
    run_id: "run-b",
    shard_id: "duplicate-containment-history-b",
  },
  {
...
function establishScenarioPrecondition(container, scenarioId) {
  ...
  throw new Error("action_661j5r6.scenario_not_supported");
}
...
async function runOne(plan, manifest, inspectedImage) {
  ...
  const container = `ture-661j5-r6-${plan.scenario_id}-${plan.run_id}-${suffix}`;
  ...
  try {
    ...
    if (runtimeIdentity.identity_digest !== expectedRuntimeIdentityDigest) {
      throw new Error("action_661j5r6.runtime_identity_mismatch");
    }
...
function preflight() {
  ...
  if (POSTGRES_READINESS_POLICY_DIGEST !== expectedReadinessPolicyDigest) {
    throw new Error("action_661j5r6.readiness_policy_mismatch");
  }
  const stale = docker([
    "ps",
    "-a",
  ...
  if (
    ...
    throw new Error("action_661j5r6.historical_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r6.relation_state_runtime_report_mismatch");
  }
  if (
    ...
    throw new Error("action_661j5r6.history_runtime_report_mismatch");
  }
  return {
...
async function main() {
  ...
  const terminalFiles = runs.map((entry) => entry.file);
  ...
  const report = {
    report_version: "action_661j5r6_terminal_boundary_runtime_certification_report_v1",
