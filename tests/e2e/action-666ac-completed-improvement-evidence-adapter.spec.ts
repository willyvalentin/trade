...
test.describe("Action 666AC completed improvement evidence adapter", () => {
  ...
  test("previous binding collision is read-only and explicit", () => {
    ...
    if (!harness.replay) throw new Error("collision_harness_not_ready");
    const result = harness.replay({
      bundle: action666acMappedBundle,
      expected_bundle_digest: action666acMappedBundleDigest,
    }).adapter_result;
    expect(result).toMatchObject({
      status: "conflicting",
      mapping: null,
    ...
    expect(harness.counters.previous_binding_lookups).toBe(2);
    expect(harness.counters.proposal_builds).toBe(1);
  });

  test("duplicate experiment identity fails before proposal build", () => {
    const counters = zeroCounters();
    const bundle = action666acDuplicateExperimentIdentityBundle();
    const harness = createCanonicalImprovementProposalReplayHarness({
      enabled: true,
      kill_switch_engaged: false,
      previous_binding_lookup: action666acEmptyPreviousBindingLookup,
    ...
    if (!harness.replay) throw new Error("duplicate_harness_not_ready");
    expect(harness.replay({
      bundle,
      expected_bundle_digest:
        canonicalCompletedImprovementEvidenceBundleDigest(bundle),
    }).adapter_result).toMatchObject({
      status: "conflicting",
      mapping: null,
      reason_codes: expect.arrayContaining([
    ...
    expect(counters).toEqual(zeroCounters());
    expect(harness.counters.proposal_builds).toBe(0);
    expect(harness.counters.previous_binding_lookups).toBe(0);
  });
  ...
  test("runtime gate requires literal booleans and never reads lookup while closed", () => {
    const invalidEnabledValues: unknown[] = [
      undefined,
      null,
      0,
      1,
    ...
    const invalidKillSwitchValues: unknown[] = [
      undefined,
      null,
      0,
      1,
    ...
    const invalidModes = [
      ...invalidEnabledValues.map((enabled) => ({
        enabled,
        kill_switch_engaged: false,
      })),
      ...invalidKillSwitchValues.map((kill_switch_engaged) => ({
        enabled: true,
        kill_switch_engaged,
      })),
    ];
    for (const mode of invalidModes) {
      ...
      Object.defineProperty(options, "previous_binding_lookup", {
        ...
        get() {
          lookupReads += 1;
          throw new Error("closed_gate_lookup_read");
        },
      });
      const harness = createCanonicalImprovementProposalReplayHarness(
    ...
    const proxy = new Proxy(
      { enabled: true, kill_switch_engaged: false },
      {
        getOwnPropertyDescriptor() {
          throw new Error("gate_proxy_trap");
        },
      },
    );
    expect(
    ...
    ).toMatchObject({ enabled: false, replay: null });
  });

  test("lookup boundary is descriptor-safe, construction-snapshotted, and privately counted", () => {
    const callerCounters = deepFreeze(zeroCounters());
    const mutableLookup: {
      lookup_proposal_binding: () => { semantic_digest: string } | null;
      lookup_experiment_binding: () => { semantic_digest: string } | null;
    } = {
    ...
    const harness = createCanonicalImprovementProposalReplayHarness({
      enabled: true,
      kill_switch_engaged: false,
      previous_binding_lookup: mutableLookup,
      counters: callerCounters,
    });
    if (!harness.replay) throw new Error("snapshot_harness_not_ready");
    mutableLookup.lookup_proposal_binding = () => ({
      semantic_digest: "a".repeat(64),
    });
    mutableLookup.lookup_experiment_binding = () => ({
    ...
    const accessorLookup: Record<string, unknown> = {
      lookup_experiment_binding: () => null,
    };
    ...
    symbolLookup[Symbol("extra")] = true;
    const invalidLookups: unknown[] = [
      accessorLookup,
      symbolLookup,
      {
        lookup_proposal_binding: () => null,
      ...
      {
        lookup_proposal_binding: "not-a-function",
        lookup_experiment_binding: () => null,
      },
      new Proxy(
        {
        ...
        {
          ownKeys() {
            throw new Error("lookup_proxy_trap");
          },
        },
      ),
    ];
    for (const previous_binding_lookup of invalidLookups) {
      expect(() =>
        createCanonicalImprovementProposalReplayHarness({
          enabled: true,
          kill_switch_engaged: false,
          previous_binding_lookup:
            previous_binding_lookup as typeof action666acEmptyPreviousBindingLookup,
        }),
      ).not.toThrow();
      expect(
        createCanonicalImprovementProposalReplayHarness({
  ...
  test("lookup return values fail closed on every noncanonical shape", () => {
    const invalidBindings: unknown[] = [
      undefined,
      {},
      { semantic_digest: "short" },
      { semantic_digest: "a".repeat(64), extra: true },
      Object.defineProperty({}, "semantic_digest", {
        enumerable: true,
        get() {
          throw new Error("binding_accessor_read");
        },
      }),
    ];
    for (const binding of invalidBindings) {
      const result = replay(action666acMappedBundle, {
        lookup_proposal_binding: () => binding as never,
        lookup_experiment_binding: () => binding as never,
      });
      expect(result).toMatchObject({
        status: "unmappable",
  ...
  test("replay request boundary is exact, recursively canonical, and never throws", () => {
    const harness = createCanonicalImprovementProposalReplayHarness({
      enabled: true,
      kill_switch_engaged: false,
      previous_binding_lookup: action666acEmptyPreviousBindingLookup,
    });
    if (!harness.replay) throw new Error("request_shape_harness_not_ready");
    const sparseArray = new Array<unknown>(2);
    sparseArray[0] = "present";
    const extraKeyArray = ["present"] as unknown[] & {
      extra?: boolean;
    };
    ...
    Object.defineProperty(accessorRequest, "expected_bundle_digest", {
      enumerable: true,
      get() {
        throw new Error("request_accessor_read");
      },
    });
    const hiddenRequest: Record<string, unknown> = {
      bundle: action666acMappedBundle,
      expected_bundle_digest: action666acMappedBundleDigest,
    };
    ...
    const invalidRequests: unknown[] = [
      ...
      {
        bundle: action666acMappedBundle,
        expected_bundle_digest: 1,
      },
      accessorRequest,
      hiddenRequest,
      symbolRequest,
      { bundle: { sparseArray }, expected_bundle_digest: "a".repeat(64) },
      { bundle: { extraKeyArray }, expected_bundle_digest: "a".repeat(64) },
      { bundle: cyclic, expected_bundle_digest: "a".repeat(64) },
      new Proxy(
        {
          bundle: action666acMappedBundle,
          expected_bundle_digest: action666acMappedBundleDigest,
    ...
    for (const request of invalidRequests) {
      let result: CanonicalImprovementReplayResult | null = null;
      expect(() => {
  ...
  test("replay verification requires the private frozen harness authority", () => {
    ...
    if (!harness.replay) throw new Error("authority_harness_not_ready");
    const request = {
      bundle: action666acMappedBundle,
      expected_bundle_digest: action666acMappedBundleDigest,
    };
    ...
    expect(Reflect.set(harness, "replay", () => result)).toBe(false);
    expect(
      verifyCanonicalImprovementReplayResult({
        request,
        result,
    ...
    expect(
      verifyCanonicalImprovementReplayResult({
        ...
        typeof verifyCanonicalImprovementReplayResult
      >[0]),
    ).toEqual({
      valid: false,
      canonical_result: null,
