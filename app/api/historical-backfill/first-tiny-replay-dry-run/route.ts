...
type FirstTinyReplayDryRunRouteBody = {
...
async function parseBody(
  request: Request,
): Promise<FirstTinyReplayDryRunRouteBody> {
  try {
    const text = await request.text();
    if (!text.trim()) return {};
    const parsed = JSON.parse(text);
    return parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as FirstTinyReplayDryRunRouteBody)
      : {};
  } catch {
    return {};
  }
...
function hasRejectedOverride(body: FirstTinyReplayDryRunRouteBody) {
  return [
    body.ticker,
    body.provider,
    body.interval,
...
export async function POST(request: Request) {
  ...
  if (body.execute_replay_dry_run !== true) {
    return jsonNoStore(
      {
  ...
  const preflight = await executeFirstTinyHistoricalReplayDryRun({
