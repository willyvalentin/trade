...
const evidencePath =
  "docs/evidence/action-666is-position-version-lineage-v2-committed-result-receipt-undefined-disposition-own-data-rejection-review.json";
...
const undefinedDispositionField = "disposition";
...
function undefinedDispositionOwnDataReceipt() {
  ...
  for (const field of receiptFields) {
    Object.defineProperty(receipt, field, {
      value: field === undefinedDispositionField ? undefined : values[field],
      enumerable: true,
      configurable: false,
      writable: false,
    });
...
function captureUndefinedDispositionOwnDataRejection(
  ...
  throw new Error("expected undefined disposition own-data receipt comparison to reject");
}
...
function expectUndefinedDispositionOwnDataReceiptShape(
  receipt: object,
  ...
  expect(Object.keys(receipt)).toEqual(receiptFields);
  expect(Reflect.ownKeys(receipt)).toEqual(receiptFields);
  expect(Object.hasOwn(receipt, undefinedDispositionField)).toBe(true);
  expect(undefinedDispositionField in receipt).toBe(true);
  expect(Object.getOwnPropertyDescriptor(receipt, undefinedDispositionField)).toEqual({
    value: undefined,
    enumerable: true,
    configurable: false,
    writable: false,
...
test("666IS independently reviews undefined disposition own-data receipt rejection", () => {
  const comparator = loadComparator();
  const values = receiptValues();
  const control = localReceipt();
  const matchingControl = localReceipt();
  const undefinedDispositionReceipt = undefinedDispositionOwnDataReceipt();
  const errors: Error[] = [];
  const validVerdict =
    comparator.comparePositionVersionLineageV2WriterImmutableCommittedResultReceipts(
      control,
  ...
  expectUndefinedDispositionOwnDataReceiptShape(undefinedDispositionReceipt, values);
  ...
  for (const [left, right] of [
    [undefinedDispositionReceipt, control],
    [control, undefinedDispositionReceipt],
  ]) {
    for (let call = 0; call < 3; call += 1) {
      errors.push(captureUndefinedDispositionOwnDataRejection(comparator, left, right));
    }
  }
  ...
  for (const error of errors) {
    expect([
      control,
      matchingControl,
      undefinedDispositionReceipt,
      validVerdict,
    ]).not.toContain(error);
  }
});

test("666IS retains the selected undefined disposition own-data boundary without widening authority", () => {
  ...
  expect(evidence.contract_version).toBe(
    "trade.action666is.position-version-lineage-v2-committed-result-receipt-undefined-disposition-own-data-rejection-review.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "dad94d55a68fac1ba9d287e3bd1662cff476bc67",
    protected_main_tree: "24bbd39b3e3405a71fbf787ce8cf3693ad448961",
    exact_main_ci_run: 33460860745,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666IR",
    selection_pr_number: 283,
  });
  expect(selectionEvidence.action_id).toBe("ACTION_666IR");
  expect(selectionEvidence.selected_outcome).toMatchObject({
    review_action: "ACTION_666IS",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_undefined_disposition_own_data_rejection_review",
    ...
    review_fixture_undefined_disposition_field: undefinedDispositionField,
    review_fixture_undefined_disposition_field_is_own_data_field: true,
    review_fixture_undefined_disposition_field_value_is_undefined: true,
    ...
    undefined_disposition_own_data_receipt_rejected_in_either_argument_slot: true,
  ...
  expect(source(roadmapPath)).toMatch(/Action 666IS/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IS/);
