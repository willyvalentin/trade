...
const forbiddenExecuteRoutePath = join(
  process.cwd(),
  "app/api/historical-backfill/first-tiny-replay-with-signal-package-dry-run/route.ts",
);
const docPath = join(
  process.cwd(),
...
test("minimal ping route does not add POST or execute handler", () => {
  ...
  expect(existsSync(forbiddenExecuteRoutePath)).toBe(false);
});

test("proxy only allows the exact minimal ping path without 307K diagnostics", async () => {
  ...
  try {
    const route = await proxy(
      new NextRequest(
        "http://localhost/api/historical-backfill/first-tiny-replay-with-signal-package-ping",
      ),
    );
    ...
    expect(source).toContain(
      "/api/historical-backfill/first-tiny-replay-with-signal-package-ping",
    );
    expect(source).not.toContain("action_307k_proxy_runtime_crash_isolation");
    expect(source).not.toContain("x-ture-proxy-marker");
    expect(source).not.toContain("isApiRouteHandlerPassThrough");
    expect(source).not.toContain("historical-backfill/first-tiny-replay-with-signal-package-dry-run");
  } finally {
