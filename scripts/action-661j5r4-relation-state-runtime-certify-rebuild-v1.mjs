...
import {
  ...
  persistEightShardAggregateRebuildV1,
...
import { runRelationStateScenarioRebuildV1 } from "../lib/action-661j5r4-relation-state-runtime-runner-rebuild-v1.mjs";
import {
...
const plans = [
  {
    ...
    run_id: "run-a",
    shard_id: "non-table-a",
  },
  {
    scenario_id: "non_table",
    run_id: "run-b",
    shard_id: "non-table-b",
  },
  {
    scenario_id: "wrong_owner",
    run_id: "run-a",
    shard_id: "wrong-owner-a",
  },
  {
    scenario_id: "wrong_owner",
    run_id: "run-b",
    shard_id: "wrong-owner-b",
  },
];
...
function establishScenarioPrecondition(container, scenarioId) {
  if (scenarioId === "non_table") {
    psql(container, [
      "-c",
      `drop table public.historical_candles cascade;
       create view public.historical_candles as
       select null::uuid as id where false;`,
    ]);
    return;
  }
  if (scenarioId === "wrong_owner") {
    psql(container, [
      "-c",
      `do $owner$
       begin
         if not exists (
           ...
           where rolname = 'action_661j5_fixture_owner'
         ) then
           create role action_661j5_fixture_owner nologin;
         end if;
       end
       $owner$;
       alter table public.historical_candles
         owner to action_661j5_fixture_owner;`,
    ]);
    return;
  }
  throw new Error("action_661j5r4.scenario_not_supported");
}
...
function sanitizeMigrationFailure(result, scenarioId) {
  ...
  if (
    observedSqlstate !== policy.terminal_sqlstate
  ) {
    throw new Error(
      `action_661j5r4.observed_sqlstate_mismatch:${observedSqlstate}`,
    );
  }
  const projection = {
  ...
  if (
    ...
    throw new Error(
      `action_661j5r4.terminal_policy_mismatch:${observedSqlstate}:${reason}`,
    );
  }
  return sidecar;
}
...
async function runOne(plan, manifest, inspectedImage) {
  ...
  const container = `ture-661j5-r4-${plan.scenario_id}-${plan.run_id}-${suffix}`;
  ...
  try {
    ...
    const fileIdentity = `${plan.run_id}.${plan.shard_id}.${plan.scenario_id}.relation-state-rebuild-v1.json`;
    const result = await runRelationStateScenarioRebuildV1({
    ...
    verifyRelationStateFileRebuildV1(result.file);
    const idempotent = persistRelationStateResultFileRebuildV1({
      file: result.file,
      output_path: join(runDirectory, fileIdentity),
    });
...
function preflight() {
  ...
  if (
    ...
    throw new Error("action_661j5r4.historical_runtime_report_mismatch");
  }
  return {
...
async function main() {
  ...
  const relationStateFiles = runs.map((entry) => entry.file);
  const aggregate = buildEightShardAggregateRebuildV1({
  ...
  const aggregateRetry = persistEightShardAggregateRebuildV1({
    aggregate,
    historical_files: preflightResult.historical_files,
    output_path: aggregatePath,
    relation_state_files: relationStateFiles,
  ...
  const report = {
    report_version: "action_661j5r4_relation_state_runtime_certification_report_v1",
