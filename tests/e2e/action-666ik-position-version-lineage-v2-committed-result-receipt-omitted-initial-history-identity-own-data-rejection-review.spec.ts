...
const actionPath =
  "docs/action-666ik-position-version-lineage-v2-committed-result-receipt-omitted-initial-history-identity-own-data-rejection-review.md";
const evidencePath =
  "docs/evidence/action-666ik-position-version-lineage-v2-committed-result-receipt-omitted-initial-history-identity-own-data-rejection-review.json";
const selectionEvidencePath =
  "docs/evidence/action-666ij-position-version-lineage-v2-committed-result-receipt-omitted-initial-history-identity-own-data-rejection-selection.json";
...
const omittedCanonicalField = "initialHistoryIdentity";
const errorName =
...
function omittedInitialHistoryIdentityOwnDataReceipt() {
...
function captureOmittedInitialHistoryIdentityOwnDataRejection(
  ...
  throw new Error(
    "expected omitted initial-history-identity own-data receipt comparison to reject",
  );
}
...
function expectOmittedInitialHistoryIdentityOwnDataReceiptShape(
  receipt: object,
  ...
  expect(
    Object.getOwnPropertyDescriptor(receipt, omittedCanonicalField),
  ).toBeUndefined();
  expect(omittedCanonicalField in receipt).toBe(false);
...
test("666IK independently reviews omitted initial-history-identity own-data receipt rejection", () => {
  const comparator = loadComparator();
  const values = receiptValues();
  const control = localReceipt();
  const matchingControl = localReceipt();
  const omittedReceipt = omittedInitialHistoryIdentityOwnDataReceipt();
  const errors: Error[] = [];
  const validVerdict =
    comparator.comparePositionVersionLineageV2WriterImmutableCommittedResultReceipts(
      control,
  ...
  expectOmittedInitialHistoryIdentityOwnDataReceiptShape(
    omittedReceipt,
    values,
  );
  ...
  for (const [left, right] of [
    ...
    for (let call = 0; call < 3; call += 1) {
      errors.push(
        captureOmittedInitialHistoryIdentityOwnDataRejection(
          comparator,
          left,
          right,
        ),
...
test("666IK retains the selected omitted initial-history-identity own-data boundary without widening authority", () => {
  ...
  expect(evidence.contract_version).toBe(
    "trade.action666ik.position-version-lineage-v2-committed-result-receipt-omitted-initial-history-identity-own-data-rejection-review.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "7b0a27e769627d85c4a5feed8d7140be2eb65d94",
    protected_main_tree: "9bfa5f7623663b6980446b800ac3a08bbdcd0ce5",
    exact_main_ci_run: 33397694978,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666IJ",
    selection_pr_number: 275,
  });
  expect(selectionEvidence.action_id).toBe("ACTION_666IJ");
  expect(selectionEvidence.selected_outcome).toMatchObject({
    review_action: "ACTION_666IK",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_omitted_initial_history_identity_own_data_rejection_review",
    ...
    review_fixture_has_no_inherited_replacement_key: true,
    ...
    omitted_initial_history_identity_own_data_receipt_rejected_in_either_argument_slot:
      true,
  ...
  expect(documentation).toMatch(/omitted initial-history-identity/i);
  expect(documentation).toMatch(/initialHistoryIdentity/);
  expect(documentation).toMatch(/Reflect\.ownKeys/i);
  expect(documentation).toMatch(/no CI deduplication is\s+authorized/i);
  expect(source(roadmapPath)).toMatch(/Action 666IK/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IK/);
