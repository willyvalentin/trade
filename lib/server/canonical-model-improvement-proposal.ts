...
export function createCanonicalModelImprovementEngine(input: {
  ...
  const enabled = input.enabled ?? DEFAULT_OFF_MODEL_IMPROVEMENT_ENABLED;
  const killSwitch =
  ...
  const build = (
    ...
    return buildFromPost({
      ...
      registryAuthority: input.trust_boundary!.registry_authority,
      expectedFeatureContextRegistryRoot:
        input.trust_boundary!.registry_authority
          .expected_feature_context_registry_root_digest,
      expectedTrainingInputRegistryRoot:
        input.trust_boundary!.registry_authority
          .expected_training_input_registry_root_digest,
      previousBindingLookup: input.previous_binding_lookup!,
      counters,
    });
  };
  return {
