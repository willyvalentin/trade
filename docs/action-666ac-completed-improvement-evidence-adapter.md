...
compared byte-for-byte by `verifyCanonicalImprovementReplayResult`; a caller
cannot change identity, input digests, adapter version, status, reasons, or
projection and then legitimize that result by recomputing internal digests.
