# Scenario C SELL Order-Prep Preflight Checkpoint

Date: 2026-07-07

## 1. Summary

Purpose: verify that Scenario C - SELL order-prep boundary smoke is ready to be considered in a separate future task.

Scope: Scenario C preflight verification only. This checkpoint verifies named roles, required artifacts, safe mocked/review-only live position/exit input boundaries, allowed/forbidden Scenario C actions, abort conditions, evidence policy, gate state, env boundaries, script/API/Trade UI boundaries, and SELL order-prep risks.

Scenario C is not run by this task.

Decision: `scenario_c_sell_order_prep_preflight_ready_with_warnings`

Warnings carried forward:

- Scenario C has not been executed.
...
- A future Scenario C dry-run must still use only safe mocked/review-only exit input and must not use live order intent, production account/order flow, credentials, BankID/MFA, cookies/session, browser storage, final SALJ, order submission, Supabase execution writes, live position mutation, Trade UI execution, API activation, or production readiness.
...
| Artifact | Exists? | Decision / status | Blockers? | Warnings? | Scenario C impact |
| --- | --- | --- | --- | --- | --- |
| `docs/scenario-b-buy-order-prep-boundary-dry-run-result.md` | Yes | `scenario_b_buy_order_prep_boundary_dry_run_passed_with_warnings` | No | Documentation-only dry-run warning | Confirms BUY mock/review boundary baseline |
| `docs/scenario-b-buy-order-prep-preflight-checkpoint.md` | Yes | `scenario_b_buy_order_prep_preflight_ready_with_warnings` | No | Scenario B was not executed by that checkpoint | Confirms prior preflight chain |
...
| `docs/first-gated-local-dev-smoke-test-plan.md` | Yes | `first_gated_local_dev_smoke_test_plan_ready_with_warnings` | No | Warning set carried forward | Defines Scenario C as future-gated |
...
Required artifact result: pass with warnings. All required artifacts are present. Warnings remain because Scenario C is not executed here and future order-prep boundaries require separate explicit authorization.

## 4. Safe Input Contract Preflight

Allowed future Scenario C input:

- Safe mocked/review-only live position/exit contract.
- Side: SELL.
- No live order intent.
...
- Planned exit reason: target / stop / manual-review-only.
- Reference entry, if needed, as mock/review-only.
- Target/stop, if needed, as mock/review-only.
- Order type, if needed, as mock/review-only.
- Position reference.
- Plan reference.
- Explicit no-submit marker.
- Explicit stop-at-review marker.
- Explicit human-final marker.
- Explicit no-live-position-mutation marker.
...
- Final SALJ authority.
- Order submission authority.
- Live position mutation authority.

Safe input contract result: pass.

## 5. Scenario C Allowed Actions

A future Scenario C task may only:

- Verify SELL order-prep boundary policy.
- Verify safe mocked/review-only live position/exit contract.
- Verify no-submit/no-final-click.
...
A future Scenario C task must not:

- Perform real Avanza order-prep.
- Enter account/order area unless separately authorized in the actual Scenario C task.
- Click final SALJ.
- Submit order.
...
- Final SALJ button is visible and next action could confirm/submit.
...
- Any system attempts final SALJ.
...
Future Scenario C evidence may only contain:
...
- Safe mocked/review-only exit contract reference.
...
| Gate | Expected state | Verified state | Evidence | Result | Blocks Scenario C? |
| --- | --- | --- | --- | --- | --- |
...
## 11. SELL Order-Prep Risk Assessment

| Risk | Severity | Mitigation | Abort condition | Evidence rule | Future follow-up |
| --- | --- | --- | --- | --- | --- |
| Accidental final SALJ risk | Critical | Stop at review/final-confirmation boundary; no final-click authority | Final SALJ is visible and next action could confirm/submit | Redacted note only | Keep final action impossible for agent |
...
| Live account exposure risk | High | Safe mocked/review-only exit input only | Live account data appears | No account/customer IDs, balances, holdings, or account numbers | Require separate account-area authorization |
...
| Position/exit mismatch risk | Medium | Mock/review-only position and exit references only | Position or exit data cannot be verified as mock/review-only | Redacted mismatch note | Require explicit fixture review before future dry-run |
...
| Reviewer visibility risk | Medium | Reviewer verifies contract, gates, evidence, and stop boundary | Reviewer cannot verify safe mocked/review-only exit contract or no-submit boundary | Redacted blocker note | Block next task until visibility improves |

Risk assessment result: pass with warnings. Risks are high around broker UI, final-click, live account exposure, live position mutation, and live trade mutation. They remain mitigated only by safe mocked/review-only exit input, stop-at-review/final-confirmation boundaries, and separate future authorization.
...
```bash
rg -n "Scenario C|SELL|SÄLJ|order-prep|order prep|exit|position|Avanza|BankID|MFA|credential|cookie|session|browser|dry-run|smoke|bridge|invocation|submit|submitted|Supabase|Trade UI execution|API route activation|production readiness|ENABLE_" docs app lib scripts tests
```
...
- Future-gated hits: expected for Scenario C and mock/review-only order-prep planning.
...
- Future-gated hits are expected for Scenario C and mock/review-only order-prep planning.
- Warning hits remain carried forward from prior warning-class docs and legacy references.
- Blocker hits: none found for this preflight; Scenario C still requires a separate task before any dry-run can occur.

## 14. Scenario C Readiness Decision

`scenario_c_sell_order_prep_preflight_ready_with_warnings`

Recommended next task:

`Task 362 - Execute Scenario C SELL order-prep boundary dry-run, no Avanza, no submit`

Task 362 may only run Scenario C as a mock/review-only SELL order-prep boundary dry-run. It must not use real Avanza order-prep, click SALJ, submit an order, handle credentials/BankID/cookies/sessions, mutate live positions, write Supabase execution records, activate an API route, introduce Trade UI execution, or open production readiness.

## 15. Out Of Scope

- No Scenario C execution in this task.
