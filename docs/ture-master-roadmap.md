...
| B-03 staging readiness | PR #314 verifies only an opaque-reference admission candidate. Action 666IY verified the private writer package, RLS containment and direct-table denial. [ACTION 666IZ](./action-666iz-b03-staging-private-transport-proof-containment.md) then verified current metadata for an existing dedicated least-privileged writer principal and direct-port reachability, but stopped before an authenticated session when its proof token lacked a secret-safe outbound caller path. The stated Pro subscription cannot supply literal private database transport: Supabase PrivateLink requires Team/Enterprise plus an AWS VPC in the project region; IP network restrictions remain public-path allowlisting, not a private path. | Keep B-03 `not_admitted`. A future product/infrastructure decision must first provide an eligible PrivateLink/AWS-VPC path (or explicitly revise the runtime requirement); only then may a separately reviewed operation provide a secret-safe caller transport, one dedicated-principal rollback call and an independent zero-row readback. |
...
| Next decision | [AI-02.17's staging actual-evidence operation](./ai-02.17-staging-actual-evidence-decision.md), [AI-02.18](./ai-02.18-preview-route-attestation-closeout.md) and [AI-02.20](./ai-02.20-proxy-route-attestation-closeout.md) are all closed. AI-02.20 resolved the earlier proxy attribution but did not receive a success receipt. CAT-00.12 consumed its one disposable identity and stopped fail-closed. ACTION 666IZ established that B-03 has an existing dedicated principal and a reachable **public** direct port, then closed fail-closed before a writer call. The stated Pro subscription has no literal private database path: [PrivateLink](https://supabase.com/docs/guides/platform/privatelink) requires Team/Enterprise and a same-region AWS VPC, while [network restrictions](https://supabase.com/docs/guides/platform/network-restrictions) do not make public routing private. | Do not retry consumed AI/CAT proofs or treat 666IZ as a writer result. Do not silently downgrade the private-transport requirement. A separate product/infrastructure decision must choose an eligible PrivateLink/AWS-VPC path or explicitly revise the runtime requirement before any secret-safe caller or rollback-only proof is scheduled. No current receipt authorizes runtime, deployment, broker or production work. |
