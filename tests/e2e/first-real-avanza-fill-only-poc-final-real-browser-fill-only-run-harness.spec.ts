...
function payload(
  ...
  return {
    ...buildSemiAutoRecommendationBuyPayload(
      {
        recommendation_id: "rec-first-fill-only-final-browser-harness-001",
        recommendation_fingerprint:
          "first-fill-only-final-browser-harness-fp-001",
        ticker: "GME",
        quantity: 2,
        order_type: "limit",
        entry_price: 21.98,
        ...
        stop_price: 20,
        target_price: 25,
        created_at: "2026-06-30T17:20:00.000Z",
        expires_at: "2026-06-30T17:50:00.000Z",
        stale_after: "2026-06-30T17:45:00.000Z",
        broker_target_label: "First fill-only final browser harness fixture",
      },
      { now },
    ).payload,
    ...overrides,
...
test.describe("first fill-only POC final real browser fill-only run harness", () => {
  test("harness defaults to disabled", () => {
    const decision =
      buildFirstFillOnlyPocFinalRealBrowserFillOnlyRunHarnessDecision();

    expect(decision.status).toBe("disabled");
    expect(decision.final_real_browser_fill_only_run_harness_enabled).toBe(
      false,
    );
  ...
  test("enabled without final harness gate ready blocks", () => {
    const decision =
      buildFirstFillOnlyPocFinalRealBrowserFillOnlyRunHarnessDecision(
  ...
  test("enabled without approval state blocks", () => {
    ...
    expect(decision.status).toBe("blocked");
    expect(decision.blocked_reasons).toContain(
      "gated_real_browser_fill_only_run:real_browser_run_approval:not_approved_for_fill_only",
    );
  });
  ...
  test("review requested blocks", () => {
    ...
    expect(decision.blocked_reasons).toContain(
      "gated_real_browser_fill_only_run:review_click_requested",
    );
  });
  ...
  test("final confirm requested blocks", () => {
    ...
    expect(decision.status).toBe("failed_safety");
    expect(decision.blocked_reasons).toContain(
      "gated_real_browser_fill_only_run:final_confirm_requested",
    );
  });
  ...
  test("submit or order placement requested blocks", () => {
    ...
    expect(decision.status).toBe("failed_safety");
    expect(decision.blocked_reasons).toContain(
      "gated_real_browser_fill_only_run:order_submit_or_placement_requested",
    );
  });
  ...
  test("credential or session handling requested blocks", () => {
    ...
    expect(decision.status).toBe("failed_safety");
    expect(decision.blocked_reasons).toContain(
      "gated_real_browser_fill_only_run:credential_or_session_handling_requested",
    );
  });
  ...
  test("safe input returns ready for future final fill-only run", () => {
    const decision =
      buildFirstFillOnlyPocFinalRealBrowserFillOnlyRunHarnessDecision(
        safeRequest(),
      );
  ...
  test("ready result exposes run phases, metadata field-fill plan, forbidden selectors, aborts, and evidence", () => {
    const decision =
      buildFirstFillOnlyPocFinalRealBrowserFillOnlyRunHarnessDecision(
        safeRequest(),
      );
  ...
  test("result and function names do not imply order placement", () => {
    const functionName =
      buildFirstFillOnlyPocFinalRealBrowserFillOnlyRunHarnessDecision.name;
    const decision =
      buildFirstFillOnlyPocFinalRealBrowserFillOnlyRunHarnessDecision(
        safeRequest(),
      );
  ...
  test("production harness code does not import live browser, database, route, provider, or audit writer code", () => {
