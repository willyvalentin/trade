...
| Current bounded workstream | AI-02.3 through AI-02.10 are completed on exact main; AI-02.10's additive v2 contract passed its seven-scenario disposable-local PostgreSQL matrix while preserving v1. AI-02.11 has selected one existing server-owned completed-outcome source shape locally but has not observed its availability. The staging-only preflight still found no completed outcome rows. The existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Do not manufacture a cohort or weaken v1. Only a separately authorized staging-only availability preflight may determine whether the selected completed-bundle source exists; it does not authorize migration, identity/digest preflight, insert, containment, minimized readback, evaluator, promotion, writer/runtime, provider/model, deployment, broker or production work. |
...
| Next decision | [AI-02.11's server-owned completed-outcome source profile](./ai-02.11-server-owned-completed-outcome-source-profile.md) selects one bounded candidate source without reading it or granting any authority | Preserve v1 provenance. A separately authorized staging-only availability preflight may establish only whether a completed bundle exists. No migration, source-row read, evidence write, cohort, evaluation, promotion, runtime, provider/model, deployment, broker or production authority follows. |
...
| Agent Intelligence AI-02.11 | [Server-owned completed-outcome source profile](./ai-02.11-server-owned-completed-outcome-source-profile.md) is locally implemented and I/O-free; it selects the authenticated official scheduled outcome-evaluation shape over `public.recommendation_outcomes`, but reports `source_profile_selected_not_available_not_admitted` | Only a separately authorized staging-only availability preflight may check whether a real completed bundle exists. It creates no migration, source-row read, evidence write, evaluator, promotion, runtime, provider/model, deployment, broker or production authority. |
