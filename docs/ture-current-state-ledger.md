...
provider or broker work. Keep private visitor access intact; deploy this
internal execution boundary to staging and observe the next natural cadence.
The earlier MVP-01c probe rollback remains probe-free. Earlier in
...
| MVP-05 | Private staging now has the existing bounded scan and outcome schedules deployed; local recovery/outcome contracts pass | Active: the local raw-function-safe adapter replaces the private-gated HTTP callback. Deploy it only to private staging, retain the bounded cadence and observe the next natural active-window run; no manual/provider bypass. |
...
| MVP-05a | A supported scan uses licensed data within its declared usage budget | active | Private `ture-staging` revision `cafd9c6c`, 2026-09-11: the natural 10:00 America/New_York cadence proved dispatch and secret availability, but its HTTP callback received Netlify's private-login `401`. No provider, broker or production request occurred. The local successor bundles the exact Next route with Next's `react-server` condition, includes it only in the scheduled-function archive, and an extracted archive reached the existing owner-identity `503` gate using a temporary local secret. Deploy only to private staging, then observe the next natural active-window cadence. The 23-check schedule/budget group, 11-check MVP-05 recovery/outcome group and two adapter checks pass locally (36 checks total). |
...
| Active product slices | 1; MVP-05 staging operational-evidence slice has verified scheduler dispatch, secret loading and its local execution boundary; it now needs private-staging behavior evidence | Normally at most one; checkpoint count does not authorize parallel workstreams |
