...
| Current bounded workstream | AI-02.3 through AI-02.15 are completed on exact main. Their prior deploy-preview preflights are historical and did not create a source. The product owner's [AI-02.17 decision](./ai-02.17-staging-actual-evidence-decision.md) separately authorizes one actual, staging-only, calendar-gated source/evaluation operation through an isolated Draft adapter. It is limited to one forced-fresh source request, then one mature outcome bundle with one reused evaluator candle request (at most two provider requests total); no execution has occurred. The existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Preserve v1 provenance and execute AI-02.17 only under its fixed one-shot conditions. It does not authorize a cohort, migration, active-evidence write, offline evaluation, promotion, runtime, deployment, broker or production work. |
...
| Next decision | [AI-02.17's staging actual-evidence operation decision](./ai-02.17-staging-actual-evidence-decision.md) resolves the prior preflight-only transport dead end with one separately authorized, actual staging source/evaluation proof. It distinguishes that new operation from AI-02.16's historical preflight attempts and fixes the provider, calendar, receipt, containment and cleanup limits | Wait for a verified regular or early-close US-market session, execute exactly once, then either record the bounded proof or close it fail-closed. No result authorizes v2 persistence, cohort formation, offline evaluation, promotion, runtime, deployment, broker or production work. |
...
| CAT-00.9 SEC EDGAR evidence-bundle contract | Selected local provider-free contract for protected delivery | It keeps all four future CI/readback claims `claimed_complete_not_independently_verified` and validates no external fact; no request, credential, workflow/policy read, runtime, deployment, broker or production authority follows. |
...
| Agent Intelligence AI-02.17 | [Staging actual-evidence operation decision](./ai-02.17-staging-actual-evidence-decision.md) records the separately authorized one-shot staging source/evaluation proof. It is calendar-gated, source-bounded, receipt-redacted, temporary and unexecuted | One verified US-market session may produce one source and one mature outcome proof. Success or definitive failure requires immediate cleanup and does not authorize a shared migration, cohort, promotion, runtime, deployment, broker or production work. |
