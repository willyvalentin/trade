...
| Agent Intelligence AI-02.18 | [Preview-route attestation](./ai-02.18-preview-route-attestation-decision.md) is a Draft-only, function-route/token diagnostic selected after AI-02.17's opaque 404. Its one-purpose handler is restricted to the expected `trade-vl` deploy-preview and returns a fixed redacted receipt; it imports no application route and has no database, market-data, provider, scanner, source, outcome, evaluator, runtime or broker capability | At most one branch-preview attestation may be attempted after focused and Ready Full CI verification and one fresh branch-scoped token. Any result stops the step and requires cleanup. It cannot retry AI-02.17 or authorize source/provider/staging work. |
...
- AI-02.18 is the independent, route-only prerequisite. It may attest exactly
  one token-protected Draft preview function and returns no deploy identity,
