...
| Current bounded workstream | AI-02.3 through AI-02.8 are completed on exact main. One separately authorized, append-only `historical_synthetic` receipt now exists in `ture-staging`; it is reproducible but `inactive_readiness_only`, has no primary outcome or diagnostic horizons, and is not an evaluation cohort. AI-02.6 profiles only that frozen receipt metadata as `receipt_profiled_not_admitted`; AI-02.7 records the no-automatic-successor decision; AI-02.8 validates exactly one future staging-only evidence-creation plan without authorizing or executing it. The private legacy-evidence relation separately preserves 500 redacted rows fixed as `legacy_incomplete` / `not_admitted` | Do not manufacture a cohort from the receipt or legacy preservation. Only separately authorized execution of AI-02.8's exact staging-only plan, with preflight, containment/rollback and independent minimal readback, could create new evidence; it must not infer evaluator, promotion, writer/runtime, provider/model, deployment, broker or production authority. |
...
| Next decision | AI-02.8 closes the remaining plan-definition ambiguity at the evidence-creation boundary: the only receipt remains non-evaluable, `inactive_readiness_only` and `receipt_profiled_not_admitted`, while the one admissible future plan is validated but `not_authorized_not_executed` | Do not treat `quality_metrics_eligible` or an AI-02.8-valid plan as evaluator, promotion or execution authority. A later evidence-creation operation requires separate exact staging-only authorization, identity/digest preflight, containment/rollback and independent minimal readback before a cohort, evaluation or runtime follow-on can be considered. |
