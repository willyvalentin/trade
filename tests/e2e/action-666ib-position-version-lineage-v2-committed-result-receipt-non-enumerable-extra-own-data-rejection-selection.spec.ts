...
const actionPath =
  "docs/action-666ib-position-version-lineage-v2-committed-result-receipt-non-enumerable-extra-own-data-rejection-selection.md";
const evidencePath =
...
test("666IB selects only the immutable receipt non-enumerable extra-own-data rejection review", () => {
  const evidence = JSON.parse(source(evidencePath));

  expect(evidence.contract_version).toBe(
    "trade.action666ib.position-version-lineage-v2-committed-result-receipt-non-enumerable-extra-own-data-rejection-selection.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "0d3246b43a12ec1003236f9909118dc3c76b3dee",
    protected_main_tree: "b1e06b1b5aa2f5040b46aaff720f58d72a835afa",
    exact_main_ci_run: 33344760305,
    exact_main_ci_conclusion: "success",
    non_enumerable_own_data_rejection_review_action: "ACTION_666IA",
  });
  expect(evidence.candidates).toEqual({
    v2_receipt_comparator_source_or_hidden_key_adaptation:
    ...
    v2_immutable_receipt_equivalence_non_enumerable_extra_own_data_rejection_review:
      "selected",
  });
  expect(evidence.selected_outcome).toMatchObject({
    ...
    review_fixture:
      "one_frozen_local_ordinary_receipt_with_exactly_one_non_enumerable_extra_legacySnapshotId_own_data_key_with_literal_forbidden_value",
  });
  expect(evidence.selected_outcome.required_observations).toEqual([
    "the_hidden_extra_legacySnapshotId_own_data_key_is_visible_to_reflect_own_keys_while_the_five_canonical_fields_remain_enumerable",
    "the_frozen_local_ordinary_extra-own-data_receipt_is_rejected_in_either_argument_slot",
    "every_extra-own-data_rejection_is_a_fresh_dedicated_comparator_error_with_the_stable_public_name_and_message",
  ...
  expect(evidence.decision).toMatchObject({
    type: "select_one_source_only_milestone_b_receipt_equivalence_non_enumerable_extra_own_data_rejection_review",
...
test("666IB retains the comparator boundary without authorizing hidden-key adaptation or runtime", () => {
  ...
  expect(source(roadmapPath)).toMatch(/Action 666IB/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IB/);
});

test("666IB is registered once without changing the six-shard verification plan", () => {
