...
test("route rejects missing automation secret", async () => {
  const response = await withAutomationSecret(() =>
    routePost({
      body: { execute_provider_call: true },
    }),
  );
...
test("route rejects missing execute provider call true", async () => {
  const response = await withAutomationSecret(() =>
    routePost({
...
test("route rejects arbitrary ticker provider or date scope", async () => {
  const response = await withAutomationSecret(() =>
    routePost({
      secret: "test-automation-secret",
      body: {
...
test("approved attempt missing approval signal returns not approved", async () => {
  ...
  expect(result.route_context.execute_provider_call_requested).toBe(true);
  expect(result.route_context.approval_signal_source).toBe("none");
});

test("approved attempt invalid approval blocks provider call", async () => {
  const result = await executeFirstTinyHistoricalFetchApprovedNoPersistAttempt({
    execute_provider_call: true,
    env: {
      ...approvedEnv(),
...
test("approved attempt scope mismatch blocks provider call", async () => {
  const result = await executeFirstTinyHistoricalFetchApprovedNoPersistAttempt({
    execute_provider_call: true,
    env: {
      ...approvedEnv(),
...
test("approved attempt cache hit skips provider call", async () => {
  const result = await executeFirstTinyHistoricalFetchApprovedNoPersistAttempt({
    execute_provider_call: true,
    env: approvedEnv(),
    storage_detection: storageDetection(),
...
test("approved attempt cache lookup unavailable blocks provider call", async () => {
  const result = await executeFirstTinyHistoricalFetchApprovedNoPersistAttempt({
    execute_provider_call: true,
    env: approvedEnv(),
    storage_detection: storageDetection(),
...
test("approved attempt cache miss performs one mocked no-persist provider call", async () => {
  let providerCalls = 0;
  const result = await executeFirstTinyHistoricalFetchApprovedNoPersistAttempt({
    execute_provider_call: true,
    env: approvedEnv(),
