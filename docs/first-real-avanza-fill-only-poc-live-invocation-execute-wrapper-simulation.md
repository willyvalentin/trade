...
# First Real Avanza Fill-Only POC Live Invocation Execute Wrapper Simulation

## Purpose

This document records Action 1068: adding a local-only live invocation execute wrapper simulation for the first real Avanza fill-only POC.

This is not a live run. It does not access Avanza, launch or control a browser, query DOM, click `Granska kop`, click `Bekrafta kop/salj`, submit, place an order, handle credentials/session data, call Supabase, invoke provider/scan routes, or mutate trade/stats/PnL state.

## Simulation Basis

The simulation models the locked ready state:
...
- Real browser approval state: `real_browser_run_approved_for_fill_only`.
- Operator present.
- Manual Avanza login confirmed.
- Account and instrument manually verified.
- Buy side, Avancerad/Limit, amount-based sizing.
- Intended amount: 427,26 SEK.
- Intended price: 21,98 USD.
- Cap: 1,000 SEK or lower.
- Stop point: before review.

The execute wrapper is enabled only in local simulation input. All hard stops remain active.

## Positive Scenarios

- No-runner plan-only simulation reaches `ready_for_live_invocation_execute`.
- Fake/no-op runner simulation reaches `execute_plan_created`.
- Execution phases are exposed.
- Field-fill plan is amount/price only, with total read metadata and stop before review.
- Evidence requirements are exposed.
- Abort conditions are exposed.
- No live browser/Avanza/DOM interaction occurs.
- No review/final/submit/order-placement methods are available.

## Negative Scenarios

The simulation covers blocked or failed-safety results for:
...
- Account or instrument not verified.
- Cap above 1,000 SEK.
- Wrong side.
- Wrong order type.
- Review requested.
...
- Sell/Stop Loss/Glidande requested.

## Runner Boundary

The simulation is both metadata-only and fake-runner-based.

Allowed fake runner methods:
...
No review, final-confirm, submit, place-order, cookie/session, credential, account-switching, or side-switching runner method exists in the production wrapper. The fake runner is local-only and does not access Avanza.
...
- No service-role or environment value access.
...
- Focused Playwright specs passed: `tests/e2e/first-real-avanza-fill-only-poc-live-invocation-execute-wrapper-simulation.spec.ts` and `tests/e2e/first-real-avanza-fill-only-poc-live-invocation-execute-wrapper.spec.ts` passed with 32 tests. The first sandboxed Playwright attempt hit the known local port 3010 `EPERM` startup issue, so the focused specs were rerun with local port-binding escalation.
- Surrounding local safety stack passed with 363 tests, covering live invocation attempt simulation/wrapper, final live invocation simulation/wrapper, final harness, gated real browser run simulation/adapter, real browser adapter simulation/skeleton, execution dry-run simulation/adapter, manual run setup simulation/adapter, gated adapter skeleton, implementation stub, approval state contract, dry-run harness, fill-only guard, selector mapping, human-final-confirmation guard, and browser automation safety boundary.
- `./node_modules/.bin/tsc --noEmit` passed.
- `npm run lint` passed with the existing Babel deopt note for large `app/trade-app.tsx`.
- Source-only safety scan found no production/lib Playwright/Puppeteer import, browser launch/control, DOM query, fetch, Supabase, env/service-role, provider/route/scan, or audit-writer invocation code. Expected matches were limited to docs safety text and the test-only Playwright import.
- Runner-boundary forbidden-method scan found no `clickReview`, `clickConfirm`, `submit`, `placeOrder`, `readCookies`, `readSessionStorage`, `handleCredentials`, `switchAccount`, or `switchSide` method in the production/lib wrapper or simulation runner.
- Status and next-action scans confirmed `first_real_avanza_fill_only_poc_live_invocation_execute_wrapper_simulation_added` and Action 1069.
...
Reason: after the execute wrapper simulation passes, add the final execute checklist immediately before any live execution attempt is considered.
