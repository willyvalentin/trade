import {
  ...
  type AvanzaFillOnlyAdapterContractFixture,
} from "@/lib/avanza-fill-only-adapter-contract-fixtures";

type AvanzaFillOnlyAdapterContractHarnessProps = {
  fixtures?: readonly AvanzaFillOnlyAdapterContractFixture[];
};
...
export function AvanzaFillOnlyAdapterContractHarness({
  fixtures = avanzaFillOnlyAdapterContractFixtures,
}: AvanzaFillOnlyAdapterContractHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Avanza fill-only adapter contract fixtures
        </h3>
        <p className="mt-2 text-xs leading-5 text-zinc-400">
          Static fixture results only. This harness renders explicit
          request/response models and display-only lifecycle states. It does
          not read Trade UI state, does not fetch, does not call a bridge, does
          not control a browser, does not submit orders, and does not write
          execution records.
        </p>
      </div>
            ...
            >
              ...
              <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["mode", formatOptional(request?.mode ?? fixture.mode)],
                  ...
                  ["stopLoss", formatOptional(request?.stopLoss)],
                  ["target", formatOptional(request?.target)],
                  ["timeInForce", formatOptional(request?.timeInForce)],
                  ...
                  ["userMustConfirm", formatBoolean(response.userMustConfirm)],
                  [
