...
type AvanzaPassiveExecutionReadinessPreviewHarnessProps = {
  fixtures?: readonly AvanzaPassiveExecutionReadinessPreviewFixture[];
};
...
export function AvanzaPassiveExecutionReadinessPreviewHarness({
  fixtures = avanzaPassiveExecutionReadinessPreviewFixtures,
}: AvanzaPassiveExecutionReadinessPreviewHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Passive readiness preview fixtures
        </h3>
        <p className="mt-2 text-xs leading-5 text-zinc-400">
          Static fixture models only. This harness renders passive execution
          readiness state without reading Trade UI state, calling APIs, fetching,
          polling, running smoke tests, accessing credentials, controlling a
          browser, submitting orders, or writing execution records.
        </p>
      </div>
            ...
            >
              ...
              <dl className="grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["label", model.label],
                  ["source", model.source],
                  ["selectedTicker", formatValue(model.selectedTicker)],
                  ["selectedSide", formatValue(model.selectedSide)],
