...
The factory is default-off and activates only when `enabled:true` and the
literal boolean `kill_switch_engaged:false` are both present. Missing, null,
numeric, string, array, object, or otherwise malformed gate values remain
disabled. Both gates are checked before the trust boundary, previous bindings,
or any request is read, cloned, looked up, validated, or built. There is no
live call-site.

After activation, the factory creates and validates a private construction-time
snapshot of the registry while retaining only the separately created,
deep-frozen opaque authority. Later caller mutations of posts, payloads,
digests, roots, boundary properties, or lookup method properties cannot change
that snapshot. Before cloning, every active boundary, request, and lookup result
must have a canonical runtime surface: plain objects and dense arrays only,
finite canonical scalars, enumerable data properties, no undefined values,
symbols, accessors, hidden keys, sparse slots, or extra array properties.
Request objects additionally require the exact three-field runtime shape.
Malformed boundaries, posts, payloads, requests, or lookup results return a
...
## Historical Action 666X remediation
...
policy, and canonical row-derived split/diversity evidence. The historical
Action 666W/666X freeze and review artifacts remain provenance only and are not
reused as current authority. Action 666CK independently binds the rebuilt
current-main foundation and its delivery gates; this documentation alone is
not approval.
