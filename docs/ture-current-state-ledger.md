...
| --- | --- | --- |
| MVP-01 | Existing session gate, owner-bound dashboard API, preview sign-in/reload/sign-out evidence and authenticated private-staging dashboard readback | Unverified: MVP-01a and MVP-01b are verified; loading and empty states are observed. Three bounded MVP-01c attempts stayed normal, including one bound to the exact private staging site ID. A subsequent authenticated, values-free ordinary staging page confirmed the exact private site and canonical host but read the intended flag as `false`; follow-up CLI reads showed the value was not stored in either requested scope. A code-only replacement probe passed local checks but Netlify skipped its staging deploy for exhausted account credits before any request. All temporary changes were rolled back. |
...
blocker_or_fallback: The three consumed dashboard reads cannot be retried. Authenticated client access and exact staging-site identity are usable, but the intended flag was not actually stored by the CLI invocation. The currently decisive blocker is external: the Netlify account rejected both the code-only probe deploy and its revert deploy for exhausted credits. A later fourth failed-dashboard read requires separately bounded staging-only scope for the exact corrected mechanism after staging build capacity returns. Do not keep a probe deployed or widen scopes silently.
...
1. Restore private staging build capacity without changing production or
   purchasing anything automatically. Then use the proven code-only exact-site
   probe for one separately bounded authenticated failed-state read, immediately
   revert it and verify the rollback deploy. Do not reuse the three consumed
   reads, weaken the private visitor/authentication boundary, or retain a probe
   after verification.
...
| MVP-01c failed-state proof | Unverified, not a product-runtime defect: three query reads stayed normal. The values-free ordinary-page reader proved authenticated access, exact staging site and canonical host, but observed the intended flag as false; later CLI reads showed no Build or Functions value had been stored. The code-only exact-site replacement passed locally but was never built because Netlify rejected staging deploys for exhausted account credits. All temporary code was reverted in `8f16ee5c`; the last ready staging revision remains clean. | Restore Netlify staging build capacity. Then make one separately bounded read with the local-tested exact-site code gate, immediately revert it and verify the ready clean staging deployment. |
