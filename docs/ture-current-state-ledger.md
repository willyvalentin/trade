...
dedicated-session barriers. Its current rollback revision `bf384012` is ready
as Git deployment `6aa37da73a952200079c07c0`. This is not a production
...
| --- | --- | --- |
| MVP-01 | Existing session gate, owner-bound dashboard API, preview sign-in/reload/sign-out evidence and authenticated private-staging dashboard readback | Unverified: MVP-01a and MVP-01b are verified; loading and empty states are observed. Two bounded MVP-01c attempts stayed normal; the second had the non-secret flag before its Build-and-Functions deploy, so the exact Netlify runtime/configuration boundary remains unproven. Both attempts were fully rolled back without a controlled failed-dashboard observation. |
...
smallest_change_and_reused_components: Reused the previously rollbacked exact-origin probe implementation only on the `staging` branch. It could return the existing sanitized `503 application_data_unavailable` response only after an authenticated session, when all four conditions held: a non-secret enabled flag, dedicated-staging deployment context, canonical `ture-staging.netlify.app` origin and the explicit one-read query parameter. No code reached main.
behavior_check_and_environment: Local Next 16.3.4 production build completed and the two focused dashboard suites passed 8/8. The first temporary private staging deploy `6aa379739547a400085d04f4` for `00015d14` had built before its Functions-only flag was added, so its one read could not activate the probe. A separately bounded second deploy `6aa37cd8f73dc100080fccd8` for `4fbf2f0e` had the non-secret flag in both Build and Functions scopes before it built; the authenticated browser confirmed the exact canonical query URL, but still rendered the ordinary truthful no-trade state rather than the failure branch. Neither attempt is failure-state evidence. Each flag was removed immediately; all code was removed in `bf384012`, whose rollback deploy `6aa37da73a952200079c07c0` is ready.
external_effects_and_existing_authority: Only the private `ture-staging` site and its staging branch were touched. The non-secret flag existed only in that site's production context and was first Functions-only, then separately Build-and-Functions scoped; it was removed after each attempt. No production site, production database, provider, broker, secret or synthetic business record was touched.
blocker_or_fallback: The two consumed reads cannot be retried. The second attempt rules out configuration order alone; the remaining unknown is whether the deployed Next App Router handler receives this Netlify scope/context at all. Before any third attempt, perform a values-free staging configuration/route-boundary diagnosis and obtain a separately bounded scope for its exact mechanism. Do not keep a probe deployed or widen scopes silently.
...
1. Diagnose MVP-01c's staging configuration/route boundary with values-free
   metadata before selecting any new reversible trigger. A third failed-state
   read requires a separately bounded staging-only scope for that exact
   diagnosed mechanism; do not reuse the two consumed reads, weaken the private
...
| MVP-01c failed-state proof | Unverified, not a product-runtime defect: the correctly ordered Build-and-Functions deploy `4fbf2f0e` still did not activate its exact-origin probe; no cause is asserted, and flag/probe were removed in `bf384012` | First perform a values-free metadata/route-boundary diagnosis; only then select a newly authorized reversible staging trigger with one fresh read, preserving the private visitor gate and exact-origin session boundary. |
