...
- Final pre-live review consolidates this gated simulation as a passed prerequisite.
...
- No live run or browser action was performed.
...
- Added final harness local simulation coverage above the gated real browser simulation layer.
- Result status: `first_real_avanza_fill_only_poc_final_real_browser_fill_only_run_harness_simulation_added`.
- The final simulation keeps the gated adapter readied locally while preserving no live browser, Avanza, DOM, fill, click, review/final/submit, or placement.
...
- Added final harness regression coverage on top of the gated simulation stack.
- Result status: `first_real_avanza_fill_only_poc_final_real_browser_fill_only_run_harness_added`.
- The new harness confirms the same hard stops remain active while adding final harness gate and final pre-run evidence gate checks.
- No live run, Avanza access, browser launch/control, DOM query, field fill, click, review/final/submit, or placement was performed.
- Recommended next action: Action 1053 - Add Final Harness Local Simulation.

# First Real Avanza Fill-Only POC Gated Real Browser Fill-Only Run Simulation

## Purpose

This action adds a local-only simulation for the gated real browser fill-only run adapter.
...
- Run gate ready: `real_browser_fill_only_run_gate_ready`.
- Gated run adapter exists.
- Adapter is enabled only in local simulation input.
- Operator presence is simulated as confirmed.
- Manual Avanza login is simulated as confirmed.
- Account and instrument verification are simulated as confirmed.
- Payload is buy-only, amount-based, Avancerad/Limit, and capped at 1,000 SEK.
- All hard stops remain active.

## Positive Scenario

The positive simulation reaches `ready_for_fill_only_browser_run`.

The result exposes:

- Planned phases from `verify_operator_presence` through `stop_before_review`.
- Field-fill plan metadata only for amount, price, and total amount.
- Evidence requirements for visible state, intended values, selector plan, stop-before-review, no review modal, and no final/submit state.
- Abort conditions for mismatch, wrong account/instrument, wrong side/order type, modal/final button risk, total parse failure, cap exceeded, review/final/submit requests, credential/session handling, sell/Stop Loss/Glidande, unattended operation, and uncertainty.
...
The simulation covers these blocking scenarios:

- Adapter disabled.
- Run gate not ready.
- Wrong or missing approval state.
- Operator absent.
...
After the local simulation passes, the next safe step is a final pre-run evidence checklist before any actual real browser fill-only run attempt is considered.
