# Action 307K Original Proxy Notes

Action 307K replaces `proxy.ts` with a standalone diagnostic proxy to isolate a possible proxy runtime crash. The previous implementation is preserved here for restoration/reference only.

Do not import this document from runtime code.
