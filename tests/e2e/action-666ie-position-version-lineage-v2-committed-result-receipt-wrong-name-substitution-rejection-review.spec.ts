...
const actionPath =
  "docs/action-666ie-position-version-lineage-v2-committed-result-receipt-wrong-name-substitution-rejection-review.md";
const evidencePath =
  "docs/evidence/action-666ie-position-version-lineage-v2-committed-result-receipt-wrong-name-substitution-rejection-review.json";
const selectionEvidencePath =
  "docs/evidence/action-666id-position-version-lineage-v2-committed-result-receipt-wrong-name-substitution-rejection-selection.json";
...
const replacementField = "legacyCanonicalCommandDigest";
...
const wrongNameReceiptFields = [replacementField, ...retainedReceiptFields] as const;
const errorName =
...
function wrongNameSubstitutionReceipt() {
  const values = receiptValues();
  const receipt = {} as Record<typeof replacementField | RetainedReceiptField, unknown>;

  Object.defineProperty(receipt, replacementField, {
    value: values.canonicalCommandDigest,
    enumerable: true,
    configurable: false,
    writable: false,
  });
...
function captureWrongNameSubstitutionRejection(
  ...
  throw new Error("expected wrong-name substitution receipt comparison to reject");
}
...
function expectWrongNameSubstitutionReceiptShape(
  receipt: object,
  values: ReceiptValues,
) {
  expect(Object.isFrozen(receipt)).toBe(true);
  expect(Object.getPrototypeOf(receipt)).toBe(Object.prototype);
  expect(Object.keys(receipt)).toEqual(wrongNameReceiptFields);
  expect(Reflect.ownKeys(receipt)).toEqual(wrongNameReceiptFields);
  expect(Object.getOwnPropertyDescriptor(receipt, "canonicalCommandDigest")).toBeUndefined();
  expect(Object.getOwnPropertyDescriptor(receipt, replacementField)).toEqual({
    value: values.canonicalCommandDigest,
    enumerable: true,
    configurable: false,
    writable: false,
...
test("666IE independently reviews wrong-name substitution receipt rejection", () => {
  const comparator = loadComparator();
  const values = receiptValues();
  const control = localReceipt();
  const matchingControl = localReceipt();
  const wrongNameReceipt = wrongNameSubstitutionReceipt();
  const errors: Error[] = [];
  const validVerdict =
    comparator.comparePositionVersionLineageV2WriterImmutableCommittedResultReceipts(
      control,
  ...
  expectWrongNameSubstitutionReceiptShape(wrongNameReceipt, values);
  ...
  for (const [left, right] of [
    [wrongNameReceipt, control],
    [control, wrongNameReceipt],
  ]) {
    for (let call = 0; call < 3; call += 1) {
      errors.push(
        captureWrongNameSubstitutionRejection(comparator, left, right),
      );
    }
  }
...
test("666IE retains the selected wrong-name substitution boundary without widening authority", () => {
  ...
  expect(evidence.contract_version).toBe(
    "trade.action666ie.position-version-lineage-v2-committed-result-receipt-wrong-name-substitution-rejection-review.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "bd2179f28f66498d669d78f1ed5710cdf1842f65",
    protected_main_tree: "046eff836b5f5e3998b529d98118ab03d90bd73e",
    exact_main_ci_run: 33358915058,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666ID",
    selection_pr_number: 269,
  });
  expect(selectionEvidence.action_id).toBe("ACTION_666ID");
  expect(selectionEvidence.selected_outcome).toMatchObject({
    review_action: "ACTION_666IE",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_wrong_name_substitution_rejection_review",
    ...
    review_fixture_exact_enumerable_own_data_fields: wrongNameReceiptFields,
    review_fixture_replaced_canonical_field: "canonicalCommandDigest",
    review_fixture_replacement_field: replacementField,
    review_fixture_replacement_digest_matches_local_control: true,
    review_fixture_replaced_canonical_field_is_absent: true,
    reflect_own_keys_exposes_exact_five_key_wrong_name_shape: true,
    normal_own_data_descriptors_only: true,
    wrong_name_substitution_receipt_rejected_in_either_argument_slot: true,
  ...
  expect(documentation).toMatch(/wrong-name/i);
  expect(documentation).toMatch(/legacyCanonicalCommandDigest/);
  expect(documentation).toMatch(/Reflect\.ownKeys/i);
  expect(documentation).toMatch(/no CI deduplication is\s+authorized/i);
  expect(source(roadmapPath)).toMatch(/Action 666IE/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IE/);
