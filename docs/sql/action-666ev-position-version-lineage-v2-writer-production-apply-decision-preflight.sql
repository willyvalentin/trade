-- Action 666EV: read-only production dependency decision for the v2 writer.
--
-- This transaction returns one aggregate object. It neither returns
-- application-row contents nor changes any database state.
begin read only;
...
select jsonb_build_object(
  ...
  'recommendation_projection_marker_present', exists (
    select 1
  ...
  'position_projection_marker_present', exists (
    select 1
    from pg_catalog.pg_attribute
