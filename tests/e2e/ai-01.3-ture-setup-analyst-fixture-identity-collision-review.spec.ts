...
import {
  ...
  type TureSetupAnalystFixtureEvaluation,
...
function cloneWithIdentity(
  evaluation: TureSetupAnalystFixtureEvaluation,
  values: Readonly<{
    fixtureId: string;
  ...
  return Object.freeze({
    ...evaluation,
    fixture_id: values.fixtureId,
    subject: Object.freeze({
      ...evaluation.subject,
      recommendation_id: values.recommendationId,
    }),
    trace_metrics: Object.freeze({
      ...evaluation.trace_metrics,
      trace_id: values.traceId,
    }),
  });
}

function withFixtures(input: ReturnType<typeof validInput>, fixtures: unknown) {
  return Object.freeze({ ...input, fixture_evaluations: fixtures });
}

test("AI-01.3 rejects duplicate recommendation or trace identity behind a fresh fixture ID", () => {
  const input = validInput();
  const first = input.fixture_evaluations[0];
  const second = input.fixture_evaluations[1];

  const duplicateRecommendation = cloneWithIdentity(second, {
    fixtureId: "fixture:setup-analyst:duplicate-recommendation",
    recommendationId: first.subject.recommendation_id,
    traceId: "trace:setup-analyst:duplicate-recommendation",
  });
  const duplicateTrace = cloneWithIdentity(second, {
    fixtureId: "fixture:setup-analyst:duplicate-trace",
    recommendationId: "recommendation:duplicate-trace",
    traceId: first.trace_metrics.trace_id,
  });

  for (const invalid of [duplicateRecommendation, duplicateTrace]) {
    expect(() =>
      compareTureSetupAnalystMultiFixtureBaseline(
        withFixtures(
          input,
          Object.freeze([first, invalid]),
        ) as CompareTureSetupAnalystMultiFixtureBaselineInput,
      ),
    ).toThrow("Invalid Ture Setup Analyst multi-fixture baseline input.");
  }
});

test("AI-01.3 retains order-independent detached scalar output for unique identities", () => {
  const input = validInput();
  const forward = compareTureSetupAnalystMultiFixtureBaseline(input);
  const reverse = compareTureSetupAnalystMultiFixtureBaseline(
    withFixtures(
      input,
      Object.freeze([...input.fixture_evaluations].reverse()),
    ),
  );

  expect(forward).toEqual(reverse);
  expect(forward).not.toBe(reverse);
  expect(forward.baseline).not.toBe(reverse.baseline);
  expect(forward.summary).not.toBe(reverse.summary);
  expect(JSON.stringify(forward)).not.toContain("recommendation-");
  expect(JSON.stringify(forward)).not.toContain("trace:setup-analyst");
  expect(forward.promotion_disposition).toBe("not_admitted");
});
