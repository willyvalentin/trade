...
**SV-A.2 generic observation-cycle receipt — CLOSED delivery completed,
2026-09-26:** PR #649 merged as
`e3f9ea435d76545a39c4343327833aa7ab8ee67f`. The normal scan route advances one
...
transactional rollback test. Protected exact-main CI run `36234687506` passed;
Netlify production deploy `6ab798ba2ecf42000803665b` is `ready` on the merge
revision. Supabase production records the additive migration as
`20260926100240_sv_a2_observation_cycle_receipts`; production catalog and
privilege readback verifies RLS, service-role-only select/insert/update, no
delete grant, the constrained trigger/function and all four indexes. The table
was empty at verification time. This proves source, migration and deployment
delivery, not live observation behavior or improved strategy quality. Detailed
...
provider budget and persisted retryable failures. Successful and honest
`no_trade` cycles retain the highest admissible quarter-hour cadence; retryable
failures use bounded 15/30/60-minute backoff. Missing/unbounded budget,
unverified session, clock mismatch, unsupported segment and future history
reject before the normal provider path. Manual `force` cannot bypass this gate.

The policy receipt is persisted inside the owner-bound observation-cycle
receipt and exposed in Market Diagnostics. Its authority remains entirely
inert: it cannot call a provider, reserve credits, change ranking, publish or
reach a broker. The separately armed Basic Free reference-only catalog mode is
retained. The older paid background observer was removed from this route so a
`no_request` cannot cause an unbudgeted provider call through a second path.
...
**SV-A.2 observation-cycle pre-run backoff — CLOSED delivery completed,
2026-09-26:** PR #651 merged as exact main
`89c8764c15307badfd374a298ea447815dca1721`. Versioned
...
build pass. Protected PR CI run `36238845640` and exact-main CI run
`36240318337` passed. Netlify production deploy
`6ab7b2aa2cfb2c00080d3f8f` is `ready` on the exact merge revision. No scan was
activated, so OPEN behavior remains separate evidence.

**SV-A.2 observation-cycle browser/readback consumer transition — CLOSED
delivery completed, 2026-09-26:** PR #652 merged as exact main
`a9179bd7a9abb4e050104080abddb05301590e3c`. The dashboard now projects the
owner-bound generic cycle receipt into its compatibility readback for the
latest attempted cycle and latest completed evaluation. A truthful `no_trade`
is therefore a completed engine result with zero publication, while a later
`no_request` remains visible as the latest attempt. Manual diagnostics and
receipts from another New York trading date are excluded. Ordering uses the
attributable scheduled/triggered cycle time rather than a later persistence
time, preventing an old delayed finalization from becoming the newest attempt.
Historical `scheduled_scan_*`, scan-log and recommendation-run consumers remain
as an explicitly named fallback; no rows or contracts are deleted. Market
Diagnostics also exposes the v2 cadence anchor, its source and whether the
retry chain includes a pre-run failure. The projection is read-only and grants
no scheduler, provider, ranking, publication, paper or broker authority.
Delivery evidence is 57 focused projection, receipt, policy, diagnostics,
dashboard and decision/outcome tests plus a 180-test integrated Basic Free,
scheduler, discovery, dashboard and decision/outcome regression. Strict
TypeScript, changed-file ESLint, diff validation, scheduled-runtime packaging
and a complete Next 16.3.4 webpack production build also pass. Protected PR CI
run `36241886484` and exact-main CI run `36243393419` passed. Netlify production
deploy `6ab7c075b5d4da00088605bb` is `ready` on the exact merge revision. A future
OPEN receipt remains separate environment-behavior evidence. See
[SV-A.2 observation-cycle readback consumer transition](./sv-a2-observation-cycle-readback-consumer-transition.md).

**SV-A.2 bounded observation-series control — CLOSED delivery candidate,
2026-09-26:** owner Codex on `codex/observation-series-control`, based on exact
main `a9179bd7a9abb4e050104080abddb05301590e3c`. The default-off controller binds
one date and half-open quarter-hour window to explicit attempt and provider-credit
caps, immutable scheduler/build identity, owner-bound durable history and
automatic expiry. It stops before another provider reservation on publication,
an unresolved cycle, three consecutive failures or exhausted attempt/credit
capacity; `no_trade` remains eligible under the separate cadence/backoff policy.
Malformed configuration or history rejects. Series and one-shot modes conflict,
global scheduler disable stays on, and catalog/outcome/paper workers must stay
off. Current local evidence is 53 focused controller/admission/receipt/scheduler
tests and a 142-test integrated scheduler, discovery, budget and observation
regression. Strict TypeScript, changed-file ESLint, diff validation,
scheduled-runtime packaging and a complete Next 16.3.4 webpack production build
also pass. No series, provider request, migration, publication, paper work or
broker action was activated. Protected PR/CI, merge, exact-main deploy and later
bounded OPEN behavior evidence remain separate. See
...
registered in the protected foundation shard. The schema is now applied in
production as an inert, access-contained foundation; it creates no account,
schedule, provider request, candidate publication or broker path. Runtime
account provisioning/activation, an autonomous worker, exits and OPEN pilot
evidence remain separate successors.
...
The correction merged through PR #624 and was then applied to production as
described below. C.2–C.5/D.1 remain separate because C.2 alters C.1 tables,
...
evidence. PR #624 added the 13-index hardening follow-up as feature revision
`a2c23ddb30752bd7a57f545db1296de92a3e572c`, passed all protected PR checks in
run `36052602141`, and merged as
`d952fb10039e1936baca8e51717445c9940cb2c6`. Exact-main run `36056047302`
passed, and Netlify production deploy `6ab589df6a8b4200088e58c9` is `ready` on
that exact revision.

**SV-C.1 production schema evidence, 2026-09-24:** the Trade Supabase project
recorded the base lifecycle as remote migration `20260924204114` and the
strictly additive FK-index follow-up as `20260924204158`. Post-apply readback
verifies five empty RLS-enabled tables, no `PUBLIC`, `anon` or `authenticated`
table grants, and both `SECURITY DEFINER` functions pinned to
`search_path=pg_catalog, public` with execute authority only for `service_role`.
All 13 follow-up indexes are present, ready and valid; all 16 C.1 foreign keys
have a covering index and no C.1 index is invalid. The security advisor reports
the expected five informational RLS-without-policy findings for these
intentionally service-only tables, with no C.1 warning/error, while the
performance advisor reports zero unindexed C.1 foreign keys and seven
unrelated pre-existing findings. No paper account, intent, fill, position or
ledger row exists, and no runtime flag, provider request, candidate
publication or broker path was activated. C.1 is therefore production-schema
verified, not runtime- or OPEN-pilot-accepted.
...
ledger checks it must replace by validated deparsed definition rather than name
alone, requires the two identity columns it relaxes to still be `uuid NOT NULL`,
and removes data backfills, permissive
...
function exists. The same harness now also proves that a same-named but altered
amount check and a nullable pre-C.2 `intent_id` each fail with
`sv_c2_unexpected_c1_ledger_contract` before any C.2 schema appears. After
restoring the exact C.1 contract, the migration passed the full
partial/final/stop, retry/restart, owner isolation, reconciliation and
...
or worker behavior was intentionally altered. Production readback corroborates
...
Local acceptance includes eleven focused adversarial tests covering
...
validation. Strict TypeScript, changed-file lint, full repository lint with zero
errors/eight existing warnings and a complete Next production build pass.
Feature revision `53709df00a278583e2722158cf8888b607967c21` passed all six
protected provider-free CI shards, aggregate and merge-candidate provenance in
run `36093710360`, then merged through PR #637 as
`27d606790291fa1239e4772c2191655d96e85016`. Netlify production deploy
`6ab5fac8f8fe850008aa526d` is `ready` in `production` on `main` at that exact
merge revision. The slice contains no IBKR transport, credential read, database
write, provider request, broker submission or live-account authority.
Account/auth/session feasibility and real IBKR Paper readback remain later L
CLOSED/OPEN evidence.

**Selected SV-L.2 CLOSED vertical slice — IBKR paper read-only session
feasibility, 2026-09-25 (8–16h):** owner Codex on isolated
`codex/sv-l2-ibkr-session-feasibility`; interfaces
`ibkr_session_feasibility_v1` and `ibkr_session_probe_receipt_v1`. The pure
contract validates only an already supplied, short-lived and redacted
account/session probe receipt. It requires exact paper-account identity, the
transport-specific authentication shape, initialized brokerage session,
account/buying-power/positions/open-orders/executions readback, US-stock
permission, persistent server-worker ownership, server-only secrets, one active
broker session, health evidence, explicit pacing and maintenance constraints,
and disabled order/live/recommendation-data paths. Browser and short-lived
function connection owners fail closed. Missing session or worker restart proof
remains explicit operator-restoration debt rather than a claim of unattended
authentication; missing market-data entitlement remains a separate gap rather
than being inferred from account access.

This source-only evaluator does not connect to IBKR, inspect credentials,
authenticate, query an account, persist evidence or authorize submit/modify/
cancel. Local fixtures therefore prove the admission contract only, not account
...
suite is registered in the protected provider-free foundation shard. Feature
revision `fbfadf9a2d7015a2e56011d0bb8d04c0f2601717` passed all six protected
provider-free CI shards, aggregate and merge-candidate provenance in run
`35735500430`. PR #589 merged as
`f67e63e4cc72a9246291f0a784621b3765033f80`; Netlify production deploy
`6ab28b7445dcad00083cc59c` is `ready` in `production` on that exact merge
revision. This proves source deployment only; no historical corpus, provider
request, candidate publication, policy comparison or strategy-quality
evaluation ran in production.

**Selected SV-F.1 CLOSED vertical slice — frozen paired replay experiment,
2026-09-22 (8–16h):** owner Codex on
`codex/sv-f1-frozen-replay-experiment`; interfaces
`internal_paper_replay_experiment_v1`,
`internal_paper_replay_experiment_manifest_v1` and
`internal_paper_replay_experiment_result_v1`. The provider-free runner binds an
exact baseline and distinct candidate E.4 corpus before either result is
inspected. Both corpora must use the same owner, paper account, deterministic
seed, account/execution policy, point-in-time dataset/source/rights references,
calendar, corporate-action policy, dates, universe snapshots and exact
opportunity population. It freezes every baseline/candidate session input,
baseline and evaluation-charter fingerprints, policy lineages, a 15/30/60m
primary outcome horizon and chronological non-empty training, validation,
held-out and walk-forward partitions. Each partition transition must have at
least the frozen outcome horizon between the prior close and next open.

The runner independently replays both E.4 corpora and reports deterministic,
six-decimal costed session/decision/position/PnL/expectancy/drawdown measures
and paired deltas for every partition. Any post-freeze mutation, changed
opportunity population, mixed source/account/execution fact, identical policy,
blocked child corpus or unsafe numeric result fails closed; it never shrinks
the population. A completed result is explicitly
`research_engine_result_not_strategy_accepted` with no calibrated-probability,
charter-threshold, uncertainty/effective-sample or forward-shadow verdict.
Nine focused adversarial tests pass, including a complete all-`no_trade`
experiment. This is repository-fixture engine evidence only. A later F slice
...
SV-H.3 passed all six protected provider-free CI shards, aggregate and
merge-candidate provenance in PR #602, then merged as main
`e9cb91c3a2cf5e939e26d4fe21daf639546f83ae`. Netlify's current
production deploy `6ab358f84f69e40008bddcf7` is `ready` on that exact
revision. This verifies source/deploy only; no real IOC fill, provider request,
...
Next production build also pass. PR #646 then passed all six protected
provider-free CI shards plus aggregate and merge-candidate provenance, and
merged as main `968aacfc377ddb5344b5c89495c3af0d5c2dc0c9`. Netlify production
deploy `6ab687adae4793000852226c` is `ready` on that exact revision. Protected
source/deploy delivery is therefore complete, while a later separately bounded
OPEN receipt remains required; the 2026-09-25 predecessor observation is not
environment verification of the repair.

**A.2 bounded outcome-evaluation observation result, 2026-09-25:** exact main
`968aacfc377ddb5344b5c89495c3af0d5c2dc0c9` remained frozen throughout the
authorized one-shot. Activation deploy `6ab68ce0a6abfb5a4ca86657` was the ready
production release for the exact `2026-09-25T16:15:00.000Z` slot. The ordinary
Netlify scheduler delivered exactly one owner-bound attempt: the route received
it at `16:15:20.888Z` and retained a terminal `blocked` research-only receipt at
`16:15:22.127Z`. Its explicit blocker was that no non-diagnostic official live
batch existed for the day. The receipt therefore had zero eligible/evaluated
snapshots, zero planned/executed candle requests, zero created/updated/skipped
outcomes and `not_attempted` persistence. Authoritative reservation readback
showed zero matching Basic Free reservations, zero reserved credits and zero
provider attempts. The bounded owner/window readback also showed zero new scan
runs, snapshots, recommendations, persisted outcomes or execution records.

This is a **pass for scheduled delivery and truthful terminal receipt**, but a
blocked/inconclusive outcome-persistence result: it proves neither outcome
coverage nor recommendation quality. It also cannot environment-verify the
terminal scan-trace repair because no normal scan ran. Cleanup reset only the
outcome one-shot flag to false and removed its date/slot while the global
scheduler disable stayed true. Git-connected cleanup production deploy
`6ab69f41af5839d77c37462f` is `ready` on the same exact main revision, and
readback confirms the three outcome one-shot values are false/absent. No retry
is implied. The next useful OPEN evidence is a separately frozen normal scan on
the next eligible New York trading date after the v2 credit allocation is
merged and exact-revision deployed.

**Selected A.2 CLOSED Basic Free pre-ranking credit allocation v2,
2026-09-25 (4–8h):** owner Codex on isolated
`codex/a2-pre-ranking-credit-allocation`, based on exact main
`968aacfc377ddb5344b5c89495c3af0d5c2dc0c9`. The 2026-09-25 normal OPEN scan
reserved and finalized exactly eight credits, evaluated eight universe members,
retained three observed/ranked candidates and honestly published zero. Its
source trace exposed a separate data-fitness bottleneck: budget policy v1
reserved two market-regime calls, only one shared pre-ranking scanner call and
five post-ranking reference-refresh calls. Because ranking requires fresh
candidate evidence first, those five later calls were unreachable when every
selected input was stale or incomplete. This is not evidence for lowering a
ranking or publication threshold.

The local v2 policy preserves the same atomic eight-credit ceiling and daily
claim, but reallocates it as two market-regime plus six shared scanner credits
and zero post-ranking refresh credits. The scheduled route now passes one
versioned budget object through both stages. The scanner's existing shared
counter covers daily-candle and intraday-indicator provider-capable calls, and
fresh cache reuse still reserves no provider credit. A fresh intraday reference
captured before ranking remains directly usable by the plan-reference gate, so
zero later attempts do not block an already-fresh candidate. Diagnostic and
paid-plan behavior are unchanged. The durable active-scan trace additionally
records the v2 policy, cap, total provider-capable reservations, separate daily-
candle/intraday reservation counts and fresh intraday-cache reuse, allowing the
later OPEN receipt to prove actual stage allocation rather than infer it from
logs. Local evidence currently includes 26/26 focused budget, freshness and
fetch-attribution regressions, 12/12 applicable adjacent scheduler,
owner-boundary and dynamic-budget checks, strict TypeScript, changed-file lint,
diff check and a
complete Next webpack production build. The exact cost-bounded Draft verifier
also passes lint, strict TypeScript, 17 security tests, 22 registered
affected tests and all 904 browser/server containment tests. The historical
Action 324 inventory-only verifier intentionally rejects a dirty runtime
worktree and is not this
slice's acceptance contract.
...
the arrival minute's *final* volume to size an IOC fill at that minute's open.
A deterministic red test changed only that future volume and changed the
reported fill from five to ten shares. The locally implemented, versioned
E.2/H.3 interfaces now cap simulated IOC quantity with the preceding
completed regular minute's volume. A first-minute IOC or absent prior volume
blocks as unknown. The result names the proxy's candle, volume and model
version; this is an arrival-time liquidity estimate, never a quote-level or
exchange fill receipt. Acceptance is the red-to-green point-in-time
regression, no first-minute fabricated fill,
unchanged future-volume perturbation, downstream E.3/E.4/F.1/H.3 replay
tests, strict TypeScript, lint, build, protected CI and exact-main deploy.
The change owns only provider-free replay code, fixtures and this ledger: no
schema, runtime writer, provider, ranking/publication or broker path. Licensed
point-in-time data, actual prior-candle provider availability at arrival, and
a real forward fill comparison remain unverified. Local verification: 77/77
E.1–H.3 replay tests, strict TypeScript, changed-file lint with zero findings,
repository lint excluding generated bundles with zero errors and eight
...
| --- | --- | --- |
| Primary development | SV-A.2 bounded observation-series control is locally implemented on `codex/observation-series-control` from exact main `a9179bd7a9abb4e050104080abddb05301590e3c`. It adds the default-off attempt/credit caps, stop conditions, immutable identity and automatic expiry required before multi-slot OPEN use. | Complete full regression/build, protected PR/CI, merge and exact-main deploy. Keep every series flag off; only a separately frozen OPEN contract may activate it later. |
...
receipt computes the response disposition from explicit time band, upstream,
request/response and receipt timestamps, coverage counts and bounded policy
limits. It distinguishes accepted, partial, stale, delayed, ambiguous and
unavailable observations; a caller-supplied `accepted` label cannot bypass a
non-accepted computed result. Regular and extended-hours observations get
...
### Closed supporting delivery — IF-2b symbol-master freshness admission

**Active IF-2b source-freshness admission, 2026-09-20:** While the separately
authorized Monday capability probe remains the next `OPEN` acceptance, this
...
`basic_free_catalog_collection_plan_v3` now separates page-coverage capacity
from snapshot coherence: it names the page count and quota-day estimate, marks
whether the page schedule must span quota days, and withholds any
coherent-snapshot claim because one has not been separately observed and
...
not presented as one. Before merge, eight focused plan/readback tests, 26
affected Basic Free discovery/collection regressions, targeted ESLint, strict
TypeScript, the Webpack production build and the 166-test
intelligence-foundation suite (including isolated PostgreSQL checks) passed.
The authenticated browser readback itself was not exercised because no local
credentials were read or entered; the route/readback contract asserts the
panel's exact user-facing branch text, so that UI behavior is not
environment-verified. There is no provider, migration, configuration,
candidate, publication or broker effect. The separately bounded Monday Basic
Free capability probe remains the next market-bound action.
...
**Merged and production-verified IF-1 source-timestamp integrity, 2026-09-19:**
PR [#551](https://github.com/willyvalentin/trade/pull/551) merged as
`79e570444d99a75fb094d2ecbc1b78d83fe0ede1` after protected CI
`35412733103` passed. Netlify production deploy
`6aadea7812250500072e9ad1` is ready on that exact revision. The retained
decision record now uses only a parseable canonical reference-price or
indicator-cache timestamp. Missing, invalid or capture-future source time is
retained as the explicit existing `candidate_provider_gap`, and source evidence
later than the actual decision cannot appear as `fresh`. Readback rejects
malformed timestamps and future source evidence that lacks that explicit gap;
it still presents an honestly gapped record for investigation. This is a
decision-evidence integrity correction under IF-1, not a scanner, ranking,
confidence, publication, provider, migration or execution change. The focused
decision-record, rejected-research and baseline-readiness suite passed 36/36;
strict TypeScript, targeted ESLint and the Webpack production build passed
locally. No provider request, database migration, broker action or staging
invocation occurred. The authenticated production behavior remains
unverified because the available browser session is at the login boundary; the
deploy does not prove live data freshness or stronger recommendation quality.
...
Delivery state: MVP-02, MVP-03, MVP-04 and MVP-05 corrections are merged on
GitHub main. The server-owned MVP-03 attestation correction merged as PR #478
at `ff66d7e6`; its Ready Full CI passed all six unchanged shards, the aggregate
gate and merge-candidate provenance, and the exact-main post-merge attestation
passed. A separate private `ture-staging.netlify.app` site remains the only
non-production behavior-evidence environment. No public-production deployment
or production database operation occurred in this delivery.
...
| MVP-06 | Private staging can complete a fresh authenticated recommendation-card readback and exact cleanup | Unverified: complete the manual candidate journey, then applicable release checks and supervised market-session evidence. |
...
| MVP-03c | Record an exit and reload the correct closed state without a broker call | verified | Private staging used the ordinary close form for a labelled synthetic position: 5/10 shares closed, reload showed the durable partial state, then the final 5 shares closed. Readback showed one closed row and no broker or provider call. |
| MVP-04a | Closed history preserves plan versus actual prices, quantity and timestamps | unverified | — |
| MVP-04b | Realized result and aggregate statistics reconcile, with explicit fee assumptions | verified | The synthetic two-fill staging close used explicit zero commission and FX fees. Durable readback and reloaded Statistics reconciled $35.00 PnL, 3.5%, 0.70R, one winner, zero active positions and one closed exit-fill record. |
...
blocker_or_fallback: The staging and protected-main PR/CI/merge sequences are complete. The full MVP-06 journey remains separately unverified and must use a supported market-session/provider path or the selected cache-only boundary; no test payload may be invented to bypass the form's manual broker attestation.
result_and_remaining_gap: The current UI workflow remains compatible while incomplete direct API writes now fail closed. This is a main-delivered security/data-integrity correction and does not change the 9/18 isolated-staging behavior count or release acceptance.
```
...
- At the time of this narrow entry/retry/read observation, MVP-03c and
  MVP-04a/b remained unverified. Later same-day staging lifecycle evidence,
  recorded in the active board above, verified MVP-03c and MVP-04b only;
  MVP-03a/MVP-03b/MVP-04a still require their stated full-journey or
  closed-history evidence. A future supported lifecycle proof needs either a
  genuine manually confirmed broker fill or a separately designed, visibly
  synthetic test-only exit-capture path that does not weaken the
  human-confirmation production control.

### MVP-03 server-owned broker-attestation boundary — 2026-09-12

- PR #476 (`7d144457`) was first squashed into `staging` as `082ecda`. Its
  Ready Full CI run passed all six provider-free shards, their aggregate, and
  the exact merge-candidate-provenance check. The Netlify `ture-staging`
  deploy reported `ready` for that same staging revision.
- The main-delivery PR #478 then merged as `ff66d7e6` after Ready Full CI run
  `34683717171` passed all six unchanged shards, aggregate and merge-candidate
  provenance. Exact-main post-merge attestation run `34684961333` passed for
  that same revision. Netlify still reports the pre-existing public
  `trade-vl` deploy as current, so this records no public-production publish.
