...
const actionPath =
  "docs/action-666io-position-version-lineage-v2-committed-result-receipt-omitted-position-version-own-data-rejection-review.md";
const evidencePath =
  "docs/evidence/action-666io-position-version-lineage-v2-committed-result-receipt-omitted-position-version-own-data-rejection-review.json";
...
const omittedCanonicalField = "positionVersion";
const errorName =
...
function omittedPositionVersionOwnDataReceipt() {
...
function captureOmittedPositionVersionOwnDataRejection(
  ...
  throw new Error("expected omitted position-version own-data receipt comparison to reject");
}
...
function expectOmittedPositionVersionOwnDataReceiptShape(
  receipt: object,
...
test("666IO independently reviews omitted position-version own-data receipt rejection", () => {
  const comparator = loadComparator();
  const values = receiptValues();
  const control = localReceipt();
  const matchingControl = localReceipt();
  const omittedReceipt = omittedPositionVersionOwnDataReceipt();
  const errors: Error[] = [];
  const validVerdict =
    comparator.comparePositionVersionLineageV2WriterImmutableCommittedResultReceipts(
      control,
  ...
  expectOmittedPositionVersionOwnDataReceiptShape(omittedReceipt, values);
  ...
  for (const [left, right] of [
    ...
    for (let call = 0; call < 3; call += 1) {
      errors.push(captureOmittedPositionVersionOwnDataRejection(comparator, left, right));
    }
  }
...
test("666IO retains the selected omitted position-version own-data boundary without widening authority", () => {
  ...
  expect(evidence.contract_version).toBe(
    "trade.action666io.position-version-lineage-v2-committed-result-receipt-omitted-position-version-own-data-rejection-review.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "2b1b2fe0e0c10fa65a97190bd639768661854dbf",
    protected_main_tree: "af0e72ba1e93849514a640e3feb5ac38c8ef4ad5",
    exact_main_ci_run: 33433837603,
    exact_main_ci_conclusion: "success",
    selection_action: "ACTION_666IN",
    selection_pr_number: 279,
  });
  expect(selectionEvidence.action_id).toBe("ACTION_666IN");
  expect(selectionEvidence.selected_outcome).toMatchObject({
    review_action: "ACTION_666IO",
  });
  expect(evidence.review).toMatchObject({
    type: "independent_source_only_immutable_committed_result_receipt_omitted_position_version_own_data_rejection_review",
    ...
    omitted_position_version_own_data_receipt_rejected_in_either_argument_slot: true,
  ...
  expect(documentation).toMatch(/omitted position-version/i);
  ...
  expect(source(roadmapPath)).toMatch(/Action 666IO/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IO/);
