...
function verifiedChain() {
  ...
  return { approval, execution, requestPreview, operatorApproval };
}
...
function baseDiagnosticsInput(): MarketDiagnosticsConsoleInput {
  ...
  return {
    ...
    live_market_trial_runbook: {
      status: "ready",
      phase: "regular",
      next_action: { label: "Review first tiny execution plan" },
      blockers: [],
      warnings: [],
    },
    scan_orchestration: {
    ...
    scanner_output_qa: {
      ...
      metadata_coverage: {
        ...
        qa_checked_source_path:
          "first_tiny_historical_fetch_execution_plan_test",
...
test("empty input does not throw and remains advisory plan only", () => {
  const plan = buildFirstTinyHistoricalFetchExecutionPlan();

  expect(plan.advisory_only).toBe(true);
  expect(plan.execution_plan_only).toBe(true);
  expect(plan.execution_plan_status).toBe("not_requested");
  expect(plan.readiness.ready_to_execute_now).toBe(false);
});

test("missing operator approval blocks", () => {
  const { requestPreview } = verifiedChain();
  const plan = buildFirstTinyHistoricalFetchExecutionPlan({
    request_preview: requestPreview,
  });

  expect(plan.execution_plan_status).toBe("blocked");
  expect(plan.blockers).toContain("operator_approval_record_missing");
});

test("missing request preview blocks", () => {
  const { operatorApproval } = verifiedChain();
  const plan = buildFirstTinyHistoricalFetchExecutionPlan({
    operator_approval: operatorApproval,
  });

  expect(plan.execution_plan_status).toBe("blocked");
  expect(plan.blockers).toContain("request_preview_not_ready");
});

test("pending operator decision and ready preview create future approval plan", () => {
  const { operatorApproval, requestPreview } = verifiedChain();
  const plan = buildFirstTinyHistoricalFetchExecutionPlan({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  });

  expect(plan.execution_plan_status).toBe("ready_for_future_approval");
  expect(plan.execution_context.operator_approval_status).toBe(
    "pending_operator_decision",
  );
  expect(plan.execution_context.request_preview_status).toBe("ready");
  expect(plan.readiness.ready_to_review_execution_plan).toBe(true);
});

test("pending operator decision does not execute", () => {
  const { operatorApproval, requestPreview } = verifiedChain();
  const plan = buildFirstTinyHistoricalFetchExecutionPlan({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  ...
  expect(plan.execution_context.provider_call_allowed_now).toBe(false);
  expect(plan.execution_context.persistence_allowed_now).toBe(false);
  expect(plan.readiness.ready_to_execute_now).toBe(false);
});

test("planned steps are ordered and never execute now", () => {
  const { operatorApproval, requestPreview } = verifiedChain();
  const plan = buildFirstTinyHistoricalFetchExecutionPlan({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  ...
  expect(plan.planned_steps.map((step) => step.order)).toEqual([
    1, 2, 3, 4, 5, 6, 7, 8,
  ]);
  expect(plan.planned_steps.every((step) => step.executes_now === false)).toBe(
    true,
  );
  expect(plan.planned_steps[2]).toMatchObject({
    step_id: "provider_request_if_cache_miss",
    status: "blocked_until_future_approval",
  });
  expect(plan.planned_steps[5]).toMatchObject({
    step_id: "fetch_run_audit_plan",
    status: "planned_no_write",
  });
  expect(plan.planned_steps[6]).toMatchObject({
    step_id: "candle_persistence_plan",
    status: "planned_no_write",
  });
  expect(plan.planned_steps[7]).toMatchObject({
    step_id: "post_fetch_replay_gate",
    status: "blocked_separate_approval_required",
  });
});

test("request scope and execution limits stay tiny", () => {
  const { operatorApproval, requestPreview } = verifiedChain();
  const plan = buildFirstTinyHistoricalFetchExecutionPlan({
    operator_approval: operatorApproval,
    request_preview: requestPreview,
  ...
  expect(plan.request_scope.provider).toBe("twelve_data");
  expect(plan.request_scope.endpoint).toBe("time_series");
  expect(plan.request_scope.ticker).toBe("COIN");
  expect(plan.request_scope.interval).toBe("5min");
  expect(plan.request_scope.request_count).toBe(1);
  expect(plan.request_scope.estimated_credits).toBe(1);
  expect(plan.execution_limits.max_tickers).toBe(1);
  expect(plan.execution_limits.max_trading_days).toBe(1);
  expect(plan.execution_limits.max_requests).toBe(1);
  expect(plan.execution_limits.max_estimated_credits).toBe(1);
});
...
test("runtime effects remain disabled", () => {
  ...
  expect(plan.readiness.ready_to_execute_now).toBe(false);
  expect(plan.readiness.ready_to_call_provider_now).toBe(false);
  expect(plan.readiness.ready_to_persist_candles_now).toBe(false);
  expect(plan.readiness.ready_to_create_fetch_run_now).toBe(false);
  expect(plan.readiness.ready_to_create_synthetic_outcomes).toBe(false);
  expect(plan.readiness.ready_to_run_replay).toBe(false);
  expect(plan.readiness.ready_to_affect_scanner).toBe(false);
...
test("diagnostics section prints expected safety lines", () => {
  ...
  const section = diagnostics.sections.find(
    (item) => item.section_id === "first_tiny_historical_fetch_execution_plan",
  );
  ...
  expect(section?.lines).toContain("Execution plan only: yes");
  expect(section?.lines).toContain(
    "Execution plan status: ready_for_future_approval",
  );
  expect(section?.lines).toContain(
    "Operator approval status: pending_operator_decision",
  );
  expect(section?.lines).toContain("Request preview status: ready");
  ...
  expect(section?.lines).toContain("Persistence allowed now: no");
  ...
  expect(section?.lines).toContain(
    "Planned step 1: operator_approval_check / planned / executes now no",
  );
  expect(section?.lines).toContain(
    "Planned step 3: provider_request_if_cache_miss / blocked until future approval / executes now no",
  );
  expect(section?.lines).toContain(
    "Planned step 6: fetch_run_audit_plan / planned no write / executes now no",
  );
  expect(section?.lines).toContain(
    "Planned step 7: candle_persistence_plan / planned no write / executes now no",
  );
  expect(section?.lines).toContain(
    "Planned step 8: post_fetch_replay_gate / blocked separate approval required / executes now no",
  );
  expect(section?.lines).toContain("Ready to review execution plan: yes");
  ...
  expect(
    intelligence?.lines.some((line) =>
      line.startsWith(
        "First tiny fetch execution plan: ready_for_future_approval / execute no / provider call no / persist no",
      ),
    ),
  ).toBe(true);
});
