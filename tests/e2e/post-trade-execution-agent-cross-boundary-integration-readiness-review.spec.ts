...
import { CREDENTIAL_SOURCE_ADAPTER_BOUNDARY_IDENTITY, buildCredentialSessionCapability, buildFixtureNoCredentialRequirementCapability } from "../../lib/post-trade-credential-source-adapter-boundary-core";
...
test("valid cross-boundary structural inputs remain fixture-only and deterministic", () => {
  ...
  const credentialSession = buildCredentialSessionCapability();
  expect(credentialSession).toMatchObject({
    capabilityKind: "credential_session",
    capabilityVersion: 1,
    intendedPurpose: "no_credential_required",
    fixtureOnly: true,
    containsSecretMaterial: false,
  });
  expect(credentialSession.capabilityFingerprint).toMatch(/^[a-f0-9]{64}$/u);
  expect(buildFixtureNoCredentialRequirementCapability({ boundarySessionId: credentialSession.boundarySessionId }).fixtureOnly).toBe(true);
  expect(new Set([
    TRUSTED_LIVE_RESOLVER_ADAPTER_IDENTITY.resolverId,
    SCOPED_MACOS_PROCESS_OBSERVER_IDENTITY.observerId,
    DIRECT_SPAWN_DRIVER_BOUNDARY_IDENTITY.driverId,
    CREDENTIAL_SOURCE_ADAPTER_BOUNDARY_IDENTITY.adapterId,
  ]).size).toBe(4);
});
