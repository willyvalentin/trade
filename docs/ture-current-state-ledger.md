...
| CAT-00.12 GitHub evidence-read operator decision | `pre_authorized_pending_identity_verification`: the decision constrains one future disposable fine-grained identity to the CAT-00.10 five GET-only metadata paths, in-memory redaction, CAT-00.11 validation and immediate revocation | No token has been created or used and no GitHub request has been made. The broad local development credential is excluded. Identity verification and the exact `Actions: read`, `Administration: read`, `Metadata: read` repository scope are still required before the one-shot operation; no SEC request, runtime, deployment, broker or production authority follows. |
...
- CAT-00.11 is complete on exact main as PR #407. It can validate only
  ...
  external authority. CAT-00.12 now supplies the fresh, bounded operator
  decision, but its five GET-only GitHub operation remains blocked pending a
  dedicated least-privileged identity and independent evidence. It cannot
