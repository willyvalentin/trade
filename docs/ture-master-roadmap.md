...
| Current bounded workstream | AI-02.3 through AI-02.12 are completed on exact main. AI-02.10's additive v2 contract passed its seven-scenario disposable-local PostgreSQL matrix while preserving v1; AI-02.11's separately authorized staging-only aggregate preflight returned `no_completed_bundle_available` without returning a source row, count, owner, fingerprint or payload. AI-02.12 fixes one future source-creation design without executing it and explicitly excludes the existing five-batch/ten-snapshot scheduled producer. AI-02.13 is a provider-free candidate that fixes the future cost ceiling at one official-snapshot, one-batch and one reused provider candle request. The existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Do not manufacture a cohort or weaken v1. Only a separately authorized staging-only, non-secret credential-presence and branch-transport preflight may decide whether an isolated branch-deploy adapter can be prepared. It does not authorize provider invocation, migration, source-row read, insert, evaluator, promotion, writer/runtime, deployment, broker or production work. |
...
| Next decision | [AI-02.11's availability preflight](./ai-02.11-staging-availability-preflight.md) established that the selected staging outcome source has no completed bundle, without returning source material; [AI-02.12's server-owned source-creation admission](./ai-02.12-staging-completed-outcome-source-creation-admission.md) fixes one future one-shot design; [AI-02.13's cost and transport admission](./ai-02.13-staging-one-shot-cost-transport-admission.md) bounds it to one provider candle request and excludes the scheduled function's five-batch/ten-snapshot default | Preserve v1 provenance. A separately authorized staging-only, non-secret credential-presence and branch-transport preflight may establish only whether an isolated branch-deploy adapter can be prepared. No provider invocation, migration, source-row read, evidence write, cohort, evaluation, promotion, runtime, provider/model, deployment, broker or production authority follows. |
