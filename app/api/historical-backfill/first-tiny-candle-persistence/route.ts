...
type FirstTinyCandlePersistenceRouteBody = {
...
async function parseBody(
  request: Request,
): Promise<FirstTinyCandlePersistenceRouteBody> {
  try {
    const text = await request.text();
    if (!text.trim()) return {};
    const parsed = JSON.parse(text);
    return parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as FirstTinyCandlePersistenceRouteBody)
      : {};
  } catch {
    return {};
  }
...
function hasRejectedOverride(body: FirstTinyCandlePersistenceRouteBody) {
  return [
    body.ticker,
    body.provider,
    body.endpoint,
...
export async function POST(request: Request) {
  ...
  if (body.execute_candle_persistence !== true) {
    return jsonNoStore(
      {
  ...
  const preflight = await executeFirstTinyCandlePersistence({
    execute_candle_persistence: true,
  });

  if (
    preflight.execution_status !== "blocked" ||
    !preflight.blockers.includes("supabase_service_role_unavailable")
  ) {
