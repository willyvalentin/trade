...
- Final pre-live review includes the captured approval state `real_browser_run_approved_for_fill_only`.
...
- Approval still does not mean a live run has happened.
...
# First Real Avanza Fill-Only POC Real Browser Run Approval Capture

## Purpose

This document captures the exact real browser run approval for the first real Avanza fill-only POC.

This is documentation and approval-capture only. It is not a real browser run, not implementation, not order execution, and not approval to place an order. This action does not launch a browser, attach to a browser, access Avanza, query DOM, fill fields, click buttons, submit orders, or place trades.

## Approval Source

The user provided the exact approval text in chat for Action 1044.
...
This text matches the Action 1043 required approval text.

## Approval Scope

- First real Avanza browser fill-only run.
- Buy-only.
- Avancerad/Limit.
- Amount-based sizing.
- Max cap 1,000 SEK or lower.
- User present.
- Avanza already opened and logged in manually by the user.
...
- Previous state: `real_browser_run_not_approved`.
- New state: `real_browser_run_approved_for_fill_only`.

## Remaining Hard Stops

- Final confirmation remains permanently forbidden.
- Review click remains forbidden for the first run unless a separate future approval changes it.
- Order placement remains forbidden.
- Credentials/session handling remains forbidden.
- Any mismatch or uncertainty aborts.

## What This Approval Still Does Not Authorize

- It does not authorize clicking Granska köp.
- It does not authorize opening or interacting with the review modal.
- It does not authorize clicking Bekräfta köp/sälj.
- It does not authorize submitting or placing an order.
- It does not authorize unattended operation.
- It does not authorize credential, 2FA, session, cookie, localStorage, or sessionStorage access.
- It does not authorize sell, Stop Loss, or Glidande behavior.
- It does not authorize a cap above 1,000 SEK.
...
After approval capture, the next step should add a final run gate that verifies all prerequisites immediately before any future fill-only real browser run action is considered. That next step should still add no browser automation.
