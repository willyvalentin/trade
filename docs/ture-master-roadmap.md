...
| REL-00 CI-B2 raw diff acquisition | CI-B0 is verified on exact main, and CI-B1 is verified on exact main `7ca4543c…` after PR #291; the six-shard Ready/main CI model remains unchanged | Build the source-only, fail-closed raw NUL name-status acquisition seam without changing the workflow; CI-B7 activation needs a separately authorized required-check/branch-protection decision |
