...
restored on 2026-09-11: the temporary MVP-01c revision `92f21f1d` built ready
as deploy `6aa3dac454f41d0008922e64`, and its clean rollback `b1fd5f09` is
ready as `6aa3db8f92f450000959f084`. This is not a production deployment; the
public production release remains unchanged, while the new main candidate
remains unpublished behind its deployment lock.
...
blocker_or_fallback: The three consumed dashboard reads cannot be retried. The flag path is closed because its intended value was not stored. After Netlify capacity returned, the local-tested code-only exact-site mechanism built ready and a single browser navigation was attempted; it reached application sign-in before any dashboard request because that browser session had expired. The temporary code was immediately reverted and the ready rollback is `6aa3db8f92f450000959f084`. Re-establish one private staging application session before any later separately bounded read; do not keep a probe deployed or widen scopes silently.
result_and_remaining_gap: The staging environment is clean and the probe is absent, but MVP-01c remains unverified because the controlled failed state was not actually observed. Netlify capacity is no longer a blocker. MVP-01 remains unaccepted at release scope.
```

### Next — ordered, next product slice

1. Re-establish a private staging application session. Then use the proven
   code-only exact-site probe for one separately bounded authenticated
   failed-state read, immediately revert it and verify the rollback deploy. Do
   not reuse the three consumed reads, weaken the private visitor/authentication
   boundary, or retain a probe after verification.
...
| MVP-01c failed-state proof | Unverified, not a product-runtime defect: three query reads stayed normal. The values-free ordinary-page reader proved authenticated access, exact staging site and canonical host, but observed the intended flag as false; later CLI reads showed no Build or Functions value had been stored. The code-only exact-site replacement then built ready once Netlify capacity returned, but its one browser navigation stopped at application sign-in before dashboard access because the session had expired. All temporary code was rolled back in `b1fd5f09`; clean rollback deploy `6aa3db8f92f450000959f084` is ready. | Re-establish the private staging application session. Then make one freshly authorized, separately bounded authenticated dashboard read with the local-tested exact-site code gate, immediately revert it and verify the ready clean staging deployment. |
