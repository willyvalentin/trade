...
| Current bounded workstream | AI-02.3 through AI-02.8 are completed on exact main. AI-02.10 supplies an additive v2 active-evidence contract whose seven-scenario migration matrix passed in disposable local PostgreSQL on 2026-09-06, preserving v1; the staging-only preflight still found no completed outcome rows. The existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Do not manufacture a cohort or weaken v1. Separately select and authorize one bounded real server-owned completed-bundle source before any staging migration, identity/digest-preflight, insert, containment or minimized readback is reconsidered. No evaluator, promotion, writer/runtime, provider/model, deployment, broker or production authority follows. |
...
| Next decision | [AI-02.10 active-evidence v2 contract and local migration](./ai-02.10-active-evidence-contract-and-migration.md) implements the additive, default-deny compatibility boundary required by AI-02.9 and has passed its disposable local PostgreSQL proof; it does not apply the migration to a shared database or create a source bundle | Preserve v1 provenance. Select a bounded real server-owned completed-bundle source under separate staging-only authority before any staging migration is reconsidered. A compatibility fix must not itself create a cohort, evaluation, promotion, runtime, provider/model, deployment, broker or production authority. |
...
| Agent Intelligence AI-02.10 | [Additive active-evidence v2 contract and local migration](./ai-02.10-active-evidence-contract-and-migration.md) are implemented locally; the relation is separate from v1, RLS-contained, append-only and has no application-role grant, and its seven-scenario disposable PostgreSQL proof passed on 2026-09-06 | A separate real server-owned completed-bundle source binding remains required. The package does not apply a migration to a shared database, access staging, write evidence, form a dataset, evaluate, bind runtime or affect provider/model, deployment, broker or production authority. |
