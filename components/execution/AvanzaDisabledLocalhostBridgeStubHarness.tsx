import {
  ...
  type AvanzaDisabledLocalhostBridgeStubFixture,
} from "@/lib/avanza-disabled-localhost-bridge-stub-fixtures";

type AvanzaDisabledLocalhostBridgeStubHarnessProps = {
  fixtures?: readonly AvanzaDisabledLocalhostBridgeStubFixture[];
};
...
export function AvanzaDisabledLocalhostBridgeStubHarness({
  fixtures = avanzaDisabledLocalhostBridgeStubFixtures,
}: AvanzaDisabledLocalhostBridgeStubHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Disabled localhost bridge stub fixtures
        </h3>
        <p className="mt-2 text-xs leading-5 text-zinc-400">
          Static fixture results only. This harness renders explicit disabled
          localhost bridge stub states for a future local-only Avanza fill-only
          bridge. It does not add an API route, does not expose an endpoint,
          does not fetch, does not call a bridge, does not control a browser,
          does not fill forms, does not click review or confirm, does not submit
          orders, and does not write execution records.
        </p>
      </div>
            ...
            >
              ...
              <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["stubRequestId", formatOptional(request?.stubRequestId)],
                  ...
                  ["stubEnabled", formatBoolean(response.stubEnabled)],
                  [
                    "canExposeEndpoint",
