...
type AvanzaModelOnlyLocalDevBridgeDryRunnerHarnessProps = {
  fixtures?: readonly AvanzaModelOnlyLocalDevBridgeDryRunFixture[];
};
...
export function AvanzaModelOnlyLocalDevBridgeDryRunnerHarness({
  fixtures = avanzaModelOnlyLocalDevBridgeDryRunFixtures,
}: AvanzaModelOnlyLocalDevBridgeDryRunnerHarnessProps) {
  return (
    <section className="grid gap-4 rounded-md border border-white/10 bg-black/20 p-3">
      <div>
        ...
        <h3 className="mt-3 text-sm font-semibold text-zinc-100">
          Model-only local-dev bridge dry-run fixtures
        </h3>
        <p className="mt-2 text-xs leading-5 text-zinc-400">
          Static dry-run metadata only. The model simulates a bridge run to the
          invocation boundary, then stops before smoke runners, terminal
          scripts, browser automation, credentials, API calls, order behavior,
          final KÖP/SÄLJ, or Supabase writes.
        </p>
      </div>
            ...
            >
              ...
              <dl className="grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  ["disabledRunnerId", report.disabledRunnerId],
                  ...
                  ["simulatedOutcome", report.simulatedOutcome],
              ...
              <div className="grid gap-2 rounded-md border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="grid gap-2">
                  {report.simulatedSteps.map((step) => (
                    <div
                    ...
                    >
                      ...
                      <p className="mt-1 text-zinc-500">
                        simulatedInputSummary:{" "}
                        {formatValue(step.simulatedInputSummary)}
                      </p>
                      <p className="mt-1 text-zinc-500">
                        simulatedOutputSummary:{" "}
                        {formatValue(step.simulatedOutputSummary)}
                      </p>
                      <p className="mt-1 text-zinc-500">
                        stopCondition: {formatValue(step.stopCondition)}
                      </p>
                      <p className="mt-1 text-zinc-500">
              ...
              <div className="grid gap-2 text-xs sm:grid-cols-2">
                <div className="rounded-md border border-white/10 bg-white/[0.03] p-3">
                  <h4 className="font-semibold text-zinc-200">
                    Would require before real invocation
                  </h4>
                  <p className="mt-2 text-zinc-400">
                    {formatValue(report.wouldRequireBeforeRealInvocation)}
                  </p>
                </div>
                <div className="rounded-md border border-white/10 bg-white/[0.03] p-3">
                  <h4 className="font-semibold text-zinc-200">
