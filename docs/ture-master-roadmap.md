...
| Agent Intelligence AI-02.17 | [Staging actual-evidence operation decision](./ai-02.17-staging-actual-evidence-decision.md) records the separately authorized one-shot staging source/evaluation proof. It is calendar-gated, source-bounded, receipt-redacted, temporary and unexecuted. A read-only [source-route budget audit](./ai-02.17-source-route-budget-audit.md) found that Draft PR #397 can make an unbudgeted Polygon calendar request on a cold cache | Resolve and verify that readiness blocker before any verified US-market session may produce one source and one mature outcome proof. The eventual proof still requires immediate cleanup and does not authorize a shared migration, cohort, promotion, runtime, deployment, broker or production work. |
