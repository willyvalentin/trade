...
const expected = Object.freeze({
  ...
  nextAction: "action_490_full_candidate_command_inventory_binding_remediation_gate",
...
const missingNames = Array.isArray(record.command_inventory_missing_items)
  ? record.command_inventory_missing_items.map((item) => item.name)
  : [];
...
const checks = {
  ...
  action488_approval:
    ...
    action488.deployment_performed === false &&
    action488.preview_activated === false,
  action487_aborted:
    action487.rehearsal_decision === "full_candidate_rehearsal_aborted" &&
    action487.abort_reason === "source_safety_marker_detected" &&
    action487.next_action === "action_488_source_safety_marker_classification_remediation_gate",
  documentation_contract:
    hasAll(doc, [
      "Action 489 executed exactly one local full-candidate rehearsal attempt",
      "Source-safety result: `source_safety_passed`",
      "Dependency copy: `passed_temporary_verified_node_modules_copy`",
      "failed_required_command_artifacts_missing_from_bound_candidate",
      expected.nextAction,
    ]),
  candidate_binding:
  ...
  source_reconstruction:
    ...
    record.source_reconstruction_result ===
  ...
  overlay_inventory_safety:
    record.overlay_verification_result ===
  ...
  source_safety:
    record.source_safety_result === "source_safety_passed" &&
    record.source_safety_filename_indicators_advisory === true &&
    record.source_safety_unknown_sensitive_file_count === 0 &&
    record.source_safety_prohibited_file_count === 0 &&
    record.raw_credential_values_recorded === false,
  git_integrity:
    record.source_only_git_integrity_result ===
  ...
  dependency_materialization:
    ...
    record.dependency_copy_result === "passed_temporary_verified_node_modules_copy" &&
  ...
  command_inventory_abort:
    record.command_inventory_result === "failed_required_command_artifacts_missing_from_bound_candidate" &&
    record.command_inventory_missing_count === 9 &&
    missingNames.includes("focused Action 481 suite") &&
    missingNames.includes("focused Action 489 suite") &&
    record.serial_commands_started === false &&
    record.no_parallel_temp_execution === true &&
    commandResults.length === 27 &&
    commandResults.every((command) =>
      ["not_run_due_command_inventory_unresolvable", "not_run_missing_required_candidate_artifact"].includes(
        command.status,
      ),
    ),
  runtime_and_flag:
    ...
    record.alternate_preview_flag_alias_detected === false &&
  ...
  attempt_and_next_action:
    ...
    record.abort_reason === "command_inventory_unresolvable_in_bound_30_file_candidate" &&
...
const report = {
  ...
  source_safety_result: record.source_safety_result ?? null,
  source_only_git_integrity_result: record.source_only_git_integrity_result ?? null,
  dependency_copy_result: record.dependency_copy_result ?? null,
  extraneous_packages_excluded: record.extraneous_packages_excluded ?? null,
  command_inventory_result: record.command_inventory_result ?? null,
  command_inventory_missing_count: record.command_inventory_missing_count ?? null,
