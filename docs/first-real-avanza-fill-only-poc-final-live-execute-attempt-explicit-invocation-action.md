...
Action 1079 adds a typed, explicit-trigger-only invocation action around the existing Action 1073 final live execute attempt wrapper.

This action does not perform a live run. It does not access Avanza. It does not launch or control a browser. It does not query DOM. It does not fill real fields unless a separately approved injected runner is provided in a future action. It does not click anything. It does not open a review modal. It does not submit or place an order. It does not handle credentials or session data. It does not mutate trades, stats, or PnL.

## 2. Safety boundary

The action is safe by construction:

- Disabled by default.
- Explicit-trigger-only.
- Dependency-injected runner only.
- No default live runner.
- No browser/page/DOM dependency.
- No Avanza integration.
- No Supabase/provider/route/scanner/audit-writer dependency.
- Not wired to UI, routes, provider, scanner, package scripts, cron, or automatic jobs.
- Not capable of review, final confirm, submit, or order placement.

Ready never means execution occurred.

## 3. Gate basis

The explicit invocation action is based on:

- Final live execute attempt execution gate ready: `final_live_execute_attempt_execution_gate_ready`
- Final live execute attempt checklist confirmation ready: `final_live_execute_attempt_checklist_confirmation_ready`
- Final live execute attempt checklist ready: `final_live_execute_attempt_checklist_ready`
- Final execute attempt gate ready: `final_execute_attempt_gate_ready`
- Execute checklist confirmation ready: `execute_checklist_confirmation_ready`
- Final live invocation execute checklist ready: `final_live_invocation_execute_checklist_ready`
- Live invocation execution gate ready: `live_invocation_execution_gate_ready`
...
- Existing wrapper added: `first_real_avanza_fill_only_poc_final_live_execute_attempt_wrapper_added`
- Existing wrapper simulation added: `first_real_avanza_fill_only_poc_final_live_execute_attempt_wrapper_simulation_added`
...
## 4. Explicit invocation action behavior

The action imports and uses the existing Action 1073 wrapper module:

`lib/first-real-avanza-fill-only-poc-final-live-execute-attempt-wrapper.ts`

It adds a final explicit invocation layer that verifies the Action 1078 execution gate, the Action 1077 confirmation state, the Action 1075 checklist state, and the required operator trigger before delegating to the wrapper.

If the action is disabled, it returns `final_live_execute_attempt_explicit_invocation_disabled`.

If any readiness, scope, or safety input is missing or unsafe, it blocks or aborts before runner calls.

If no runner is injected and all gates pass, it returns `ready_for_final_live_execute_attempt_explicit_invocation`.

If a local fake/no-op runner is injected in tests and all gates pass, it delegates to the existing wrapper and may return `final_live_execute_attempt_explicit_invocation_plan_created` after the approved stop-before-review sequence.
...
The action requires `operatorExplicitlyRequestedFinalLiveExecuteAttempt: true`.

If the explicit operator trigger is absent, false, stale, or ambiguous, the action must not call the wrapper runner.

The action also requires operator presence confirmation and a fresh final checklist confirmation that matches the captured confirmation scope.

## 7. Locked approved values
...
## 8. No-runner behavior

With all required readiness states satisfied, explicit operator trigger true, operator present, locked scope valid, and no runner injected, the action returns:

`ready_for_final_live_execute_attempt_explicit_invocation`

This means the explicit invocation action is ready for a separately approved injected runner boundary. It does not mean execution occurred.

No runner is called. Avanza, browser, DOM, Supabase, provider, scanner, route, audit writer, credentials, sessions, trades, stats, and PnL remain untouched.

## 9. Fake/no-op local test behavior

With all required readiness states satisfied, explicit operator trigger true, operator present, locked scope valid, and a local fake/no-op runner injected in tests only, the action may call the existing Action 1073 wrapper and return:
...
No new runner method is added by Action 1079.

## 11. Approved runner sequence

The only approved runner sequence remains:
...
The action must stop after `stopBeforeReview`.

## 12. Mandatory abort/block conditions

The action must abort or block before runner calls if any of these are true:

- Action disabled.
- Explicit operator trigger absent, false, or ambiguous.
- Operator presence not confirmed.
- Final live execute attempt execution gate not ready.
- Final live execute attempt checklist confirmation not ready.
- Final live execute attempt checklist not ready.
- Final execute attempt gate not ready.
- Execute checklist confirmation not ready.
- Final live invocation execute checklist not ready.
- Live invocation execution gate not ready.
- Immediate pre-invocation confirmation not ready.
- Final operator GO missing or wrong.
- Final pre-run evidence not ready.
- Live invocation run attempt gate not ready.
- Fresh final checklist confirmation absent or stale.
- Account expectation missing or mismatched.
- Instrument expectation missing or mismatched.
- Side is not buy-only.
- Order mode is not Avancerad/Limit.
- Amount is not exactly the approved amount.
- Price is not exactly the approved price.
- Cap is missing or above 1,000 SEK.
- Total parse fails.
- Total exceeds cap.
- Any modal state is open or unknown.
- Final confirm visibility is true or unknown.
- Bekräfta köp visibility is true or unknown.
- Bekräfta sälj visibility is true or unknown.
- Review click is requested.
- Granska köp click is requested.
- Submit/order placement is requested.
- Credential/session handling is requested.
- Cookies/localStorage/sessionStorage handling is requested.
- Sell is requested.
- Stop Loss is requested.
- Glidande Stop Loss is requested.
- Automatic or unattended mode is requested.
- Unsupported runner method is present or requested.
- Any uncertainty exists.

The action must abort immediately if the existing wrapper returns any abort, block, or safety-failure result.

## 13. Forbidden behavior

The action must not include or perform:

- No live run.
- No browser launch/control.
- No Avanza access.
- No DOM query implementation.
- No click.
...
- No Supabase/provider/route/scan/audit-writer invocation.
- No trade/stats/PnL mutation.

## 14. Status meanings

- `final_live_execute_attempt_explicit_invocation_disabled`: action is disabled by default.
- `final_live_execute_attempt_explicit_invocation_blocked`: pre-run gate, trigger, scope, or safety input blocked runner calls.
- `ready_for_final_live_execute_attempt_explicit_invocation`: all gates passed with no runner injected; this does not mean execution occurred.
- `final_live_execute_attempt_explicit_invocation_plan_created`: local fake/no-op runner exercised the stop-before-review plan; this does not mean order placement.
- `final_live_execute_attempt_explicit_invocation_aborted`: wrapper or runner safety result aborted the plan.
...
Action 1080 must still not perform a live run, must still stop before Granska köp, and must still not place an order.
...
- This was local implementation and local test coverage only.
- Action 1079 adds an explicit invocation action only. It does not perform a live run.
- No live Avanza/browser/DOM/fill/click/review/final/submit/order-placement activity occurred.
- No Supabase/provider/scan/audit-writer invocation occurred.
- No credentials/session data were handled.
- No trades, stats, or PnL were mutated.
- Denial harness scripts were skipped because they would execute live Supabase checks outside this local-only action.
