...
- Final pre-live review confirms this run gate remains ready.
...
- Added final harness local simulation coverage.
- Result status: `first_real_avanza_fill_only_poc_final_real_browser_fill_only_run_harness_simulation_added`.
- The simulation proves local readiness only and does not perform a real browser run, Avanza access, DOM query, fill, click, review/final/submit, or placement.
- Recommended next action: Action 1054 - Final Pre-Live Run Review.

## Action 1052 Follow-Up - Final Harness Added

- Added the final real browser fill-only run harness decision module and regression tests.
- Result status: `first_real_avanza_fill_only_poc_final_real_browser_fill_only_run_harness_added`.
- The harness composes the existing gated real browser fill-only run adapter and adds final harness plus final pre-run evidence checks.
- It remains disabled by default and does not perform a live run, browser launch/control, Avanza access, DOM query, field fill, click, review/final/submit, or placement.
...
# First Real Avanza Fill-Only POC Real Browser Fill-Only Run Gate

## Purpose

This document adds the real browser fill-only run gate for the first real Avanza fill-only POC.
...
- Exact real browser run approval captured in Action 1044.
- Approval state: `real_browser_run_approved_for_fill_only`.
- Operator setup evidence ready: `operator_setup_ready_for_manual_run_setup`.
- Manual run setup gate ready: `manual_run_setup_gate_ready`.
- Manual run setup simulation passed.
- Execution dry-run adapter gate ready: `execution_dry_run_adapter_gate_ready`.
...
- All locked-scope hard stops remain active.
...
| Approval state approved for fill-only | pass | `real_browser_run_approved_for_fill_only` |
| Operator setup evidence ready | pass | `operator_setup_ready_for_manual_run_setup` |
| Operator screenshot evidence captured | pass | Operator setup evidence trail |
| Manual setup simulation passed | pass | Action 1036 simulation |
...
| Selector contract exists | pass | `docs/real-avanza-selector-mapping-contract.md` |
| Guard/cap/final-submit protections exist | pass | Guard contract and cap policy docs |
| Real browser run action added | warn | Not added yet, expected |
| Real browser run performed | warn | Not performed yet, expected |
| Real run evidence package exists | warn | Not created yet, expected |

## Run-Gate Decision

Run-gate decision: `real_browser_fill_only_run_gate_ready`.

This means ready to add a future real browser fill-only run adapter/action. It does not mean the run has been performed.

## Allowed Future Run Scope
...
- Adapter may read only required visible order-form state.
- Adapter may fill only approved amount/price fields.
- Stop before Granska köp.
- No review click.
- No final confirm.
- No submit or order placement.
- No credentials, 2FA, session, cookies, localStorage, or sessionStorage handling.
- Abort on mismatch or uncertainty.

## Mandatory Abort Conditions

- User not present.
- Browser not already manually opened.
- Avanza not already manually logged in.
- Account not manually verified.
- Instrument not manually verified.
- Wrong instrument.
- Wrong account.
- Wrong side.
- Wrong order type.
- Cap above 1,000 SEK.
- Total parse failure.
- Amount or price mismatch.
- Validation errors.
- Modal open.
- Review click requested or targeted.
- Final confirm button visible or targeted.
- Submit/order placement requested.
- Credential/session handling requested.
- Any uncertainty.

## What Next Action May Add

Action 1046 may add a gated real browser fill-only run adapter skeleton/path, but it must:

- Be disabled by default.
- Require explicit run input.
- Require approved state.
- Require operator presence.
- Require manual login already completed.
- Never handle credentials.
- Stop before review.
- Never click review or final confirmation.
- Never submit or place an order.
- Expose full evidence package requirements.
- Keep final selector hard-stop.

## What Next Action Must Not Add

- No final click.
- No review click unless separately approved later.
- No submit or order placement.
- No credential/session handling.
- No unattended mode.
- No sell, Stop Loss, or Glidande behavior.
- No cap above 1,000 SEK.
- No Supabase, trade, or PnL mutation unless separately approved.
...
After this final run gate, the next step can add a gated adapter that is capable of a narrowly scoped fill-only browser run, still stopping before Granska köp and still never submitting.
