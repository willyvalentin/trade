...
| REL-00 CI-B3 shadow reconciliation | CI-B0 is verified on exact main; CI-B1 and CI-B2 are also verified on exact main, with CI-B2 merged as PR #292 (`5ceeb1e…`) and matching post-merge provenance. The six-shard Ready/main CI model remains unchanged | Build and verify an inert, source-only CI-B3 receipt that reparses CI-B2 raw NUL name-status evidence at Tier 3 without selector, workflow or required-check changes; CI-B7 activation still needs a separately authorized required-check/branch-protection decision |
