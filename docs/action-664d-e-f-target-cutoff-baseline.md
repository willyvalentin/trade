...
baseline is available without a network action during the test. The Action
664D static contract keeps that checkout property under regression coverage:
every checkout step must retain full history while this immutable baseline is
required.
