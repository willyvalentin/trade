...
const actionPath =
  "docs/action-666ih-position-version-lineage-v2-committed-result-receipt-omitted-disposition-own-data-rejection-selection.md";
const evidencePath =
  "docs/evidence/action-666ih-position-version-lineage-v2-committed-result-receipt-omitted-disposition-own-data-rejection-selection.json";
...
test("666IH selects only the immutable receipt omitted disposition own-data rejection review", () => {
  const evidence = JSON.parse(source(evidencePath));

  expect(evidence.contract_version).toBe(
    "trade.action666ih.position-version-lineage-v2-committed-result-receipt-omitted-disposition-own-data-rejection-selection.v1",
  );
  expect(evidence.reviewed_revision).toEqual({
    protected_main_commit: "1f9d59781b13980889238d4296fc40de3355d344",
    protected_main_tree: "29651df3a406c8a3291ec6830d23afc71d8da64f",
    exact_main_ci_run: 33376606690,
    exact_main_ci_conclusion: "success",
    omitted_canonical_own_data_rejection_review_action: "ACTION_666IG",
  });
  expect(evidence.candidates).toEqual({
    v2_receipt_comparator_source_or_omitted_disposition_adaptation:
    ...
    v2_immutable_receipt_equivalence_omitted_disposition_own_data_rejection_review:
      "selected",
  });
  expect(evidence.selected_outcome).toMatchObject({
    identifier:
      "v2_immutable_receipt_equivalence_omitted_disposition_own_data_rejection_review",
    review_action: "ACTION_666II",
    comparator_source_changed: false,
    server_only: true,
    runtime_unwired: true,
    execution_authority: false,
    review_fixture:
      "one_frozen_local_ordinary_receipt_with_exactly_four_enumerable_normal_own_data_fields_canonicalCommandDigest_initialHistoryIdentity_positionId_and_positionVersion_while_disposition_is_absent_without_a_replacement_key",
  });
  expect(evidence.selected_outcome.required_observations).toEqual([
    "object_keys_and_reflect_own_keys_expose_exactly_the_four_retained_canonical_data_keys_while_disposition_is_absent",
    "the_frozen_local_ordinary_omitted_disposition_own_data_receipt_is_rejected_in_either_argument_slot",
    "every_omitted_disposition_own_data_rejection_is_a_fresh_dedicated_comparator_error_with_the_stable_public_name_and_message",
  ...
  expect(evidence.decision).toMatchObject({
    type: "select_one_source_only_milestone_b_receipt_equivalence_omitted_disposition_own_data_rejection_review",
...
test("666IH retains the exact-cardinality disposition omission boundary without authorizing adaptation or runtime", () => {
  ...
  expect(source(comparatorPath)).toContain("disposition");
  ...
  expect(documentation).toMatch(/omitted disposition/i);
  ...
  expect(source(roadmapPath)).toMatch(/Action 666IH/);
  expect(source(ledgerPath)).toMatch(/ACTION 666IH/);
});

test("666IH is registered once without changing the six-shard verification plan", () => {
