...
| Current bounded workstream | AI-02.3 through AI-02.8 are completed on exact main. The 2026-09-06 staging-only AI-02.8 preflight found two pre-write blockers: the deployed v1 envelope constraint requires `inactive_readiness_only=true`, whereas AI-02.8 requires `false`, and the outcome-minimized staging aggregate has zero rows. The one existing `historical_synthetic` receipt remains reproducible but non-evaluable, and the private legacy-evidence relation remains `legacy_incomplete` / `not_admitted` | Do not manufacture a cohort or weaken the v1 inactive contract. First implement and locally verify an additive active-evidence contract plus compatible schema migration and a bounded real server-owned completed-bundle source; only then can a separately authorized staging migration and the AI-02.8 identity/digest-preflight, one bounded insert, containment and independent minimal readback be reconsidered. No evaluator, promotion, writer/runtime, provider/model, deployment, broker or production authority follows. |
...
| Next decision | [AI-02.9 active-evidence schema compatibility blocker](./ai-02.9-active-evidence-schema-blocker.md) records why the separately authorized AI-02.8 staging execution was stopped safely before DML: v1 structurally requires inactive evidence while the plan requires active evidence | Preserve v1 provenance. Select an additive active-evidence contract and local schema-migration implementation before any new staging authorization. A compatibility fix must not itself create a cohort, evaluation, promotion, runtime, provider/model, deployment, broker or production authority. |
